...
- Prettier + ESLint are authoritative; run `pnpm format` before committing **and again before opening a PR** — unformatted code fails CI regularly. Two ways that goes wrong, both of which have already turned a PR red:
  - **`pnpm install` first, always.** With no `node_modules`, `pnpm format` does not fail — it silently runs whatever `prettier` is on `PATH` (a sandbox or globally-installed one), which is a **different version** from the lockfile's. It reports success, rewrites files the pinned Prettier disagrees with, and `format:check` then fails in CI on the very files you just "formatted". Confirm you are running the repo's binary (`./node_modules/.bin/prettier --version`) before trusting a clean `pnpm format`, and treat "CI flags a file I just formatted" as this, not as drift. **`pnpm install` is also what installs the safety net**: the husky pre-commit hook (`prepare` → lint-staged) formats staged files with the pinned Prettier on every commit, so with `node_modules` present this class of mistake cannot reach a commit at all — and without it there is no hook, which is exactly why the failure looks like the repo's fault.
  - **`pnpm format` is repo-wide (`prettier --write .`), so check what it touched before staging.** Stage your own files by name rather than `git add -A`. If it reformatted files your change never touched, that is the signal above — the wrong binary — not pre-existing drift to sweep into your commit. `main` is formatted; assume a surprise diff is yours.
