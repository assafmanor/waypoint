...
- **The done MARK is also the undo control** _(revision, 2026-07-16; the mark changed 2026-09-15, [ADR-0230](0230-one-done-mark-and-it-is-the-undo.md))._ Tapping the mark un-marks the event; it is a `role="button"` inside the card face (not a nested `<button>`) that stops propagation, so it undoes without also toggling the row open. **Which mark carries it changed.** The revision put it on the green ✓ circle and kept `שחזר` in the expanded strip as _"the discoverable twin"_, because the circle's hover/focus morph is invisible to a phone tap. ADR-0230 took the twin seriously and moved the control to the thing that already carries the WORD: the `היינו ✓` chip. The circle and the strip's `שחזר` are both gone; the chip is the one mark, it is `--ok` green (a record, never amber), and it needs no morph to explain itself. The revision's reasoning survives its own implementation.
- **Settling is reversible wherever it is allowed — including on a past day** _(revision, 2026-07-16)._ A past day within the live trip lets you _mark_ Done/Skip (§4, the retrospective job), so it must also let you _un-mark_: the interactive mark stays live on a read-only past day — the chip renders wherever the done state does, which is what makes ADR-0230's deletion of the strip's `שחזר` cost nothing here. Restore is the inverse of a Done we already permit there — offering one without the other left "check but never uncheck," a mistake with no correction. (Structural edits — create/edit/delete/move — remain Plan-gated, ADR-0029; this covers settle status only.) Reversing settle on a **finished trip** (a Plan-mode read-only archive, ADR-0040) is out of scope here and tracked as its own decision.
...
- **"אירוע חדש" is right-sized to Tier-1.** Adding on the ground is a real need, but today's button opens the full builder (any day, hard/soft, bookings) — that is Tier-2/3 _building_ (ADR-0025). Replace it with a **quick-add scoped to a soft event, today, at the next open slot**; hard events, exact scheduling, bookings, and other days route to Plan (the ⋯ sheet or the mode escape). On a **past day the add affordance is hidden entirely** — create is locked (ADR-0029) and the archive is read-only. _Amended 2026-09-16 ([ADR-0231](0231-the-day-is-changed-where-you-stand.md) §3): what shipped for this was the full builder prefilled at the day's last end — ⁦23:15⁩ for ⁦44⁩ minutes at ⁦13:51⁩, counted on the running app. The quick add exists now (`QuickAddSheet`), lands on **now** rather than "the next open slot", and is what the ＋, the gap `+` and `החלפה`'s new-event open on today._

### 4. The archive chrome — day view + top bar

- **Past-day list (Trip mode):** a calm, slightly desaturated **archive wash** led by a read-only banner. No now-line (there is no "now" on a day that's over), no amber. Editing is gated to Plan (ADR-0029), but **Done / Skip / Restore / Navigate stay** — an unresolved item can still be settled (the reframed Done's retrospective job), and a settled one can be un-settled (its inverse; see §2).
