...
- **The green ✓ is also the undo control** _(revision, 2026-07-16)._ The done badge doubles as a one-tap restore: tapping it un-marks the event (identical to `שחזר`), the fast twin of the expanded verb. It stays `--ok` green (a record, never amber); on hover/focus the ✓ morphs to an undo arrow with a soft ring, and a thin persistent ring hints it is tappable on touch. It is a `role="button"` inside the card face (not a nested `<button>`) that stops propagation, so it undoes without also toggling the row open, and `שחזר` remains in the expanded strip as the discoverable twin. On a **read-only past day** the ✓ stays a plain decorative badge — restore is locked there (ADR-0029).
