...
- **The green ✓ is also the undo control** _(revision, 2026-07-16)._ The done badge doubles as a one-tap restore: tapping it un-marks the event (identical to `שחזר`), the fast twin of the expanded verb. It stays `--ok` green (a record, never amber); on hover/focus the ✓ morphs to an undo arrow with a soft ring, and a thin persistent ring hints it is tappable on touch. It is a `role="button"` inside the card face (not a nested `<button>`) that stops propagation, so it undoes without also toggling the row open, and `שחזר` remains in the expanded strip as the discoverable twin.
- **Settling is reversible wherever it is allowed — including on a past day** _(revision, 2026-07-16)._ A past day within the live trip lets you _mark_ Done/Skip (§4, the retrospective job), so it must also let you _un-mark_: the interactive ✓ and `שחזר` stay live on a read-only past day. Restore is the inverse of a Done we already permit there — offering one without the other left "check but never uncheck," a mistake with no correction. (Structural edits — create/edit/delete/move — remain Plan-gated, ADR-0029; this covers settle status only.) Reversing settle on a **finished trip** (a Plan-mode read-only archive, ADR-0040) is out of scope here and tracked as its own decision.
