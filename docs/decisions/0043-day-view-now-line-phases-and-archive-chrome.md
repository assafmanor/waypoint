...
Settings becomes a **borderless ghost icon** rather than a third filled circle beside the identity avatars — identity is round chips (member cluster + the ringed account avatar), the gear is a UI control ("emoji are content, icons are UI," design-language). It follows mode: violet-tinted in Plan, cool blue-grey in Trip.

## Consequences

- **Frontend only, no data-model change.** `DayView.tsx` computes each event's phase from the clock (a `deriveNow`-adjacent helper) and renders the now-line, receded/settled rows, and the settle strip; the ±30 stepper and the header add-button read phase/day-scope; `App.tsx`'s day-strip switches amber from the selected pill to today's pill and adds the context ribbon; `PlanDay.tsx` renders the static now-reference when `activeDate === todayInTz(...)` within the live window. New Hebrew copy lands in `i18n/he.ts`.
