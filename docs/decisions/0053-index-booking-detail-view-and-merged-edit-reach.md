...
**1. Tapping a booking opens a read-only detail view; editing is deliberate.** The Index row opens a **detail sheet** showing every fact — title, confirmation code, provider, place / transport route, hotel room + WiFi, notes, and the linked event's timing (check-in→check-out for a span). The view carries **a visible "✏️ עריכה" button** (Assaf's call, 2026-07-17 — edit is a first-class, expected action on a booking, so it is a button in the header's free corner, not hidden only behind the menu) **plus a "⋯" menu → Edit · Delete** for the fuller set. "Edit" (either entry) opens the existing `BookingSheet`; "Delete" runs the existing delete/unlink prompt (ADR-0047 §3). The **guard** is that the view itself is read-only — you land on the facts, not in an editable form, and changing anything is a deliberate tap — which is the posture a hard commitment deserves (ADR-0011) while staying friendlier than the event card's edit-only-behind-"⋯".
