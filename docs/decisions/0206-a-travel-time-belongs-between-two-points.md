...
| **Live traffic**                  | Needs a paid vendor **and** it breaks ADR-0205 §4's "never expires", which is where most of the value is. If it returns it is a separate cache with a clock. **And it is the ONLY route to it — measured, ADR-0205 §Y6:** Valhalla accepts `date_time` and ignores it (⁦28.9⁩ min at 18:00, ⁦28.9⁩ at 04:00), because the FOSSGIS build carries no predictive speed tiles. So time of day, day of week and holidays are **unavailable** here rather than merely unsent, and sending the parameter would cost §4's cache key for nothing. Re-declined 2026-09-04 (_"No paid providers for now"_). |
