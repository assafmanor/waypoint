...
- **`ChangeFeed` (`ui/domain/ChangeFeed.tsx`).** Presentational (all data via props): a calm strip of attributed lines with relative time, a per-item dismiss and a clear-all, that **auto-collapses to nothing when empty**. Neutral chrome only — no amber/teal/plan (those stay reserved for time/location/plan); spacing/type/radius from tokens (ADR-0082). It is **not a second board** (ADR-0028 "one loud element").
