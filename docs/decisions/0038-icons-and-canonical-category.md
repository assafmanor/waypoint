...
- **`EventForm` (and the maybe-shelf add-idea flow) gain an explicit `category` selector** — the event's "type", analogous to the booking type selector, and **built in the same card idiom** (the `.typepick` icon-+-label cards, accent-tinted selection) — laid out as a **compact horizontally-scrollable row** since there are 9 categories vs. the booking picker's 6, with **no per-card colour swatch** (5 categories share the `leisure` hue; the colour lives on the pin + legend). `category` is set from _that_, never from the icon. One selector shape for "what kind of thing is this," booking or event.
