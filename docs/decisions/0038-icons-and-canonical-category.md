...
The **trip vocabulary is separate from the event set**, because a trip's glyph expresses its _character_ (destination / kind / vibe), not a timeline-item type — the event categories don't apply. So the trip picker is tab-less and shows: (a) **country flags**, generated from ISO-3166 codes (`flagFromCode`) over a curated popular-destination list (`DESTINATIONS`, not all ~250 — the long tail is unused for a small-group app and is pure data to append), searchable and **auto-suggested from the free-text destination** (`suggestFlagFromDestination`, overridable — like Tier-B for events); and (b) the archetype **vibe clusters** (`TRIP_ICON_CLUSTERS`) rendered flat with subtle spacing, no labels. Known limitation: flag emoji don't render on Windows (they show the letter pair) — acceptable since we're phone-primary (ADR-0017), where they render.
