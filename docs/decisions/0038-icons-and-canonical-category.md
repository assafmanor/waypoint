...
### Amendment (2026-07-23, via ADR-0109) — the event/idea category is an _explicit field_; the icon never decides it

The 2026-07-19 amendment left one asymmetry: a **booking's** category comes from its explicit **type**, but a **manual event's** (and a **maybe-item's**) category was still inferred from the **icon** the user picked (§4 Tier-B/manual). The Maps epic makes that untenable: **`category` now drives the map pin colour** ([ADR-0109](0109-map-tab-design.md) §3), so it must be a deliberate choice, not a side effect of picking a glyph. Resolved to make events behave like bookings:

- **`EventForm` gains an explicit `category` selector** — the event's "type", built on the **same `ui/primitives/ChoiceGrid`** the `BookingSheet` type selector already uses (not a new component; frontend `CLAUDE.md` layering / rule 8), laid out as a **compact horizontally-scrollable row** since there are 9 categories vs. the booking picker's 6, with **no per-card colour swatch** (5 categories share the `leisure` hue; the colour lives on the pin + legend). `category` is set from _that_, never from the icon. One selector shape for "what kind of thing is this," booking or event. (The maybe-shelf add flow was originally in scope here too; revised below.)
...
#### Follow-up (2026-07-23) — the maybe-shelf quick-add does _not_ get the selector
