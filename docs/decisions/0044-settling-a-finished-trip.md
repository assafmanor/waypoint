...
**Presentation — resolved: direction (a), implemented.** The Plan builder row (`PlanDay.tsx` `BuilderRow`) previously showed _no_ settle status and, when read-only, carried no trailing affordance; skipped events were filtered out. Two directions were mocked up:

- **(a) Settle control on the Plan archive row (chosen).** Keep the finished trip in Plan mode (ADR-0040) and put the settle affordance in the read-only row's free trailing slot — the one the `⋯` occupies on an editable row and vacates in the archive, so "more" and "settle" never collide. It is a `--ok` green ✓ record (identical to the Trip archive; the status mini-palette is mode-neutral, ADR-0028, so it spends no violet/amber/teal budget), tap-to-undo with the same hover/focus morph-to-arrow; an unresolved soft row shows a neutral dashed `○` that opens a "היינו / דלג" chooser sheet; a skipped row reads struck-through and receded with a `↩` restore. **Soft rows only** — hard events aren't settled (ADR-0043). Skipped events are now shown in the finished-trip archive (they were filtered out) so "what we skipped" is part of the record and restorable.

  **This row keeps the circle, and the Trip day card no longer has one** _(declared 2026-09-15, [ADR-0230](0230-one-done-mark-and-it-is-the-undo.md) §4)._ That ADR collapsed the Trip card's four affordances for one verb into the `היינו ✓` chip, and the honest reading is that it did not apply here — so the difference is written down rather than left to be discovered. Two reasons, and the first is the one that matters: after ADR-0230 this slot is the app's **only** morphing settle circle, so the duplication ADR-0139 exists to prevent is retired by a deletion, not by a second rewrite of the same control. And this slot is not that control: it carries **three** states in one square — done ✓, skipped ↩, and an unresolved `○` that opens the chooser — where the day card's chip answers one. Promoting the chips here would leave the `○` alone in the slot and is a second decision, undrawn and unmeasured. What must not drift is the **vocabulary** (ADR-0139's four axes: the words, the marks, the hues, the focus ring), and it does not: both surfaces say `היינו`, mark it with a ✓, paint it `--ok`, and mean a record.

- **(b) Reuse the past-day archive treatment.** Render a finished trip's day in the _same_ read-only DayView a live trip's past day uses. Cleaner conceptually, but re-routes the finished-trip day surface away from the Plan builder — rejected as a bigger change for no added capability.
