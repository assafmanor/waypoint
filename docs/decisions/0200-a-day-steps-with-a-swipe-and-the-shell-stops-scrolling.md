...
**Status:** Accepted — **built 2026-08-21**, **amended and extended the same day** (§6, §7 — the page turn previews the day it is turning to, and a day opens at its top), **amended 2026-08-22** (§8 — the swap is one paint; §9 — the feel: no dead zone, no lurch, no wobble, and a flick is a swipe)
**Date:** 2026-08-21
...
### 8. The swap is one paint: the offset is owed to the page that arrives (amended 2026-08-22)

Owner, on the shipped turn: _"there's like a stutter where you briefly (for a really short time, like a few ms) see the last day after swiping to the next (or previous) day."_

**The mechanism, and it is an ordering rather than a duration.** §7's commit did two things in one task: call `onStep` (which changes the day) and reset the gesture's offset. The reset is a DOM write and it lands **immediately** — dropping `data-swiping` drops the only rule that translates the page (`screens.css`), so the page returns to level in that same task. The day change is a React update and lands in a **later** one. Between the two, the surface is drawing the day you left, at level, at full opacity: not a fast animation, a wrong state, for however long the machine takes to get round to the render. `e2e/day-swipe.spec.ts` logged it across **two** mutation batches, so there was more than one opportunity to paint it.

**The obvious fix does not work here, and it is worth writing down why.** `flushSync(() => onStep(step))` should collapse the two into one task — and left the probe just as red. `BrowserRouter` (react-router 7) wraps its location update in `React.startTransition`, and `flushSync` does not flush a transition; `?day=` is the day's single source (ADR-0110 §4), so every day change is a location change. The lever that would work — `useTransitions={false}` on the router — is app-wide, and it would make every lazy route show its Suspense fallback where a transition currently holds the previous screen. Far too much blast radius for a few pixels.

**So the wait becomes the mechanism.** The offset belongs to the page it was dragged from, and a finished turn cannot give it back until the page it turned **to** is on screen. `useSwipePager` therefore takes a `pageKey` — which page is drawn — and a committed turn only marks the reset as **owed**; a `useLayoutEffect` keyed on that value pays it. A layout effect runs inside the commit that carries the new day, before the browser has painted anything, so there is no frame between the two states to shorten: the swap is **unobservable** rather than quick. `setLive(false)` from inside it retires the panes in that same paint too.

A refusal still clears at its own settle, because nothing is arriving to wait for — and a second swipe begun inside the wait takes the surface over, dropping the owed reset rather than letting it fire mid-drag.

**§6's landing moves to the same phase, for the same reason.** A scroll offset is geometry, and the day that arrives was being drawn once at the offset the day you left was reading at and only then jumping to its top — the same stutter on the other axis. It is a `useLayoutEffect` now. Honest limit, measured: with the reset deferred, the e2e probe passes with the landing as an ordinary effect too, because React flushes it before the observer's own microtask. The phase is not chosen because a test can see it; it is chosen because relying on a passive effect landing before the paint is the same incidental timing this section exists to remove.

**The contract this creates.** A host whose `onStep` does not change `pageKey` leaves the surface a page off level until the next touch re-bases it. That is documented on the option and deliberately has no timer behind it: a timer would be a second opinion about when a page has arrived, and its answer would be the flash again — just later. `useDaySurface` passes `activeDate`, which `selectDay` always changes for a day inside the trip range, and the existing e2e already fails loudly if a pane is left mounted.

### 9. The feel: the follow starts where it is claimed, the turn arrives, and a flick is a swipe (amended 2026-08-22)

Owner, using the shipped gesture: _"1. It doesn't feel smooth enough. 2. Quick swipes don't always register."_ Four things, each measured before it was changed.

**a · The gesture began with 20px of nothing and then a 24px jump.** Two thresholds were doing one job. `touchMove` decides the axis at `DECIDE_PX` (6px) and calls `preventDefault` — from that moment the browser's pan is **forfeited** — while the follow waited for `SLOP_PX` (24px) in the pointer stream. Measured in Chrome, reading the page's transform after each 6px of finger:

| finger |  +4 |  +8 | +12 | +16 | +20 |    +24 | +28 |
| ------ | --: | --: | --: | --: | --: | -----: | --: |
| page   |   0 |   0 |   0 |   0 |   0 | **24** |  28 |

So every swipe opened with a surface that had stopped scrolling and had not started moving, and then a single 24px frame. The fix is one claim seam (`claim(atDx)`) called from whichever stream decided — `touchMove` on touch, the pointer path on a mouse — plus an **origin**: the offset is measured from where the gesture was claimed, so the page leaves level at 0 and tracks the finger 1:1. What the finger spent on recognition belongs to the recognition.

This makes `SLOP_PX` mouse-only for claiming, which is where it was always right: with no pan to lose, nothing is spent by waiting until the travel means something. And it moves the commit distance onto the page's travel rather than the finger's — measured on what moved, which is also what the user was aiming.

**b · The turn was wearing the refusal's easing.** `--ease-arrive` is `cubic-bezier(0.22, 1.16, 0.36, 1)` and the `1.16` is an overshoot on purpose: on a rebuff, the strain-arrest-recoil IS the sentence. Spent on a commit, the same curve carries a ~382px page turn past level and back — a wobble at the end of every step. The turn is `--ease-standard` now; the refusal keeps `--ease-arrive`. Two settles, two meanings, and §2 already gave them separate attribute values, so nothing new was needed to tell them apart.

**c · Three day surfaces have their transform rewritten every frame, and nothing said so.** `will-change: transform` under `[data-swiping]`, which is the declaration `.wp-dragghost` and `.doc-viewer-lift` already carry for exactly this shape (fixed box, JS-driven transform). Measured over four matched arms — 30 follow moves each, same day, 241 host nodes, alternating — total main-thread work per move fell from ~2.3ms to ~1.9ms, with every bare arm worse than every promoted one. Scoped to the gesture so an idle day pays no layer.

**d · A flick was refused for not being a drag.** Distance was the only thing that committed, so a swipe thrown fast but short of `COMMIT_SHARE` did nothing — which is exactly the report. The app already knows what a flick is: `SNAP_FLICK_PX_PER_MS` (0.5px/ms, sampled from the last two moves), whose own docblock records the owner reporting the same thing about the sheet a release earlier. One threshold, one sampling rule, two surfaces. Two guards, each a real gesture: the flick must go the way the drag went — a flick **back** from a half-open page means "no", not "the other way", because position decides between the page you are on and the one you are already moving toward — and it must have cleared `SLOP_PX`, or a thumb rolling off a tap pages the day at speed.

**What is deliberately not in this round, with its numbers.**

- **The per-frame style cost.** `--swipe-dx` lives on the host and **inherits**, so every frame invalidates style for the whole subtree — the day plus both peek panes. Measured: ~1.1–1.6ms of style recalc per move for a 241-node subtree on a desktop CPU, and it scales with the day's node count (a 29-node day measured 0.43ms). `will-change` does not touch this (matched arms: 36.0ms vs 32.4ms of recalc over 30 moves — noise beside the 18% it does buy elsewhere). The lever is to stop inheriting it: register `--swipe-dx` with `@property { inherits: false }` and write it to each moved box, or compute the panes' transforms in JS. The first costs the pager a way to find the movers; the second moves geometry the stylesheet owns into the recogniser, which the gutter's own note argues against. Not attempted on a hunch.
- **The settle's duration does not depend on how far is left.** A commit always animates `--t-base` (240ms) whether 20px or 380px remain, so a nearly-complete drag decelerates from a standstill instead of carrying the finger's momentum. The fix is a duration derived from the remaining travel, published as a variable so the CSS and the JS timer read one number — but it interacts with §8's wait for the arriving render, and how that trade lands is a device question. Named as the next lever rather than guessed at.

**And what an engine cannot say here.** The flick's threshold is asserted in `lib/useSwipePager.test.tsx`, not in Playwright, and that is a measurement rather than a preference: input arrives frame-aligned, so a gesture short enough that distance cannot commit it cannot be dispatched fast enough to clear 0.5px/ms — 33ms between delivered moves at best, 83ms through two CDP round trips. The two conditions have no overlap in that environment. A real finger has no such problem (20–60px per 8–16ms frame), and the threshold is already device-validated on this owner's phone by the sheet. CDP's `timestamp` lands on the event verbatim and so can state a _slow_ gesture exactly, which is what makes the refusal assertable; it cannot manufacture a fast one.
...
- **`useDaySurface`'s peek pair has a second consumer** (ADR-0116 §2b, 2026-08-22): the drag's inline edge reads `peek.prev`/`peek.next` to know which day lies past an edge, and the mirror it uses is this feature's (`--peek-dir`) so the two cannot disagree about where tomorrow is. Changing what the peek derives now changes where a dragged card lands.
- **Two thresholds, two questions** (§9): on touch the axis is claimed at `DECIDE_PX` and that is also where the follow starts; `SLOP_PX` is the mouse's claim gate and the floor under a flick. A change to either now has to say which question it is answering.
- **The commit distance is measured on the page's travel, not the finger's** (§9). On touch that is a 6px difference; on a mouse the finger spends `SLOP_PX` first. Any spec asserting the distance has to send the finger past the claim, which `useSwipePager.test.tsx` names `CLAIM`.
- **`SNAP_FLICK_PX_PER_MS` now has two consumers** (§9) — the sheet's snap and the day's page step. It was already documented as a Phase-3 device number; moving it now moves both.
- **A committed turn is undone by a RENDER, not by the gesture that asked for it** (§8). `useSwipePager` now needs to be told which page is drawn, and its reset survives outside the listener effect. The consequence for anything reading the surface mid-swap: between the settle and the arriving page's commit the host still carries `data-swiping`, its offset, and both panes — that window is a few ms and it is deliberately not empty.
- **`flushSync` is not a way to make a day change synchronous in this app** (§8). Every day change is a router location change, and the router's updates are transitions. Anything that needs to happen in the same paint as a navigation has to wait for it, not hurry it.
...
- **Lowering `COMMIT_SHARE` so quick swipes land** (§9). It answers the report by making every deliberate drag twitchier, which is the wrong axis: the problem was that speed said nothing, not that the distance was wrong.
- **A velocity averaged over the whole gesture** rather than the last two moves. A swipe that hesitates halfway and then throws would average below the threshold and be refused for the pause, which is not what the finger said as it left. The sheet settled this for the app already (ADR-0122 §4).
- **Honouring a flick thrown against the drag** as a step to the other neighbour, the way a pure velocity model would. Refused: from a half-open page that gesture plainly means "put it back", and a pager decides between the page you are on and the one you are moving toward, not between absolute directions.
- **`flushSync` around the commit, and `useTransitions={false}` on `BrowserRouter`** — the two ways to make the day change land in the reset's own task (§8). The first does nothing (a transition is not flushed), and the second buys a few pixels with every route's Suspense behaviour.
- **A timer as a safety net behind §8's contract**, force-clearing if the page never changes. Refused: it is a second opinion about when a page has arrived, and the state it produces is the stutter it was added to prevent. A stuck offset re-bases on the next touch, and the e2e already fails on a pane left mounted.
