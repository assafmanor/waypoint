...
- **`peek` does not show one whole row** — 116px minus the sheet's own 51px top is 65px, i.e. 0.8 of a row. The stop whose job is "handle + a row or two" (ADR-0121 §5's words) shows neither.
...
| ------ | --------------: | -------------------: |
| `peek` |   358 → **420** |        0.8 → **1.2** |
| `half` |   208 → **250** |        2.6 → **3.3** |
| `full` |             n/a |        5.2 → **5.7** |
...
- **The facets open in place, on one tap.** Tapping `סינון` replaces the row's contents with the facet strip and a pinned `✕` — ADR-0100 §3's cover-the-row-in-place idiom, which the Index already runs for its search, in the other direction. A close control you have to scroll to reach is not a close control, so the `✕` sits at the row's fixed end, where the search button was.
...
**`קרוב עכשיו` moves to the sheet's own top row, beside the `רשימה · מפה` toggle.** It is neither a filter nor a scope: it re-orders the list and adds distance chips — session 138 split `located` (a fact) from `sortByDistance` (an intent) precisely to say so. So the rule is **scope belongs to the tab, filters belong to the split, sort belongs to the list**, and the sheet's top row is fixed, visible at every stop, and costs no height (the row already holds the toggle at 28px). Offline the chip is still absent, unchanged (ADR-0109 §7).

### 3. The stops: derivations first, numbers second

`MAP_SHEET_STOPS` keeps its shape (`peek` in px because a peek should be the same size on every screen; `half` a fraction because it should not) and gains one variant:

- **`peek` = the sheet's own top + one WHOLE row + a sliver of the next.** On the measured baseline that is **148px** (51 + 82 + 15), against today's 116px. The sliver is not decoration: it is the same "there is more this way" affordance ADR-0100 §6 chose a fade for. The map still comes out ahead at that stop (358 → 420) because the controls row paid for the extra row and more. **What that one row is _for_ is §7** — without it a pin tap would have nowhere to speak.
- **`half` keeps `0.56`.** The measurement is the argument: once the controls leave the layout, 0.56 lands on **3.3 whole rows and a 250px map** on the baseline phone. Whether it should lean list (0.56) or map (0.5) is a judgement about a _rendered_ map, which is the device pass's, not a desktop viewport's.
...
### 5. Two states, three stops, one axis — drawn, not just claimed
...
### 7. A pin tap does not move the sheet, and that is what the peek row is for

Two questions the owner asked of the design above, in one breath: **when the map is maxed, what does the sliver of list add?** And: **tapping a pin should surface the place, but not necessarily by raising the list — it must not interrupt the interactive map.**

They answer each other. **The peek row is where a tapped pin speaks**, and that is its whole job:

- **A pin tap selects and surfaces, and the sheet does not move.** The tapped place's row is scrolled to the **top** of the peek viewport — which is one row tall, so "scrolled to the top" and "shown" are the same thing. The map keeps every pixel it had; nothing resizes, nothing re-fits, and the pane's box is untouched, so the camera stays where the user put it.
- **This revises the raise session 136 shipped** for report #4 ("the list doesn't focus what's marked, especially when minimized"). That fix was right that both directions must show what you selected, and it used the only mechanism available at the time: at `peek: 116` the viewport was 65px, so a row genuinely could not be shown there and raising was the only way to show it. §3's peek removes that constraint, and the raise then becomes exactly the interruption the owner is describing. The half of session 136's fix that stays is the scroll — it moves from "centre the row" to "put it at the top", because the top is the only line a one-row viewport has.
- **One rule now covers both directions: the sheet moves only when what you just asked about is not on screen.**
  - A **pin** tap: the pin is on screen. Nothing moves.
  - A **row** tap **normalises the sheet to `half`** — from `full` because the map it focuses is invisible there (ADR-0121 §8's existing rule, unchanged in substance), and from `peek` because the row's own way-in block does not fit a one-row viewport. So the first tap on a pin _identifies_, and one tap on the row it just surfaced _opens the door_.
  - This is a better formulation than "the raise mirrors the drop", which is what session 136 reached for: symmetry of _mechanism_ was never the goal, keeping the surface you are using was.
- **The way-in stays revealed by selection, unchanged (ADR-0121 §8), and at `peek` it is deliberately taller than the viewport.** Measured: a selected row with one entry is 141px against a 97px viewport, so the row states the place and the top edge of its entries reads as "there is more here". The entries are then either a short scroll inside the sheet — which never resizes the map — or the one row tap above. **Sizing `peek` to fit them was considered and rejected on the numbers:** row + one entry needs ~198px, which costs the map 50px at the stop whose entire point is the map. Identification is what a maxed map needs; acting on a reference is a leaving action, and it can afford one tap.

**This closes ADR-0121 §8's open question, and closes it away from the thing that question worried about.** §8 left open "whether a **pin** tap should reveal the same entries on the canvas (an info window) or only via its row… judge it on a real map." Judged: no info window. The peek row _is_ the info window, so the map idiom's benefit is had without a second way of stating a place, without a surface that covers the pins it describes, and without a new dismissal to design. That the owner arrived at the same place from "don't interrupt the map" is the useful part of the answer.
...
- **`half`'s fraction.** "Big enough to read a rendered map" is a legibility judgement, and the trade between 250px of map and 3.3 rows of list is a preference someone has to _feel_. It joins `MAP_ZOOM` and `MAP_REFIT_FILL_SHARE` in Phase 3's tuning cluster.
- **`peek`'s 148px**, whose derivation is firm but whose sliver is a taste call.
...
- **Raise the sheet on a pin tap** (what session 136 shipped, and the obvious symmetry). Rejected in §7: it takes away the surface the tap was made on. It was the right fix for a 116px peek, where no row could be shown without moving; it stops being right once one can.
- **Reveal the tapped place in an info window on the canvas** — the map idiom, and ADR-0121 §8's own open question. Rejected in §7: the peek row already is that window, and a canvas card is a second way of stating a place, covers the pins it describes, and needs a dismissal of its own.
- **Size `peek` to fit a selected row _with_ its way-in entries** (~198px). Rejected on the numbers: it costs the map 50px at the stop whose entire point is the map, to save one tap on a leaving action.
- **Drop the peek row entirely — a handle-only peek**, which is the literal reading of "when the map is maxed the sliver adds nothing". Rejected: the row is what makes the no-raise answer possible (a pin tap would otherwise need an info window or a raise), and ADR-0121 §5's reason for "the sheet always peeks" is that the list is the only view that works offline and the only one that can hold a coordless place.
