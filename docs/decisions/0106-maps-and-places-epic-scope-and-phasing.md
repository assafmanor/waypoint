...
- **Deferred to the embedded-map phase: by area** — the one filter with no data model behind it; pan/zoom on a rendered map _is_ the area filter, so we don't build a shaky derived-locality bucket early. _(Refined by [ADR-0121](0121-embedded-map-phase-6-design.md) amendment E2: the reasoning stands and **no area chip is ever built**, but the decision was invisible — nothing on screen said the map was answering "what's around here". Phase 6 adds a quiet **`12 באזור`** count read off the camera on its `idle` event, top-inline-end and clear of Google's attribution. It counts every pin on the canvas including out-of-scope "ghosts", because it is a **spatial** readout rather than a facet count. And the **list deliberately does not follow the camera**: a list reshuffling under your thumb as you pan is the same defect as a camera re-centring under your fingers, which that ADR §6 already refuses. Pan/zoom filters what you **see**.)_
...
- **AI / web enrichment of places** (hours, photos, descriptions) — a vNext pipe ([0004](0004-integrations-are-pipes.md)); we keep `source`/keys separable so it stays unblocked, but it is not built here. _(Exception added by [ADR-0109](0109-map-tab-design.md) §9: **`rating`/`userRatingsTotal`** are pulled into Phase-1 pick-time enrichment — they ride the same Place Details response as coords, cache on the row, and are high-value for research; hours/photos/descriptions stay deferred.)_
...
- **Phase 1 — The Places picker (keystone).** `PlacePicker` over Places Autocomplete with session tokens; on select, create-or-link a `Place` enriched with `googlePlaceId` + coords + address (+ `rating`/`userRatingsTotal`, [ADR-0109](0109-map-tab-design.md) §9), deduped by `googlePlaceId`; wired into all place fields (EventForm authoring restored, with an explicit **category** selector — ADR-0109 §11 / ADR-0038 amendment). Name-only `Place` authoring stays as the offline fallback. **Also resolves + caches `Place.timezone` from the coordinates** (free offline lat/lng→zone lookup) — the foundation the multi-zone time model rides on ([ADR-0107](0107-per-place-timezones-and-multi-zone-time.md)).
...
- **Phase 5 — Plan-mode research.** Search Google Places from within the Map tab (reusing the picker's search core) → pin results → "+ maybe" onto the shelf; closes the vision's pillar-4 "discovery by location and free time." _(Designed + built in [ADR-0115](0115-plan-mode-place-research.md): the tab's one search control gains a second half whose paid side is **armed by intent** — the first surface that spends money per keystroke — and "pin the result" resolves to the `＋ אולי` write itself, since a shelf reference is what makes a place in-trip ([ADR-0112](0112-place-in-trip-is-referenced-not-cached.md)).)_
- **Phase 6 — Embedded map (fast-follow).** A rendered Google map with pins, built on the Maps **JavaScript API** and fully brand-styled; the list becomes its companion (the shape — a map pane + a draggable list sheet + a `רשימה/מפה` toggle — is designed in [ADR-0109](0109-map-tab-design.md) §10); the "by area" filter arrives as pan/zoom; day-connectors/routes layer on. Detail in the dated section below. _(Designed in [ADR-0121](0121-embedded-map-phase-6-design.md) + `mockups/map-embedded-v1.html`, which performed the API/pricing reconfirmation this ADR and ADR-0109 both required first: the §A–F direction held, `@vis.gl/react-google-maps` is the binding, the sheet/toggle is view state rather than a back layer, and paid Routes stays sequenced after. Two details moved — see the annotations on §D and §E below.)_
...
**C. Design principle — quiet base, loud pins.** The base map is a desaturated **neutral canvas**; semantic colour lives only on the **pins and routes**, never flooded across the base. Map = quiet ground, pins = loud figure — this keeps [0028](0028-plan-violet-color-budget-dark-ready.md)'s colour budget intact. _(Revised by [ADR-0109](0109-map-tab-design.md) §3: the loud pin colour is the **category** — the 5-hue palette ADR-0038/0028 already sanction — not teal. This ADR's original "teal = location on the pins" over-applied teal; teal/amber stay for location \_affordances_ / time [route ETA + the next-stop ring] respectively, off the pin fill.)\_
...
- **Live routes (paid):** the **Routes API** returns real walking/driving/transit polylines **with distance + ETA** between stops (per-request, ~25-waypoint cap), rendered on our styled map. _(Corrected by [ADR-0121](0121-embedded-map-phase-6-design.md) §1: the cheap **Compute Routes Essentials** SKU caps at **10 intermediate waypoints**, not ~25 — so a day of more than 12 stops must chunk or degrade rather than silently escalate tier. Affects only this paid enhancement, which is still sequenced after the free connectors.)_
...
_(Two [ADR-0121](0121-embedded-map-phase-6-design.md) findings land on §B: a **`mapId` is now mandatory** for advanced markers rather than merely enabling them — so creating a Map ID + cloud style is a human Phase-0 step, and the id needs its own build var (`VITE_GOOGLE_MAPS_MAP_ID`, R3; the browser key's name was also contradictory across docs and is settled there). And §B's commitment to keeping Google's logo/attribution is **load-bearing on layout**: the API renders them at the bottom-left of the map div, so the map div is sized to the visible area rather than sitting behind the list sheet, R2.)_

**E. The trip "macro" is per-day, not one route.** All pins on one fit-to-bounds map, with connectors/routes **per day** (colour per day, a day toggle). _(Refined by [ADR-0121](0121-embedded-map-phase-6-design.md) §7: connectors render in **day scope only**, which makes the tab's existing scope chip the "day toggle" and removes the need for a per-day colour palette entirely — one day, one path. The line is dashed and neutral, because a straight segment is the day's **order**, not its route. This ADR's substantive point — per-day, never one whole-trip route — is what that enforces.)_ A single whole-trip route is semantically weak (you base at a hotel and radiate out, you don't travel linearly) and blows the ~25-waypoint cap; the schedule already partitions + orders places by day, so per-day is both cheaper and more meaningful.
...
> **Design questions below are now resolved in [ADR-0109](0109-map-tab-design.md)** (the design session): geolocation permission UX (§6–7), multi-facet union + colour-by-most-committed (§4, ratified), and the Index-filter-grammar reuse (§2). The remaining open items are the FE-arch / BE-arch ones.
...
- ~~**Geolocation permission UX**~~ — **resolved in [ADR-0109](0109-map-tab-design.md) §6–7:** just-in-time (asked only on tapping "near me now", behind a reason-first pre-prompt, never on tab open); denied/unavailable degrades to the default sort with distance chips dropped and a quiet re-enable affordance — reads never depend on location.
- ~~**Multi-facet place semantics**~~ — **ratified in [ADR-0109](0109-map-tab-design.md) §4:** union semantics + colour-by-most-committed, the latter expressed via the Waypoint pin's amber-core state (`hard > soft > idea`).
- ~~**Reuse the Index filter grammar**~~ — **confirmed in [ADR-0109](0109-map-tab-design.md) §2:** the Map filter row is the `index-bookings-compact-v2` chip/search/mode-accent grammar extended, not a second copy; the FE-arch session still confirms the concrete `ChoiceGrid`/`lib/index-bookings.ts` extraction.
