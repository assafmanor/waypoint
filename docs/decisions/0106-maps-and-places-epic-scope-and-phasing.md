...
- **AI / web enrichment of places** (hours, photos, descriptions) — a vNext pipe ([0004](0004-integrations-are-pipes.md)); we keep `source`/keys separable so it stays unblocked, but it is not built here. _(Exception added by [ADR-0108](0108-map-tab-design.md) §9: **`rating`/`userRatingsTotal`** are pulled into Phase-1 pick-time enrichment — they ride the same Place Details response as coords, cache on the row, and are high-value for research; hours/photos/descriptions stay deferred.)_
