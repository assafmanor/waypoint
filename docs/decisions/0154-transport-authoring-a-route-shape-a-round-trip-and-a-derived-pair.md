...
**Connections and multi-city.** Adjacent but a different shape: a layover is a **sequence**, a round trip is a **mirror**. `legs` is the axis that would carry it; deliberately not populated now. **Closed 2026-08-02 by [0159](0159-the-day-says-what-is-between-two-events.md)**, and the axis is what carried it: `legs` widened to `{ mirrored, sequence }` — one field holding the capability and its window — with the mirror/sequence distinction above becoming the literal rule that separates the two derivations (a sequence must NOT return to its origin). Two things that ADR changes here rather than extends: the return leg moved to a step of its own (§4 put it with the shared fields, which cannot hold two return legs), and `roundTripPartner` now skips a connecting candidate first — one PNR is exactly what a through-ticket has, so without it the second half of an outbound journey was being called the return.
