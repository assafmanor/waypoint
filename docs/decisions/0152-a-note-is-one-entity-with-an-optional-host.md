...
**Amended 2026-08-16 (owner's report on the notes screen, which turned out to touch this surface too): a titled note shows its title AND its body here.** This section renders `noteTitleText(note)`, which is `title || body` — so a note carrying both showed only its title, and its body appeared on **no read surface at all**, because the notes screen was meanwhile pushing the same body into a `ListRow` meta line that collapses newlines (ADR-0153 §4's amendment of the same date). The two defects are one: the title+body pair never got a single answer, and each surface dropped a different half of it.

The fix here is one span and one weight — the title as its own block above the body — because everything else this needs is already true of `.note-item-b`: **a line on this surface does not clamp** (it never has, and §4 of the other ADR corrected itself on that point once already), and it carries `white-space: pre-wrap`, so the line breaks an author typed to make a long note readable land whole. That is what makes this surface, not the screen's row, the place a structured note is actually _read_ — the row previews two lines, this reads all of them.

`noteTitleText` keeps the untitled cases, including the url-only fallback it is the sole holder of. Counted rather than assumed before changing it: it has four call sites, and the other three — `NoteManageSheet`'s sheet title, `IndexNotesView`'s `openLabel`, `Index`'s tile preview line — are single-line by design and are correct taking the title alone.
...
**Amended 2026-08-16 — the box is REVEALED by the section's `＋ פתק`, not always on screen** ([ADR-0192](0192-a-form-has-bands-and-its-content-sections-have-one-look.md) §2's own amendment, on the owner's report against the built form). This section's rule survives whole and is the reason the alternative was refused: `＋ פתק` opens the **inline box**, never a second form. What changed is only that the box waits to be asked for, so the notes section reads like the tasks section beside it — a header, one way in, and `אין פתקים על זה` — instead of a permanently open textarea that made an untouched form look unfinished.

Two consequences for the paragraph below: on a **host form** the box no longer carries a `＋` of its own (the header's control commits and opens the next, since one control cannot sit in two places six pixels apart), and the surfaces with **no notes section** — `DocumentUploadSheet`, `MapPlaceForm` — keep both the always-open box and its `＋`, which is what `useNoteComposer`'s `standalone` names.

**More than one note per save, via one input that never moves.** The slot holds a single `textarea`; a committed note collapses to a **one-line chip** above it, the same compact shape the detail surfaces already use. `＋` commits and clears — but **the common case costs nothing**: whatever is still in the input when the host is saved becomes a note too, so one note is type-and-save with no extra press. `＋` exists only to start a second one.

**Amended 2026-08-07 (session 219) — the owner REVERSES the key rule: Enter inserts a newline, and only `＋` commits.** This paragraph shipped reading "`＋` (or Enter) commits and clears", with Shift+Enter as the way to break a line, and the owner's field report is that it is backwards: _"pressing enter in the note field creates a new note instead of a new line"_. The reversal is recorded here rather than left to drift, because the sentence above is what the code cited. A note is prose and a `textarea`'s return key already means one thing to everyone typing into one; making it also mean "finish this note" made a two-line note untypable unless you knew about Shift, to buy a one-hand path to a **second** note — which is the rare case this whole section is built around not optimising for. So the composer now binds **no key at all**: Enter is the textarea's own newline, and creating a separate note is the explicit press. **The invariant beside it is untouched** — the newline stays inside ONE note, and nothing splits on a blank line; that alternative is still rejected below, for the reason it always was.

_Same day, the owner's follow-up: **a newline is only real if it renders.**_ The first build shipped the key change alone and the notes came back one line, because the default `white-space` collapses a newline to a space — the note was written, stored and read back correctly and looked exactly as it had before. So **every surface that prints a body declares `white-space: pre-wrap`** (`.note-item-b` on the host sections, `.note-body-line` on the notes screen; the clamp counts rendered lines and is unaffected). The composer's own chip is the deliberate exception and stays on its one line — a committed note is collapsed there by design. This is pinned as a CSS contract test rather than a human pass, because jsdom has no CSS engine: a unit test can see the newline in the DOM and can never see it collapse.

**A committed note stays editable**: tapping its chip returns the text to the composer with the caret at the end, and `＋` puts it back — anything half-typed is committed first so nothing is clobbered. Without it a typo costs a delete and a retype, which is the opposite of sparing anyone. The chip's fill is the neutral ink wash `list-row.css` already uses at this layer, **not `--paper`** — that token is a warm cream (#f3efe6) that is right behind a 40px badge glyph and reads as a beige wash at full width.
