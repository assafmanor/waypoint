...
> **The layout layer gains one modifier in Phase 6** ([ADR-0121](0121-embedded-map-phase-6-design.md) R1): `AppShell`'s `<main className="body">` scrolls and carries `padding: 16px 16px 92px`, which suits every screen except a full-bleed map. It gains a body modifier (`fullBleed`/`bodyClassName` → `.body.is-fullbleed { overflow: hidden; padding: 0 }`) so the Map tab can own a fixed-height split with its own internal scroll. One contained addition in the layer that owns shell structure, reusable by any future full-bleed surface — rather than a per-screen override fighting the shell.
