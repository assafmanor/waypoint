...
>
> > **Follow-up (2026-08-03, session 188, owner's report the next morning): the two halves of symptom 3 collided.** `dirty` had been reading `booked.touched` — a fair reading of "the human turned the row on" while the flag could only mean that — and this amendment made it mean something else: _the category may no longer move this_, true from the first render of every existing event. So the fix for symptom 3 turned into its inverse. **Every** edit of **every** event opened dirty, and leaving one without typing anything cost a discard confirm on a form nobody had touched (owner: _"even when no change has been done… especially in plan mode where every tap on an event opens the edit form"_ — Plan's builder row opens this form directly, so the nag landed on the mode that taps it most). The guard now diffs the **value** against the same `initialBooked` the field was seeded from, which `BookingSheet` had already written down for its own two derived fields (_"`iconTouched`/`kindTouched` are not state the user typed"_). **The general rule, and it is the reason this sits here rather than in a note: `useDerivedField`'s `touched` answers "may the derivation still move this?", never "did a human change anything?"** — those coincide only while nothing seeds the flag, and this ADR is what stopped that being true. A dirty check reads values.
>
...
- **Two `*Touched` guards now sit side by side** (`kindTouched`, `bookedTouched`), both meaning "a human said this, stop deriving it". If a third appears, that is the moment to generalise them rather than the moment to add it. _(Session 185: `EventForm` gained **both** — it had `iconTouched` and no `kindTouched`, because until this row nothing in it derived the kind. So the form now carries three, and the generalisation moment named above has arrived.)_

  > **Done 2026-07-31 (session 186):** generalised into `lib/useDerivedField`, and counting them properly found **seven**, not three — the two authoring forms' five plus `CreateTrip`'s `nameTouched`/`iconTouched`, which auto-suggest from the destination and are the same mechanism wearing a different name. Two things the hook had to keep rather than flatten: an **initial value can already count as chosen** (an event's existing glyph, and an existing event's `kind` — this section's own §4 rule, which a hook tracking only "did the user click" would have silently broken), and a **reset**, which only `BookingSheet`'s ✨ revert uses and which therefore belongs in the shared mechanism rather than in the one call site that has it. `redrive` **returns the value now in force**, which is what lets the category pill move three fields in one handler without reading state React has not flushed. Dirtiness stayed at the call sites, because the two forms disagree about it on purpose. Seven value-plus-flag pairs became seven hook calls (13 fewer `useState` calls), no test changed, and the trap-check confirmed the guarantees survived: removing the guard fails this ADR's own "a soft event must not harden" reproduction.
