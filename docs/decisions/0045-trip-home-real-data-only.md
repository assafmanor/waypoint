...
- **Window = 07:00 (or the earliest event) → 23:00 (or the latest event).** An overnight day (ADR-0037) runs **only to its latest event** (e.g. 02:00), never padded to 07:00 next day; the crossing segment carries the amber `+1`. **Amended 2026-09-11: only what the rail DRAWS moves the window.** A bracketed booking's transition instant used to widen it too (so a marker could not fall off the scale, [ADR-0077](0077-unified-glance-rail-annotation-grammar.md) §E) — and an ambient span draws no block, so a car hire with no authored pick-up hour (`docs/backlog.md`) dragged its day's window back to 00:00: seven hours of rail with nothing drawable in them, and the day's real blocks squeezed into the far end (owner report, _"the day is skewed to the left"_). Such an instant is now **clamped** onto the rail instead, which puts it on exactly the edge the stretch put it on and leaves every block where the day says it is.
...
The through-line: the Home was showing capability we don't have. For a _living visibility layer_ whose whole promise is "what's true right now, even offline," shipping fixtures is the wrong failure mode — it erodes trust in the one screen that must be trustworthy.
...
2. **Quick access → three _real_ shortcuts (v1).** The grid stops being a concierge and becomes shortcuts into data/surfaces we actually have:
   ...
   **Navigate-to-next is deferred to v1.next (decided 2026-07-16).** A fourth tile, **ניווט ליעד הבא**, was in the approved set but is held back until the maps/location work lands. The subtlety worth recording: the tile does **not** depend on live _user_ location — a Google Maps deep-link (a catalog _Must_) hands off to Maps, which does the routing from the device's own position, so [ADR-0006](0006-no-live-location-v1.md) does not block it. What it depends on is **real place data on the event** (address / Places id / coords). Today an event carries only a free-text `title`/`location`, so the only thing buildable now is a Maps _search-query_ deep-link — which can resolve to the wrong city or an ambiguous match. That is exactly the "might-be-wrong fixture" this ADR exists to remove, and it would be the one tile on the screen that could confidently send someone to the wrong place. So navigate waits for the place-data work; when it lands, the tile slots back in as the fourth and the grid returns to four columns. The design intent (four tiles) is unchanged — only its phasing.

   **Amendment (2026-07-25, session 104): the tile has landed, exactly on the condition stated above.** The place-data work it was waiting for shipped in the Maps & Places epic's Phase 1 (picked places carrying coordinates, transport endpoints included), so [ADR-0106](0106-maps-and-places-epic-scope-and-phasing.md) §6's navigate-to-next is now built and the grid is back to four columns. It is a real coordinate deep-link, never the search-query guess this ADR refused: the derivation (`nextDestination`, `lib/places.ts`) requires a **mappable** place and skips anything coordless, so the tile is **absent** rather than wrong when nothing upcoming has a location — the same "derived, not managed" rule the code and WiFi tiles follow. Two consequences of that filter worth recording here: it resolves transport to its **origin** (the airport you fly from is where you have to get to), and it may point at a later event than the board's immediate next, since a placeless soft event is nothing to navigate to. Detail in [`planning/2026-07-25-session-104-navigate-to-next.md`](../planning/2026-07-25-session-104-navigate-to-next.md).
