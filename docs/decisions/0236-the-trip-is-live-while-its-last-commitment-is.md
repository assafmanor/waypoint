...
### 7. The map is the fourth day surface, and its pins ask §4's question too

**This section is a second correction, and it is the same correction as §4.** E26 said the Map's day scope is untouched, reasoning that it reads `liveToday` for _"where are you standing"_, which is a different question from _"which day of the trip is it"_. That is true of the Map's **clock** and says nothing about its **pins** — and the pins ask a different question again: which day of the trip does this place's reference land on. Reported from the device, on the trip's last day, against the same flight:

> _"there's still an issue with the map regarding this issue, see that it shows only one leg of the flight on the map"_

A transport booking contributes **both** its endpoints (`buildPlaceUsageIndex`, ADR-0110 §2), and `routeEndpointDay` dates the destination to `endDate` — the day after the trip ends. No chip on the strip offers that day, so on the last day the destination fell out of the scope entirely and rendered as a **`ghost`**: dimmed, numberless, filed under "another day", for a day that does not exist. The tab drew the departure airport alone, which is what a flight with one end looks like.

**Decision: `routeEndpointDay` takes the trip's range and resolves its end through `endHostDay`** — §4's rule, read rather than restated. Three derivations already share that function so they cannot disagree about which day an end happens on (`spanDays` for the pin, `placeRefs` for the row, `connectionStops` for a layover's band); giving it the hosting clause moves all three at once, and moves them to the same answer the day list's arrival row already draws. Without a range nothing is hosted, so every caller that is not day-scoped is unchanged.

Two consequences fall out and both were defects on their own:

- **The row says which day it lands on.** `PlaceRef` carries `dayOffset`, and the meta line reads `נחיתה · 02:30 · למחרת` where day-scoped silence would have read as "on this day". `relativeDayWord` moved out of `TransitionRow` into `lib/transitions.ts` for this — the map row and the day row now say the same thing about the same landing, in the same words, which is the property §4's day word was chosen for.
- **The landing becomes settleable.** The row's `unreached` guard is `refDate > today` (ADR-0139 §2), and a reference dated to a day past the trip's end was permanently unreached — so the arrival carried no settle pair on any day. Hosted, it is `today`, and the control is there.

Rejected: filtering ghosts by whether their day exists (it hides the symptom and leaves the pin on no day at all); and hosting inside the Map screen (a fourth copy of the rule, which is the thing three derivations sharing `routeEndpointDay` exists to prevent).

**No new mockup frame, deliberately.** §4's frame already draws this landing and this word; §7 adds a consumer, not a decision — nothing here is a shape, a colour or a number that a drawing could falsify. What it does claim — that the map row and the day row say the same thing about the same landing — is pinned by a spec that asserts the day list's own `t.journey.nextDay` inside `.map-ref-meta`, which is a stronger check than a picture.
...
| E26 | **The Map's day scope**                                                                         | **Half right, and corrected by §7.** Its CLOCK is untouched — it reads `liveToday` for "where are you standing", which is a different question from "which day of the trip is it". Its PINS are not: they ask which day a place's reference lands on, and the flight home's destination was dated to a day the strip has no chip for. Same shape as §4's correction, found the same way — by the owner looking at the surface. |
| E28 | **A layover on the way home**                                                                   | `connectionStops` reads the same `routeEndpointDay`, so it takes the range too: a change of planes lands on the day surface its arriving leg does, or on none. Threaded rather than left, because a third derivation drifting from the other two is what naming the rule once was for.                                                                                                                                         |
| E29 | **All-days scope**                                                                              | Unchanged: with no day to be out of, a hosted end and its own end are the same reference. The range is passed either way, so the two scopes cannot disagree about which day the row names.                                                                                                                                                                                                                                     |
...
**The window and the day clamp are one function, and that is the whole reason it is small.** `tripToday` gains the clamp; `tripPhase` compares against `tripToday` and so needs no new arm. Five surfaces then move off `liveToday` for their trip-day question — `App` (the day strip and `offToday`), `DayView`, `PlanDay`, `trip-state`'s `defaultDay` — and the Map deliberately does **not**: it asks where you are standing, which is a different question (E26 — true of its clock, and §7 is the half of E26 that was wrong).
...
## Build log — 2026-09-20 (third pass: §7, the map)

**The same correction, on the fourth surface, found the same way.** §4 corrected a first draft that said the day surfaces were fine; E26 then said the map was fine, on a reading of the Map's CLOCK that is true and answers a different question from the one its PINS ask. One `grep` over `routeEndpointDay`'s call sites is what the claim needed and did not get — root `CLAUDE.md`'s own line, landing a second time on the ADR that quotes it. The three consumers were there in plain sight, and two of them are day-scoped.

**Threading a range through one function moved three derivations at once**, which is what the shared rule was for: `spanDays` (the pin), `placeRefs` (the row) and `connectionStops` (a layover's band). Nothing in the Map screen learned the rule — it passes `trip`.

**Two defects were riding underneath the missing pin, and neither was in the report.** The row said nothing about which day the landing falls on, because day-scoped silence means "today" — and the settle pair was structurally unreachable, since `unreached` is `refDate > today` and the reference was dated past the trip's own end. Both fall out of the hosting; neither would have been found by fixing the pin alone.

**Root rule 8, the other way round.** The new call site needed a whole-day date diff — and the repo had that one line written **five times in four spellings** (`dayDiff` twice, `dayCount`, `calendarDaysBetween`, and inline in `relativeDayLabel`). It is `calendarDaysBetween` in `lib/time.ts` now and the five read it, because the alternative was a sixth copy. Same pass, same reason: `relativeDayWord` left `TransitionRow` for `lib/transitions.ts`, so the map row and the day row say `למחרת` from one definition.

**Tests: 11 new specs, and the screen pair is what pins the report.** `place-usage.test.ts` and `place-refs.test.ts` carry the derivation (hosted, not hosted without a range, a within-trip red-eye untouched, and nothing at all when the leg is shrunk entirely out); `Map.test.tsx` renders the real tab on the trip's LAST day and asserts both ends of the flight are in the day's sequence and that the arrival names its own day. Verified by reverting the two wiring lines in `Map.tsx` — both screen specs go red, which is what says they pin the reported behaviour rather than the derivation twice. The Map harness gained a settable day scope for the same reason the agreement harness did in the second pass: a spec about the last day has to be able to stand on one. Suite: **5875 green**, ⁦11⁩ up from the ⁦5864⁩ on `main` (the baseline moved with #862, which landed between the passes).

**Also here:** this file carried the §6 build log **twice**, nearly verbatim — an artifact of two pull requests landing the same section. The duplicate is gone.
