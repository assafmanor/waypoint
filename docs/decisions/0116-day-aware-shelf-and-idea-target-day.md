...
- **A long press on a button starts a text selection / iOS callout**, so a draggable card sets `user-select: none` + `-webkit-touch-callout: none`, and the context menu is prevented while the hold is live.
