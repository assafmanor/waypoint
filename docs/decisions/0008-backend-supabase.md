...
- **Framework:** **NestJS** (structured, modular, DI — a "real backend framework" feel). _Fastify is the lighter alternative if NestJS feels heavy — confirmable._
- **DB:** **Postgres**.
- **ORM / migrations:** **Prisma** (type-safe, great migrations). _Drizzle is the SQL-first alternative if more raw-SQL control is wanted — confirmable._
