...
> **Decided 2026-08-04 by [ADR-0166](0166-place-enrichment-is-a-multi-source-pipe.md) — and the TTL constraint named above is what decided it.** Read against Google's live policies, that constraint is stronger than a TTL: Places content may not be cached at all (`place_id` exempt and indefinite, coordinates ~30 days, everything else request-live-and-attribute), so a global tier of **Google** details is not the thing to build. What ADR-0166 builds instead is a global `PlaceEnrichment` table holding only **open-licensed** facts (Wikidata/Wikipedia/Commons/OSM), with **no Google-sourced value ever written to it**.
>
> Two things this ADR settled are what make that possible, so the lineage is worth stating: the **"cache vs trip entity" split is applied once more** — `Place` stays trip-scoped and unmigrated (its `icon`/`category` are the trip's _opinion_, which is also why the backlog's objection to a global cache does not block a **sibling** table), and the global row holds the world's _facts_. The rule ADR-0166 states for the boundary is the generalization of this one: **if two trips could legitimately disagree about it, it is not enrichment.**
