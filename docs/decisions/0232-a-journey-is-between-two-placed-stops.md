...
**Status:** **Accepted and built 2026-09-16**, in one session with the mapping and the mockup, on the owner's _"Build it with the recommendations"_ — every fork in §6 stands as recommended. Build log in §8.
**Date:** 2026-09-16
...
**Mockup:** [`mockups/a-journey-is-between-two-placed-stops-v1.html`](../../mockups/a-journey-is-between-two-placed-stops-v1.html) (owner: _"Mockup"_, 2026-09-16) — §1 the owner's day with F1/F2/F5 as controls, §2 the four answers to F3 side by side, §3 the seam drawn wrong on purpose, §4 Plan posture. Its render found one thing this text had not: an origin word and a sentence cannot share the block's meta line at 360 (the line clips silently), so the two **stack** — see §M below.
...
**Fit is measured against the combined slack**, not the hole the block is drawn in: `(destination start − origin end) − Σ(the part of each placeless row that falls inside that window)`. _Amended 2026-09-16 off the first deploy (§8): a crossed row that happens after the deadline eats nothing — the aurora watch at ⁦22:45⁩ cannot shorten a check-in window that closes at ⁦20:00⁩._ A ⁦15⁩-minute hole under the block holding a ⁦51⁩-minute drive is _not_ an overrun when the ⁦3:30⁩ above the aurora would have held it; a plan that does not fit even when the placeless rows are counted is the only one that may say so, and it says it with the existing `TOO_TIGHT` / `OVERRUNS` arms.
...
- **§M's mockup gate — drawn** (`mockups/a-journey-is-between-two-placed-stops-v1.html`). The arithmetic and the position spend no new axis. `מ־X` is a `--muted` span in the block's existing meta line (`.day-trv-from`, one rule): the slot's own `.day-trv-leave` is the clock's amber and a place name may not wear it (root rule 4). **What the render added:** `.day-trv-meta` is a `nowrap` flex line whose `overflow: hidden` clips a child span with no ellipsis, so `מ־Fancy Sheep · חסרות 21 דק׳ לדרך` lost its digits at 360. An origin and a sentence therefore never share the line — `.day-trv-meta:has(.day-trv-from)` stacks them (second rule), costing one line only on the arms where a sentence exists beside the origin (an overrun on the combined slack, a declared or refused mode, a warming leg). Under F3a the ordinary spanning block has no clock and stays two lines: measured ⁦40px⁩ against ⁦38px⁩ bare, the whole cost of F2a. The day list grows ⁦371⁩ → ⁦421px⁩ for the one leg it was missing.
...
## 8. Build log (2026-09-16)

What the build changed against the text above, recorded here rather than left for a reader to reconstruct from the diff.

- **The chain is a required part of `JoinContext`** (`JourneyChainContext`: `placedAt`, `movesYou`, and the bed as `from`), not an optional one. A surface that forgets to say which rows are placed is indistinguishable from a day where every row is, and that is the exact shape this ADR replaced — so the compiler asks. `journeyChainFor(bookings, places)` (`lib/day-travel.ts`) is the one place the answer is built, from the same `endpointPlaceId` + `coordOf` the reads resolve a leg with.
- **`dayBlocks` stays; `dayRun` is what the surfaces call.** It returns the blocks and the chain's **tail** — the last placed row and the placeless rows after it — because the leg back into tonight's bed leaves from there, and re-deriving it at a screen would mean reproducing the transparency rules the loop already holds. `DayBlockEntry` carries `legFrom` and `spans` beside `from`; `from` is still the join's row.
- **§4.1 was right about the readers and inverted about the walker.** `journeyFor` is now asked by the row a leg leads into (`legInto`, both surfaces), which is what makes a spanning leg findable — but ADR-0231's `livePositions` walker asks with the **adjacent** pair on purpose, and finding nothing there is R5 by construction: the shelf's default slot and the `+` land on the raw hole, never on one narrowed by a journey that may not be in it. `narrowGapForTravel` refuses a spanning journey too (`DayJourney.spansPlaceless`), so a caller that does find one cannot narrow with it either.
- **R4 is one input**: `dayJourney({ spannedSeconds })`, the time the crossed rows occupy. Given, no leave-by, no arrival, no `PASSED`; the fit is `freeAfterTravel` over the window shifted by those seconds. `OVERRUNS`, `PAST` and `ON_WAY` are reachable as before — the first only when even the combined slack does not hold the drive.
- **R5 is a required argument**: `dayTravelTotal(journeys, { unplacedLegs, spanningLegs })`. `useDayTravelReads` counts both; a caller cannot pass one and forget the other.
- **R3's word is `bindPrefix('מ', name)`, not the mockup's `מ־`.** The drawing used a maqaf; the app already decided (owner, 2026-08-31) that a one-letter Hebrew prefix binds directly to a Hebrew word and takes a hyphen before a foreign one — `מ-Nettó Hofn`, `מארוחת צהריים` — and the rule is one function. The name goes through `autoIsolate` first (ADR-0118). The two CSS rules are the mockup's: `.day-trv-from` in `--muted`, and `.day-trv-meta:has(.day-trv-from)` stacking origin and sentence, for the clipping the render found.
- **R6 on the board is `travelOrigin({ placed })`**: an unplaced started stop (or point in progress) yields the last placed started stop — or the bed — with `denied: true`, and Home's existing `originStands` gate does the rest. The docblock's refusal is rewritten to say why the walk-back is now honest. **R6 on the day row was left alone**: under F3a a spanning leg states no clock, so the only claim its live arm can make is the `בדרך` action, which is a device mark and stays offered.
- **CI caught the one hole the chain opened, and R5 gained a clause.** `e2e/day-paints-once.spec.ts` (ADR-0206 §AT2) seeds three placed rows and a placeless fourth at the end of a day with no bed, and asserts `לפחות`. The first build attached nothing to that row — no leg into it, no bed for a leg to span it — and the total read as complete: the smaller claim turned into a confident one, the exact failure §AT2 exists to prevent. So `dayRun` counts **orphan runs** (placeless rows the chain was standing nowhere behind: before anything placed with no bed seeding it, or after an exact transition), `danglingLegs(run, sleeps)` adds the trailing run where no bed follows, and both surfaces add that to `unplacedLegs`. The unit specs now hold the same day on both surfaces.
- **The deploy caught R4's arithmetic, and the owner retired a sentence with it** (2026-09-16, off the live day). The spanning leg into `Setberg` read `אין זמן לדרך`: the aurora watch's two hours were subtracted from the slack although it runs ⁦22:45–00:45⁩ and the hotel's window closes at ⁦20:00⁩ — a row cannot eat a window it is not in. `dayJourney` now takes `spanned` intervals (`spannedIntervals(spans)`) and `reservedSecondsWithin` clips them to `[departAfter, deadline]`; the same day measures ⁦45⁩ minutes for a ⁦22⁩-minute drive and fits. The sentence itself is gone everywhere (ADR-0206 §BG): the owner's call — _"doesn't add any information + is actually confusing"_ — and the record agrees, since it had fired falsely against a floor (§AJ1), against a negative hole (§AH) and now against a spanning leg. On the rows-that-touch arm the arrival alone remains (§AS5's second half).
- **A moving row with an unplaced end is a seam exactly as R2 says**, and the leg INTO it still draws where its arriving end is placed; the leg out of it is counted in `unplacedLegs`, as before this ADR.
- **What the build did not touch**: the Map (already chained), sharing (already chained), `travelEstimateSchema`, and the day head's `לפחות` line beyond its new second reason.
- **Specs**: the eighteen rows that a pure derivation can answer are in `lib/day-joins.test.ts` (`dayRun`, `dayJourney` across a placeless stop, `spannedSeconds`, the raw strips and the floor); the screenshot's shape — a placeless call between lunch and the theatre — is on both surfaces (`DayView.travel.test.tsx`, `PlanDay.travel.test.tsx`: one block, before the destination, named for the origin, no clock, raw strips, one route asked for two points); the block's own word in `DayJoinRow.test.tsx`; the board's walk-back in `hero-travel.test.ts`.
