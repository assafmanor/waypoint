...
> **Narrowed 2026-09-16 to "every event on a day you have reached"** ([ADR-0228](0228-the-quick-actions-are-one-list-and-a-commitment-can-be-settled.md) §7). Both reasons above are about **the clock inside a day** — the dinner they protect is _tonight's_ — and this row carries references from every day of the trip, so "any event reference" quietly covered Thursday's check-out as well. A reference the trip has not reached offers no pair. Neither argument is lost: the early mark still stands on today, and the undo is untouched, because an **answered** reference keeps its control (with `outcome` set it is the record plus `ביטול סימון`, never the pair) — which is the self-defeating gate this section named, avoided rather than re-introduced.
