...
**Amended 2026-08-16 (two owner reports, and they are one defect): a titled note's body is a line of its OWN, not a demotion into the meta line.** The reports: _"I want to improve how the notes screen looks. It looks really bad with long notes"_, and then _"Note descriptions formatting (new lines, spaces etc.) don't show in the note preview … the line breaks were added to make it more readable, so they must be honored."_

**What made them one defect is where a body renders, counted rather than recalled.** There are four places, and only one was broken: `.note-body-line` (a body-only note, this screen) has both a clamp and `white-space: pre-wrap`; `.note-item-b` (a host's section) has `pre-wrap`; Home's hero preview is one line by design. And `.wp-listrow-meta` — where this section sent a **titled** note's body — is a shared `ListRow` class with **neither**. So one misplacement produced both symptoms: no clamp, so a **closed** row printed the entire body at 11.5px `--muted` (measured at 285px and 16 rendered lines at 360px, with the author and the elapsed time pushed off the end of that paragraph — the meta line had lost exactly what it exists to carry); and no `white-space`, so every authored line break collapsed to a space.

**This does not retract the section's rule, it bounds it.** "A note with a title and a body shows the title, and the body drops to the meta line" was justified by _"printing both is the same sentence twice"_ — which forbids printing the body **twice** and says nothing about which line it belongs on. The body is still printed once. It was never bounded because every note in `notes-screen-v1.html`'s nine states had a short body, so no state in the design could show what an unbounded meta line does.

**The fix is one declaration, because the right element already existed.** `.note-body-line` clamps to two lines, honours the composer's newlines (ADR-0152 §6b's 2026-08-07 amendment) and unclamps under `.wp-listrow.is-open`; `.wp-listrow-title` is already `flex-wrap: wrap`. So `flex: 1 0 100%` puts that same element on its own line under the title, and the clamp, the breaks and the open-state behaviour all arrive with it. **Both shapes of note now read through one element.** A first draft added a `sub` slot to `ListRow` and it was thrown away under root `CLAUDE.md` rule 8: no other list wants a summary line, and a shared primitive should not grow a prop to avoid using the class that already does the job. Deliberately not a size step either — a note's words read the same whether or not somebody gave them a title.

**Measured** ([`mockups/notes-long-note-row-v1.html`](../../mockups/notes-long-note-row-v1.html), the real note from the owner's screenshot over the real CSS): the row goes **285px → 98.4px** at 360 and 255 → 98.4 at 390, the six-row list **679 → 493px** (−27%), and a body-only note costs **exactly 0px** — asserted in the file's own table rather than claimed, because the whole point is that the change does not reach a note shape that was not broken. The row's height is now constant per note regardless of body length. Two alternatives were drawn beside it and rejected: clamping the meta **in place** (75px) keeps the words at provenance size, still swallows the breaks, and — visible only once rendered — deletes the author and the time outright, since the clamp applies to the whole meta line and they sit at its end; and **one line for a titled note** (60px) is the densest and tells the reader nothing. Also rejected: adding `pre-wrap` + a clamp to `.wp-listrow-meta` itself, the shortest-looking fix for both reports, which would change bookings, documents and members to repair a note and still leave the body at 11.5px `--muted`. **The question is where the body lives, not which rule is missing where it was exiled to.**

**And the row's chrome aligns to the top.** `ListRow` centres the row and its trigger, which is right for the fixed two-line rows it was born on and wrong the moment one row can be four lines and more when open: the badge and the `⋯` floated at mid-height, pointing at nothing, which is visible in the owner's screenshot. It ships as a note's own `.note-row` class rather than a change to `list-row.css` — bookings, documents and members cannot grow, so moving the shared default would shift their `⋯` to fix a screen they are not on — and every note row takes it, not only the tall ones, or the badge would sit at a different height on consecutive rows. The trailing group's inset is derived from the kebab's own geometry (30px of ink in a 44px box with `margin: -7px`, so its centre lands at `padding + 15`, against the badge's centre at `space-3 + 18`); verified in a browser against the shipped stylesheets, badge centre and kebab centre both at 31px. The selector is `.wp-listrow.note-row` and not a bare `.note-row`, because the rule it overrides is also one class and a tie would be settled by whichever sheet the bundler emitted second.

**One consequence, left to a device pass rather than decided here.** Honouring the breaks makes the two-line preview more expensive: a first authored line that wraps consumes the whole clamp, so the note in the owner's screenshot previews as `⁦החלון המעשי הטוב הוא בדרך כלל: 22:00–02:00⁩` and nothing else. That is the author's own first line, so it is not wrong — but it makes 2-vs-3 lines a real call, and the mockup carries it as a control with 2 shipped as the default (it is what every body-only note on the same screen uses, and a mixed clamp would read worse than either).

**Three measurement traps this cost, recorded because each produced a confident wrong number.** Counting `Range.getClientRects()` as lines reported **32 against 61** for one sentence — rects fragment per bidi run, so a line of Hebrew carrying `Aurora forecast` yields three; line counts must come from **distinct rect tops**. `height ÷ line-height` is not the fallback either: `.wp-listrow-meta` declares no line-height, so it answers `NaN`. And counting **lines** to measure a swallowed break reported a four-line note as "+0 lines lost", which is true and meaningless — what a break costs is **which words begin a line**, so it is asked per break (0→1 kept for the note in the screenshot, 2→3 for one typed as four lines). The same trap recurred a fourth time during verification, where a Range rect count said the shipped clamp was not applying; the element's **height** said 36.4px, exactly two lines, and was right. `display` computes to `flow-root` for a blockified `-webkit-box` flex item and the clamp still applies — the computed value is not the evidence, the box is.

**And a fifth, which is about the before/after format rather than about a property.** The proposal's negative margin — cancelling the title flex's `row-gap` under a title — was first written as `.wp-listrow-title > .note-body-line`, which also matches a body-ONLY note, whose words _are_ the title line and have no sibling above them and so no gap to cancel: it was taking 2px off every body-only row (**76.4 → 74.4px**), the one note shape this change has no business touching. **The mockup reported that cost as 0px and its table was not lying** — both frames render body-only notes through the same element and the file's one PROPOSED block applies to the whole page, so the regression sat in the baseline as well as the proposal and the delta really was zero. A before/after pair cannot see a regression common to both halves, and a `0px` row written as an assertion reads as a guard while it compares two contaminated numbers. What caught it was measuring against the **shipped** stylesheet instead of against the other frame. Scoped now to `.wp-listrow-title > * + .note-body-line`.
