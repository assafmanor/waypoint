...
`noteUrlHref` (`lib/notes.ts`) is what a stored string has to pass first, and it exists for two reasons that are easy to miss: the field's own placeholder is scheme-less (`instagram.com/p/`), so a bare host is the common case and a raw `href` would navigate **inside the app**; and an href built from user text is an injection seat, so a string that already carries a scheme must carry http(s) or it is not rendered as a link at all.
