...
## Amendment (2026-09-10) — the departure clock ends once, and the lift carries it too

The owner opened the built board at 00:17 on the morning of departure and asked two things: that the flap clock never reach ordinary events or booking timers, and that the lifted hero, which read `15:12 שעות` over a board reading `15:12:08` in flaps, show the same clock.

**The gate is the day's first timed thing, not "nothing is running".** §3 says _"until the first timed thing starts"_; the build gated on `!nowEvent && today === trip.startDate`, which is true again every time day 1 falls quiet — in the layover between two legs, on the drive to the hotel — so the departure clock would have come back counting to a connection. `Home` now also requires `shownNext` to be `firstTimedOn(events, today)`, the same read the eve's hero names under its clock. So the flaps appear on one day, before one thing, and never again: the layover, the second leg, the check-in, every later day and every booking timer count on the `H:MM` ladder. A leave-by or a shutting window still keeps its words (ADR-0206 §Z1 / ADR-0184 §6).

**The lift is the same tile.** §7 stated the rule for the plan lift and the trip lift had the same defect the other way: `Board` and `HeroLift` each carried a hand-typed copy of the countdown tile, and `flap` reached one of them. The tile is now one exported component, `CountdownTile`, drawn by both elevations, and `Home` hands the lift `dayOneClock` rather than the plain countdown. `HeroLift.test.tsx` asserts the flaps on the lift; `Home.leave-by.test.tsx` asserts the ladder once the first thing has ended.
