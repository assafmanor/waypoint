...
> **Amended 2026-09-20 — `tripEnd` bounds an end a person PICKS, not one a journey DERIVES.** These bounds were written for two date fields, each operated by hand; [ADR-0203](0203-a-journey-has-one-date-and-its-arrival-is-a-clock.md) §2 replaced a journey's second date with a derived moment and the bound came along with it, so the ordinary flight home — ⁦22:10⁩ on the trip's last day, landing ⁦02:30⁩ — was refused at a field no control can move. A journey's arrival is no longer bounded by `tripEnd`. Nothing is placed outside the trip by allowing it: [ADR-0037](0037-overnight-events.md) §1 files a span under the day it STARTS, which is why an event ending at ⁦02:00⁩ on the last day has always saved from `EventForm`, whose guard bounds the **date** alone. What stays bounded is any moment that DATES a booking — a leg's departure, a layover's included — because a leg filed outside `[startDate, endDate]` is one no day surface can show. A hotel's checkout is still a picked date and keeps this bound in full.
