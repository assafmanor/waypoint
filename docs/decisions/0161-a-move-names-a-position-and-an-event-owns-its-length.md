...
3. **`החלף` had no test at any level**, which is how "skip it and post a toast telling the user to go looking" shipped and stayed. It now has the reducer's single-snapshot property, the verb's write order and undo order, the row's exclusions, and an e2e over the whole flow in Trip mode.

**Fixed same day, off the shipped build: `החלף` on an UNTIMED row took the day view blank.** §10 already says an untimed event holds no position of its own, and the verb is "taken on the slot" — so there was nothing for it to be taken on, and offering it anyway asked the shelf to be ranked against a slot with no clock. `zonedIso(date, '', tz)` builds an Invalid Date and `toISOString()` throws on it, during render, so the whole tree unmounted. Two rules now, one at each end: **an untimed row is not offered the verb** (the same shape as the hard and done exclusions beside it), and **the derivation refuses to invent an instant it was not given** — a clockless slot ranks against the day, which is `slotStops`'s own honest fallback. The exclusion list in §6 is therefore three, not two. Worth naming because of how it escaped: every fixture in the new tests carried a time, so the whole suite agreed with itself while the one shape nobody wrote down took the screen down.
...
_Amended 2026-08-09 ([0178](0178-a-day-row-says-what-then-when.md) §3), and the amendment is deliberately small: **the button changes cell, not meaning.** The row became a grid whose second line is the when line, so `.bld-time` moved out of the trailing slot and under the title — same element, same target, same picker, same rule that a row's own time is what you press to move the event. Two things this section says stop being true as written: the table's "trailing slot" is now a **row**, not a column; and the "16px off the title's width" this section charged itself is repaid many times over, since the title goes from 48px to 214px at 360px. The 55px inset overlay stands and is still what keeps the chip off ADR-0017's floor. What actually motivated the move was not this control at all — the time COLUMN was 124px of a 302px row and was sized by its own footnote (`3:30 שע׳` plus the zone pill measured wider than the range they annotate), which left the title 17% of the row and made a 16-character route paint straight onto this button._
