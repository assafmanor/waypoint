...
> **Two structural findings from [ADR-0121](0121-embedded-map-phase-6-design.md) belong to this section** (its §5): the Map tab **has no layout to hang the split on** — `.map-screen` sets only accent tokens and every screen renders inside `AppShell`'s scrolling `.body`, so the tab must opt out via a shell-level full-bleed body modifier, with the filter/sort rows becoming the split's fixed header and the sheet clearing `.nav` + `--safe-bottom`; and the map div must be sized to the **visible** area rather than sitting full-bleed behind the sheet, because the Maps JS API renders Google's required attribution at the bottom-left of the map div and a peeking sheet would obscure it (which also makes §6's fit-to-bounds honest).
>
> **Designed in full 2026-07-26 by [ADR-0121](0121-embedded-map-phase-6-design.md)** (+ [`mockups/map-embedded-v1.html`](../../mockups/map-embedded-v1.html)) — the session this ADR deferred it to, after the API/pricing reconfirmation it required. The vision below is unchanged and now concrete: **three snap heights** (peek / half / full) with the segmented toggle as a shortcut to the two extremes of the same axis, the sheet **always peeking** (the list is the only view that works offline and the only one that can hold a coordless place), and the sheet + toggle as **view state, not a back layer** — a pane of the tab rather than an overlay over it, so neither `Modal`/`useOverlay` nor a back registration is involved. §3's pin grammar becomes `AdvancedMarker` content built from **our** markup (Google's `PinElement` cannot express the dashed-idea / desaturated-ambient states, so badge and teardrop share the `--cat-*` tokens by construction), §6's single amber next-stop ring **moves onto the pin** as this ADR's session-104 amendment promised, and §3's "hollow dashed ring" coordless pin is retired as unbuildable and unneeded — a place with no coordinates has no position to pin.
...
> **Phase 5's design landed 2026-07-25 in [ADR-0115](0115-plan-mode-place-research.md)** (`mockups/map-research-v1.html`), which refines **§2** of this ADR: the Plan-mode search icon does not _replace_ the free filter with Google research — the overlay opens free in both modes and Plan mode **adds** the paid half behind an explicit `חיפוש בגוגל`, because Phase 5 is the first surface that spends money per keystroke. Mocking against the real picker (as instructed above) also killed three fields `plan-mode-v1.html` drew that the shipped pipe cannot supply: the ★ (Enterprise-tier, deferred by [ADR-0111](0111-places-field-mask-tier-and-rating-deferral.md)), a distance (a prediction carries no coordinates), and a category glyph (`types` aren't in the field mask).
>
> **Phase 6's design landed 2026-07-26 in [ADR-0121](0121-embedded-map-phase-6-design.md)** (`mockups/map-embedded-v1.html`), the last of the deferrals above. It followed this section's instruction literally — reconfirm current Maps API + pricing **first**, then design — and the reconfirmation is recorded in its §1: ADR-0106 §A–F and ADR-0108 §1 held, two details moved (a `mapId` is now mandatory rather than merely enabling, and the cheap Routes tier caps at 10 intermediate waypoints). §10 above carries the resulting shell; **paid Routes / live ETAs remain sequenced after**, the one piece of ADR-0106 §D still unbuilt.
