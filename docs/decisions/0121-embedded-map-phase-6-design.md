...
> **Amended 2026-07-27 by [ADR-0122](0122-map-split-controls-over-the-canvas.md)** (mockup: [`mockups/map-split-v2.html`](../../mockups/map-split-v2.html)), after the owner used the shipped tab on a phone: the map and the list were both too small, the drag was unpleasant, and the filter chrome was cluttered. Measured against the real layout tree, **370 of 844 phone pixels are spent before either half gets one**, 94 of them this tab's two fixed rows — so the filter row + sort strip **stop being the split's fixed header** and become one decluttered row floating **over the canvas** (the split is then the whole body; pins are kept clear of the row by the fit's inset, not by layout). `peek` becomes "one whole row" (116px showed 0.8 of one), `full` stops below the controls, `half` keeps 0.56 — the measurement exonerates it. The drag target widens to the whole `.wp-snapsheet-top` with a slop threshold and a velocity term, the toggle gains a thumb that shows the third stop, the geolocation pre-prompt moves out of the list's scroll region onto the canvas, and **a pin tap stops moving the sheet at all** (§8's note below — the peek row is where the tapped place speaks). Both constraints below survive verbatim: `--sheet-h` is still written from the snapped stop, and nothing re-creates the map. Read §5 with that amendment; the reasoning it rests on is unchanged.
...
> **Closed 2026-07-27 by [ADR-0122](0122-map-split-controls-over-the-canvas.md) §7 — no info window, and a pin tap no longer moves the sheet.** Judged on the running app, from the owner's report that a pin tap must surface the place without interrupting the interactive map. **The peek row is the info window**: with `peek` re-derived to show one whole row (ADR-0122 §3 — at 116px it showed 0.8 of one, which is why session 136's fix had to _raise_ the sheet to show anything), a tapped pin's row is scrolled to the top of that one-row viewport and **nothing resizes** — so the map keeps every pixel and the camera stays where the user put it. That **revises session 136's raise** (the Phase-1 fix for report #4) while keeping its scroll, and it replaces "the raise mirrors the drop" with the rule that actually explains both directions: **the sheet moves only when what you just asked about is not on screen** — a pin is, so nothing moves; a **row** tap normalises to `half`, from `full` because the map it focuses is invisible there (the rule above, unchanged) and from `peek` because the row's own way-in block does not fit a one-row viewport. So the first tap identifies and one tap on the surfaced row opens the door. An info window was rejected for the reason this note already suspected: a second way of stating a place, over the pins it describes, with its own dismissal to design.
