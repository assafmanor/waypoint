...
>
> **3. Still open, and stated rather than fixed:** the merge takes `at`/`edge` from the **earliest** reference on a day and `until` from the **latest**, so a place that is genuinely ahead (something real later today) can sit under `מה שלפנינו` with its meta line naming a reference that has already passed. Both halves are deliberate and only their combination misreads. Fixing it properly means the day carrying its moments rather than one collapsed pointer, so the read side — which has the clock — can name the one that is actually next. Not attempted here.
>
