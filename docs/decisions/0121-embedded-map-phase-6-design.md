...
> **Amended 2026-08-20 — "focused" was never built, and the word is the whole of this fix** (owner: _"going from a place to the event (and maybe also booking) doesn't scroll correctly. Check plan day and trip day"_). The event target navigated to the day and stopped there: on a full day that leaves you looking at whatever the day opens on — in Trip mode, the now-line it scrolls to by itself — with nothing saying which of its rows you came for.
>
> The channel it needed already existed and belonged to something else: `?event=<id>`, which ADR-0153 §8 built so a note could reach its host. So the entry hands the event to that, and the two day surfaces answer it — **Trip's card expands and lands at the top; Plan's builder row lands and opens nothing.** That difference is posture, not a half-build (ADR-0159 §1): Plan's row raises a detail SHEET, and a modal opened by a navigation would hide the day you were sent to.
>
> Three things this pins down beyond the one report:
>
> - **Both surfaces, one selector.** `EVENT_ROW_ATTR` (`data-event`) is stamped by `EventCard` and by the builder row — which gives up its own `data-bld-id`, so the drag's `closest()` and the arrival ask one question. A reference does not know which mode will draw the day it is sending you to.
> - **The landing is watched, not fired once** (`lib/land-at-top.ts`): expanding the card grows it by its documents, tasks and notes after the first aim, which is the same clamped-`scrollIntoView` trap ADR-0168 §3 records.
> - **Trip's "land on now" stands down** when the arrival names a card, latched at mount because the param is spent a render later. Two scrolls with different opinions about where a day should sit is worse than either.
>
> **The booking target is unchanged and that is not an oversight:** it opens `BookingDetail` where you are standing, so there is no day to land on and nothing to scroll. The idea target still opens the shelf's day without naming a tile — the shelf is a strip, not a scroller, and `?idea=` opening a modal on arrival is a decision nobody has asked for.
...
>
> > **Partly reversed 2026-08-07 by [ADR-0171](0171-a-time-can-be-a-floor-or-a-ceiling.md) §7 — a layover is not going somewhere twice.** Owner, from the device (field report #10): Ben Gurion `1`, Vienna `2`, Keflavik `4`, where the missing `3` is Vienna counted once as its connection's arrival and once as its departure. **"A place you go to twice is two stops" was decided the day before that report and it is right about a revisit, which is the case it was reasoning about** — a place whose landing was at 02:00 and whose car is due back at 18:00. It is wrong about the one case where the two moments are the **two ends of one derived connection**: that is not two visits, it is waiting.
> >
> > So the merge condition is exactly `connectionMinutes` — the same derivation the day's band and the pin's own word already read (ADR-0159 §3/§6), keyed by place **and** day — and nothing else merges. Two moments at one place with nothing drawn between them (a morning landing, an evening car return) still take two numbers: adjacency in the stop list is not evidence of one visit.
> >
> > **Everything else above is preserved, and preserving it is a constraint on the build rather than a hope**: the sequence is still built and numbered with **no clock**, a place visited once still cannot move, a genuine later revisit still consumes its own number, and a filter's gaps are still correct and informative. Read ADR-0171 §7 for the full argument; it is the reversal's home and this block is the pointer.
...
## Amendment (2026-08-11, session 247) — a load failure is absent too, and the signal is a watchdog

Field report **#28** (backlog workstream M): on some devices the Map tab keeps drawing the place list and our own pins — `AdvancedMarker`s are DOM overlays, independent of Google's tile layer — while Google's own terrain never paints. `MapPane.tsx` had **no load-failure, error or retry branch anywhere** around `<APIProvider>`/`<Map>` (verified 2026-08-10); a restart was the only recovery.

**This is §11's rule again, not a new one.** §2 and §11 already hold that a missing key, a missing Map ID and offline all degrade to the SAME list-only outcome — "absent, not disabled: no pane, no toggle, no map instance, no billed load." A load that fails after construction is a **third** reason to land there, not a fourth grammar: `MapPane` now keeps its own `mapFailed` state and swaps its canvas slot for `ErrorState` (with a retry) the moment either signal below fires, while the place list beside it — a sibling in `screens/Map.tsx`'s split, never a child of the pane — is untouched.

**Two signals, because one class of failure has an event and the other does not:**

- **A failed _script_ load** — bad key, blocked network, a referrer restriction — is `@vis.gl/react-google-maps`'s own `APIProvider.onError`. Reliable, immediate, no invention needed.
- **A script that loaded fine and never painted a tile** — the report's own shape, since our markers rendering at all proves the script loaded and the `google.maps.Map` constructed — has **no equivalent event**. `useApiLoadingStatus()`'s `AUTH_FAILURE` member exists in the type but is never actually set by the installed version (1.9.0) checked for this workstream; only `onError`/`FAILED` fire in practice. So this is the heuristic backlog workstream M's own bullet anticipated: **`onTilesLoaded` never firing within a bound is treated as a failure**, run through `lib/deadline.ts`'s existing `withDeadline` (the same mechanism field reports #20/#22 built for "an await with no guaranteed completion signal"). The bound is `constants.ts`'s `MAP_LOAD_TIMEOUT_MS.TILES` (10s — between `LOCAL_READ_TIMEOUT_MS.HANDLE`'s 3s and `API_TIMEOUT_MS.FETCH`'s 20s, sized as _"this is dead,"_ never _"this is slow,"_ the same house rule `constants.ts:145-155` states for those two). **Unmeasured** — no device has reproduced #28 in this session, the same caveat #22's own bounds carried at the time — and owed a real device pass before it can be called right rather than merely defensible.

**Retry is one tap, never automatic, and always a fresh instance** — §4's one-billable-instantiation invariant is not suspended for a retry: `ErrorState`'s retry bumps a `key` on `<APIProvider>`, which remounts the whole subtree rather than reusing (or silently hoping to recover) the failed one. No backoff, no retry cap: a human tap is already the throttle, and an automatic retry loop against a permanently broken connection is the thing §4's arithmetic was written to rule out.
