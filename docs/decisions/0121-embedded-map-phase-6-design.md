...
- **`disableDefaultUI`, then add back only what we need.** Google's default controls are Google-chromed, unlabelled, and unaware of an RTL page — the opposite of a brand-styled map. Zoom is the pinch; the one control we add is a **re-centre / fit** affordance, which is also the escape hatch from §6's "a manual pan wins".
