...
### 10f. Amendment (2026-09-05) — the strip retires; a commitment without a clock is a row ([ADR-0219](0219-a-day-is-a-place-you-can-see.md) §4)

§10a-i put an untimed `hard` event **at the top, in the `.day-ambient` strip that already existed**, so as to add no band. The strip is now deleted (ADR-0219 §4: it stacked three kinds of thing in one teal box, and the owner did not like how it looked). What §10a-i actually decided survives without it: **a claim on your day reads at the top**. `UnplacedCommitment` keeps its name, its props and its compact settle pair (Trip only, §10e), and is re-rendered on `.transition-row`'s tree — the committed point's amber box and 32px circle badge (ADR-0210 §1), the title, and its when line in `.tr-time` — as the **first child of `.day-list`**, above the day's first row.

**One detail the build settled, because the row has two cases and the sentence above only described one.** `ללא שעה` is not a clock, so it takes `data-bound="exact"` and draws no box. A FLOOR (`מ-15:00`) is a clock, and it takes `not-before` — the same open-ended box it would wear one row down (ADR-0210 §2). Joining a grammar means joining it: a floor that drew no bound here would be this row saying something different about the same fact, one row apart. The facts the strip also carried (the day's total, Plan's verdict) move into the day head (ADR-0219 §2); a span's middle-day count leaves the day (ADR-0163's 2026-09-05 amendment).
