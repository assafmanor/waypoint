...
The field order becomes: **title row → type selector → name → pick tiles (or preview) → actions**, so the sheet reads top-to-bottom as "what kind → what to call it → the file it attaches to." The name still auto-fills from the filename when left empty (unchanged from today), so putting it first costs nothing — a picked file back-fills a blank name, and a name typed first is preserved.

## Consequences

- **Frontend only.** `DocumentUploadSheet.tsx`: the native `.bs-field` file input becomes two off-screen inputs (file + capture) driven by two `.pick-tile`s, plus a `.dz-file` preview; the name `.bs-field` moves above the pick block; a `titlerow` is added; `pick()` gains objectURL creation/revocation for the thumbnail. New CSS in `screens.css` (`.pick-*`, `.dz-file`, `.thumb`), authored from existing tokens; the `.doc-upload .bs-field input[type='file']` rule is removed.
...
- **Copy:** new `he.ts` strings under `docs.upload` — `upload` (`העלאת קובץ`), `capture` (`צלמו עכשיו`), `pickHint` (`תמונה או PDF · עד ${mb}MB`), `remove` (`הסר`). The existing `fileLabel` is retired (the tiles' own copy replaces it), and the `titlePlaceholder` drops its person-name example (`למשל: דרכון · אסף`) for a neutral `שם לזיהוי המסמך` — the sheet shouldn't seed a real-looking name.
