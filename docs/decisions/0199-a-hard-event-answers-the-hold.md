...
- **`useHoldToDrag`'s handler type becomes a union**, and every existing call site is on the drag arm of it. The three refusal-path keeps in §1 are the test surface, and the design draft was wrong about where they belong: it said the click swallow was invisible to jsdom, and it is not — `fireEvent.click` after the release asserts it directly, which is how the **timing** of the swallow got pinned too (armed at the release, so a finger resting past `DRAG_CLICK_SWALLOW_MS` still gets it). What is genuinely browser-only is narrower and is what `e2e/hard-row-hold.spec.ts` covers: whether the beat **runs** (jsdom has no CSS engine, so `motionDurationMs` answers 0 there and every unit assertion takes the no-animation branch by construction), and whether text **selects** — `user-select` is a rendering property with no jsdom selection model behind it.
- **Two guards in this change are load-bearing in a way a green suite does not show, so each was checked by mutation.** Removing `move`'s `if (refused.current) return;` fails exactly one unit test; removing the `.is-pinned` animation fails one e2e; removing the `user-select` rule fails two. A gesture test that passes with the mechanism deleted is the failure mode this repo has paid for before (`queryByText` against a stale literal), and it is cheap to rule out here.
