...
**Status:** Accepted — designed 2026-07-28 (session 157), **§1–§8b built the same day (session 158)**, and **§6 REVERSED plus §10's picker half built from a device pass (session 159)** — see the [Build log](#build-log-2026-07-28-session-158) and the [Device pass](#the-device-pass-2026-07-28-session-159-and-what-it-overturned). **§9 (the long press) and §10's errand are NOT built.** Two further device reports — the app chrome during search, and unsaved Google results as pins — are their own design session and are named at the end. The rendered canvas has still not been seen (ADR-0121 §13) and nothing below claims otherwise; the closing sections name what was measured against a renderer and what was not.
**Date:** 2026-07-28
...
## Build log (2026-07-28, session 158)

§1–§8b shipped and needed no reversal. What the build had to decide, refine or find is here rather than in a new ADR, because none of it changes a decision this one made. The rendered canvas is still unchanged from ADR-0121 §13 — it was **not** seen in this session, and nothing below claims otherwise.

**What was deliberately left out, and why it is not a partial build.**

- **§9 (the long press) is blocked on Phase 6b, not deferred by preference.** A dropped pin needs a `Place` with coordinates and no `googlePlaceId`, and the only create path that exists is `createPlace({ name })` — `saveNameOnly`'s. A coords-bearing create is a backend + `@waypoint/shared` change, which is exactly the data-model work §12 hands to 6b (along with verifying that the nullable `@@unique([tripId, googlePlaceId])` really does let several coordinate-only places coexist). Building the gesture against a write that cannot store the coordinates would have been the half that looks finished.
- **§10 is the separable tier this ADR's Consequences already ordered**, and it lands on a different component tree (`PlacePicker`, `BookingDetail`, `IndexBookingsView`, `nav-state`, `map-scope-state`). The picker's `בטיול` half is the cheap win and should go first; the errand follows.

**Six things the build found.**

1. **`isAsidePin` has FIVE readers in the code and §4's table enumerated three.** The two it missed — `setGhostId` (the tap that surfaces a missing row) and `ghostsInArea` (the area sort's shortfall banner) — read the tier for a **different question**: not "is this subordinate?" but **"is this pin's row absent from the list?"**. Under a live query the list is trip-wide, so the answer is uniformly **no**, and both had to become query-aware for a reason that is not the promotion. Left alone, a tapped match would have surfaced a `.map-ghostrow` duplicating a row already on screen, and the area banner would have claimed `N מקומות באזור אינם ביום הזה` over a list that was showing them. §4's rule survives; its census was short.
2. **`aside` had to join `pinsKey`, and forgetting it would have been intermittent rather than absent.** The pin array is memoized on a content key so a clock tick diffs to nothing (ADR-0121 §4); a promotion that changed a rendered class but not that string hands the memo an "equal" array and the markers keep the old ratio. The pin **set** usually changes with the query too, so the bug would have appeared only when it did not — the worse kind.
3. **`MapPin.aside` defaults from the tier when absent**, which was not in the design and is what keeps it a **withdrawal** rather than a field every caller must remember. `MapPane`'s own tests construct pins from a tier alone, and making them all pass a redundant flag would have been the tail wagging the dog. Same `??` shape as `isFramedByCamera`.
4. **Two arrays became one list whose predicate switches**, and that is the change that actually retires the overlay. `searchRows` existed _because_ the query rendered somewhere else; with the query as a control on this row it is the same list narrowing, so there is one predicate, one count, one empty state and one renderer. A row that stops matching now collapses in place through the shared reveal (ADR-0120) exactly as a chip's does — which the two-array shape could not do across the boundary.
5. **`.map-instate.shelf` was a third rule-4 violation, and it is deleted rather than re-coloured.** §8 caught `.map-arm` and `.map-addmaybe`; this one is the "already on the shelf" chip on a research result row, plan violet and Plan-only until §8 put Google's half in both modes. The two chips it distinguishes say `כבר בטיול` and `על המדף`, so the **words already carry it** and the colour was restating them — one rule fewer beats one rule recoloured. (`.map-tag.mbadge`'s `--plan-deep` is the same family but **pre-existing**: shelf rows have rendered in Trip mode since long before this change, so it is not this build's to fix. Backlogged.)
6. **`PlaceResearch` hands the hook a query only once it is non-blank.** Removing the arm made the effect unconditional, which meant a whitespace-only query reached `usePlaceSearch` — harmless, since it trims before the floor, but it broke a test that was asserting something worth keeping ("nothing typed → nothing reaches the paid core"). Fixed in the component rather than the assertion: a component that hands its collaborator input it has itself decided to ignore stops being harmless later.

**Two test notes.**

- **The floor's fixtures are now derived from the floor.** `usePlaceSearch.test.ts` had three cases on a literal 2-char query, which at a floor of 3 does not **fail** — it goes inert and keeps passing while testing nothing. Both sides now read `PLACE_SEARCH_MIN_CHARS`, so the next change to it cannot silently disable a test. This is the failure mode §8b's Consequences predicted, and it is now structurally impossible rather than merely fixed.
