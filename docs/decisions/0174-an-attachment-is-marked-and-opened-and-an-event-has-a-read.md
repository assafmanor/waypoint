...
**Answered the same day, and the answer was neither** (ADR-0219 §6, amended in place; owner: _"the place's details are truncated, there should be an option to expand to read all the text (it should look subtle)"_). The unreachable text was the whole of the hole, so `deciding` opens its extract **where it stands** — `עוד` / `פחות` off the block's own `.map-know-more`, caret down and up rather than through — and renders that control only when the prose does not fit the box it is given. No new prop, no second exit, and the Map's deciding row gains it too: `עוד בגוגל` was never a way to read the rest of a paragraph, it was a way to leave.
