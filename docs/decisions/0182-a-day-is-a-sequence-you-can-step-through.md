...
## AMENDED 2026-08-13: the device pass, and the three things a desktop render could not see

The pass the Consequences below call for happened, on the owner's handset. **Neither of the two questions it was owed came back raised** — the horizontal snap did not fight the map's pan, and the selection landing on settle did not read as late. What came back was three defects, and the useful thing about them is that not one is a gesture bug.

**1. The swipe was reachable only from the top of the card.** Owner: _"swiping left or right should be made available no matter where you place your finger on the card. Currently swiping is enabled only when doing it from the top of the card (where the title and description is, but not on the notes for example)."_ Two unqualified one-axis properties, and the second one contradicted the sentence written beside it:

- `overscroll-behavior: contain` on the card's scrolling regions — `.note-sec-list`, and `.map-sum` in the expanded state. It was added for a real reason (reaching the end of the notes must not hand the gesture to the canvas), but **an `overflow-y: auto` element is a scrollPORT on both axes** — a neighbouring `overflow-x: visible` computes to `auto` — so an unqualified `contain` also refused to chain the **horizontal** gesture out to the track. `overscroll-behavior-y: contain` keeps the fact it was written for and gives the other axis back.
- `touch-action: pan-x` on the track. `touch-action` is **intersected down the ancestor chain**, so naming only the horizontal axis on the track took the vertical pan away from every scroller inside the slide — including the note list, which §the-card's-own-rule calls the card's one scrolling region. The comment beside it claimed the opposite (_"the slide's own content keeps the vertical one"_), which is how it survived a review: the code and its comment disagreed and the comment was the plausible one. Now `pan-x pan-y`, with the browser still picking by direction.

**2. The card takes the pane.** Measured on the handset: **~535pt of a ~629pt canvas — 85% of the map — and ~218pt of it empty**, on a stop with enrichment and no notes. Two causes, and only one of them is a defect.

- The bound §9's 2026-08-11 amendment made unconditional is a **guarantee** — the card is never taller than the room it has — and on a tall phone the room it guarantees is the whole pane. So it gains a second bound: `--map-card-max: 420px`, through `min()` so it can only ever bind **lower** than the guarantee, leaving a small phone (where the arithmetic is ~320px) byte-identical. Past it the note list shrinks and scrolls, which is ADR-0148 §1's shrink order already built and now actually exercised. **420 is a device-pass number like `--map-peek`, not a decision** — retune it on a handset.
- The empty part is **the density amendment above working exactly as decided**, and it stays. Confirmed by measurement rather than read: emptying the selected slide's note list in `map-stop-traversal-v2.html` leaves its box at **315px, unchanged** — every slide stretches to the tallest, so a stop with no notes wears its neighbour's height. Reverting to short neighbours would delete the empty space **and re-open what the owner rejected**: the peek is a 20px-wide strip of the neighbour's _full height_, so a short neighbour reads as a short strip — _"a sliver, not a card edge"_, which is the sentence the density amendment was written to answer. A card capped near the loaded height is the cheap half of that trade; the imbalance is the owner's own call and is not this pass's to reverse.

**3. The card had no visible way out.** Owner: _"we should add a way to close the card and/or collapse it."_ It had three dismissals — a tap on blank canvas, system back, selecting something else — and **none of them visible on it**, because its body is inert by design (ADR-0122 §7) where a list row answers a second tap with `onDeselect`. So `.map-cardclose`, and the one decision in it is **where**: the identity row's **fourth column**, not a corner overlay. The card scrolls as a whole once its pinned rows alone exceed the cap (§9's amendment), and an absolutely-positioned corner control would scroll out of reach at exactly the card's tallest. As a column it costs ~28px of the row's **width** and **0px** of its height, which is the axis this pass is about. It runs `clearSelection` itself, not a handler beside it (ADR-0103's 2026-07-29 amendments). Measured in the mockup: its 44px target overlaps `נווט` by **3px**, the same trade `.map-rename` records one element up.

Two things this pass did **not** settle. The two numbers that shipped as recommendations — `--map-peek` (20px) and `MAP_TRACK_SETTLE_MS` (120ms) — were not reported on either way, so they stay recommendations. And if 420px still reads tall, **the next lever is not the notes**: it is `.map-refs-foot`, whose wrap the 2026-08-10 amendment already measured at **44 → 82px** on a slide narrower than the card — 38px of pinned height, spent on a footer that holds `שיבוץ ליום`, `עוד בגוגל` and delete.
