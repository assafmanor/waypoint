...
- **A past trip (ADR-0040) sends nothing.** Membership is read at send time (ADR-0197 §2.4), and the sweep's filter is `endDate >= today`.
  > **Amended 2026-09-25 by [ADR-0239](0239-a-finished-trip-is-a-memory-not-a-plan.md) §3/§8.** Two exceptions. A task deadline that falls **after** the trip still notifies, since it is still owed. And one anniversary push per trip per year may be sent, fenced by that ADR's §8: it must name a place marked `היינו`, it has its own `notifyMemories` preference, and it is not time-critical. The first exception is **built 2026-09-25** (PR #870); the anniversary push is not.
