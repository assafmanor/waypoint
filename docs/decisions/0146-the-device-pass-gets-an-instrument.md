...
**Amended 2026-08-12 (session 256), a fifth choice that had to be learned rather than designed: what the panel reports about the map is HANDED to it, never re-read here.** The `diag` tab session 247 added read `mapsConfig()` in render, under a comment calling it build-time and unchanging. The three `VITE_` vars are; the call is not — it resolves `mapId`/`colorScheme` through the live `documentTheme()`, while the map latches its config at mount (`screens/Map.tsx`'s `useMemo(…, [])`). The default theme pick is `system` and its `matchMedia` listener fires on Android's own scheduled dark, so a flip needed no user action: the panel would report DARK and the night Map ID over a canvas painting light, and a capture read off it would be describing a map that does not exist. **The rule generalises past this one field — an instrument reads the same latched value the thing under test was constructed from** (`DayConnector` was already doing it, one prop over). Where the live value is genuinely interesting, show it **beside** the latched one and label both, which is what the theme readout now does: a disagreement is then a finding about the canvas instead of a defect in the instrument.
