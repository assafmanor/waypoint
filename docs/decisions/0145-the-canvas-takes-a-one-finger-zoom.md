...
**Status:** Accepted — designed 2026-07-30 (session 193), **corrected on a device the same day (session 194, §4 and §2 amended below).** Phase 9 of the map panel's third pass (#17).
**Date:** 2026-07-30
...
2. **The live reference page**, read by the owner on a phone (`developers.google.com` is blocked by this sandbox's egress policy, so this could not be read from here). `gestureHandling`'s four values are byte-identical to the typings' text, and `"greedy"` is the whole of it: _"All touch gestures and scroll events pan or zoom the map."_ No zoom gesture beyond pinch and double-tap is named. The mobile-gestures announcement lists the same four.
3. **The device.** The owner tried tap-then-press-and-drag on the shipped tab. Nothing happens.
4. **An independent web-access pass**, briefed to _falsify_ this rather than confirm it (2026-07-30). Verdict CONFIRMED against v3.65 weekly, across the interaction guide, `MapOptions`, `MapElement`/`<gmp-map>`, `Map3DElement`, and ~2 years of release notes: no such gesture, no flag, nothing beta or capability-gated. It also found the **cleanest official asymmetry**, which is the citation this ADR wanted — two parallel Google platform pages, where the web page says touchscreen users _"zoom and pan by using two-finger movements"_ and the Android page goes out of its way to enumerate _"one finger zooming by double tapping but not releasing on the second tap,"_ then sliding.

Source 1 cannot by itself settle this — a gesture Google already shipped would need no option to exist, so the absence of a flag is not the absence of a gesture. Source 3 is what closes it, and source 4 is what makes it citable. So this is a build.

**Three precisions from that pass, all of which sharpen a claim this ADR would otherwise have overstated:**

- **Google's web docs _do_ contain the words "double-tap and hold … then drag the map"** — in the accessibility/instructional text of embedded live maps, including on the cooperative sample. It is not a counter-example: it says _navigate_ and _drag the map_, never names zooming, and assigns no vertical direction. But the correct claim is narrower than "the wording is absent", so state it as **Google does not document that sequence as a _zoom_ gesture in the Maps JavaScript API** — anyone who searches this will find that string, and should not think it settles anything.
- **MapTiler did not implement it independently.** Its SDK is a MapLibre extension and its handler docs point at MapLibre's own source. So: **MapLibre implements and exposes the handler; MapTiler ships it through its MapLibre-derived SDK.** One library's decision, not two.
- **The issue-tracker check is a non-finding, not a negative.** No feature request for the gesture and no fixed issue adding it were found — but the tracker's search is sign-in/JS-gated, so that is "not found in the indexed pages", not "does not exist". Recorded at that strength deliberately, since the hoped-for long-open feature request would have been the strongest single artefact and we do not have it.

**And one finding that closes an open question in the brief rather than the reconfirmation:** the pass found **no documented extension point for Google's gesture recogniser at all** — no handler interface, no way to register a custom recogniser, no way to replace one native gesture while keeping the others, and no cancellable low-level touch stream (the documented events are result-level: `drag`, `zoom_changed`, `idle`; and `preventMapHitsAndGesturesFrom` only suppresses, it does not extend). The brief asked whether the arbitration should be owned by "Google's handler, ours, or a capture-phase guard". **The first option does not exist**, which is why §2 is not a preference.
...
**Blocking Google's double-tap means we owe it.** Suppressing Phase B's events kills Google's `dblclick` zoom, which is shipped behaviour. So the handler implements the step-zoom itself, through `useMapCamera`'s existing eased path.

> **AMENDED 2026-07-30 (session 194, device pass) — paying that debt with LOCATE's ladder was a regression, not a shortcut.** Owner: _"a double zoom just zooms in to the center of the map, and it really doesn't matter how zoomed out you are, it zooms in really a lot (~city size, even when you're zoomed out to the whole globe)."_
>
> The first build reused `zoomStepIn(current, MAP_ZOOM.PLACE, MAP_ZOOM.STEP_IN_MAX)`, and that function's first line is `if (current == null || current < floor) return floor`. **That floor is locate's whole point** — "take me to me" must land at a readable zoom — and it is meaningless for a double-tap: from a globe view, `2 < 14`, so the gesture returned **14** outright instead of stepping. Its ceiling was wrong in the same way, capping at locate's `STEP_IN_MAX` a gesture that should reach as deep as a pinch.
>
> **A double-tap means _a bit closer than this_, from wherever this is**, so it is now its own one-line function: `min(current + 1, MAX_DRAG_ZOOM)`. No floor, and the gesture's own ceiling.
>
> **Two things about how this was missed**, both worth more than the fix. The unit test that covered it **asserted the buggy ceiling** — it encoded `STEP_IN_MAX` as the expected value and passed, which is the same shape as session 122's class of miss one layer along: a test written from the implementation rather than from the behaviour. And the reuse looked like rule 8 (extend the mechanism that already does this job) when the two callers only _appeared_ to share a job: **"step the zoom in" and "take me somewhere readable" are different intents that happen to both move the zoom.** Reusing across that seam is how a floor meant for one arrived in the other.
...
> **AMENDED 2026-07-30 (session 194) — the DOUBLE-TAP anchors at the tapped point; the DRAG still anchors at the centre.** The device pass took the deferred decision, and it splits by gesture rather than going one way (owner: _"if we can use things that are given to us by Google, I would prefer them"_).
>
> **Why the split is right rather than a compromise**, which is the whole content of this amendment:
>
> - **The drag keeps the centre**, and this is now an **affirmed** call rather than an un-contradicted one (owner, after using both: _"drag zoom should be anchored to the center as it was"_). The reason §3 gave holds: the finger is _resting on the anchor for the entire gesture_, so a point-anchored drag converges on the one spot you cannot see. **Both anchors have now been felt on a phone, and the split is the answer** — so this is no longer a deferred question in either direction, and `zoomTo` writing the zoom and nothing else is the invariant that keeps it (test: _"a drag zoom is not a pan"_).
> - **The double-tap takes the point**, because after the second tap **you have lifted** — the point is visible, and a discrete "go there" is exactly what you meant. And decisively: **Google's own double-click zoom anchored there**, so centring it was not a design choice at all, it was an unnoticed downgrade that arrived with §2's takeover. A behaviour we suppressed should come back as it was.
>
> **It uses Google's projection, and that is what keeps it clear of ADR-0129 §3's warning.** That warning is against _re-deriving_ Google's projection maths; this asks Google to project. `zoomAboutPoint` works in **world coordinates**, where world-to-screen is a pure power of two, so the only arithmetic here is a scale change — every nonlinear part stays inside `fromLatLngToPoint`/`fromPointToLatLng`. Nothing even constructs a `google.maps.Point`: `fromLatLngToPoint` hands one back, so it is mutated and returned, which keeps the whole path clear of the `google.maps` global.
>
> **It degrades rather than fails.** A map has no projection until it has rendered, so a double-tap before then anchors at the centre and still zooms.
>
> **Tested as the behaviour, not the arithmetic:** the fake map gained Google's own documented Web Mercator, and the assertion is that the geography under the finger does not move. A linear stand-in would have passed while the shipped code was wrong about latitude, which is this feature's entire risk — and the sign is asserted separately, because an invariant that holds symmetrically cannot catch a flipped axis.

### 4. Sensitivity is a share of the canvas, and up means in

> **AMENDED 2026-07-30 (session 194, device pass) — the share model was BACKWARDS, and the false analogy is the part worth keeping.** Owner, on a phone: _"the more space the map takes of the screen, the more the drag feels slow. It feels the least sensitive on map mode, and the most sensitive on half."_ Which is exactly what the arithmetic below predicts and nobody checked: `half`'s pane is ~243px so a level cost ~122px of travel, while the map extreme's ~501px pane cost ~250px. **The more map you gave it, the heavier it got** — the opposite of the intuition that a taller canvas has "more room to drag".
>
> **The reasoning error, because it will be tempting again:** this was derived by analogy from ADR-0123, and the analogy does not hold. **A pin's _size_ is a share of the canvas because a pin is a visual element competing for canvas area.** **A drag's _sensitivity_ belongs to the finger, and a finger does not scale with the canvas** — a comfortable thumb stroke is the same distance whether the map is 243px tall or 501px. "Everything on this surface is a share of the canvas" was one abstraction too far.
>
> So sensitivity is now **`PX_PER_LEVEL` (120): a calibrated absolute distance**, anchored on `half` because that is the stop the owner said felt right (~243px × 0.5 ≈ 122, rounded) — and applied unchanged at every larger canvas, which makes the map extreme **2× more sensitive than it shipped**. `MAX_SHARE` (0.5) survives as a **safety cap for short canvases only**: at 360×640's 160px `half` pane (ADR-0126's measurement) a flat 120 would demand 75% of the canvas per level and be _worse_ than before the fix, so there the cap binds and holds today's behaviour. **It binds below ~240px and nowhere above.**
>
> Tested as a **property**, not a value, since the value is a preference and the property is the bug: no canvas at any size may demand more travel per level than the calibrated stop. A future re-tune cannot reintroduce "more map, slower drag" without failing.

**`PX_PER_LEVEL` (120): a calibrated absolute finger distance per zoom level, capped on short canvases.** See the amendment above for why this is _not_ a share of the pane's height, which is what it was at first and which read backwards on a device.
...
- **Direction.** The design reasoned its way to _up = in_ from a genuinely good argument: this screen already says up means more, because dragging up grows the sheet (ADR-0122 §4). A finger overruled it. **Pulling the map toward you brings it closer** — and the verification pass then found that Google's Android page documents exactly this mapping, _slide up → zoom out, slide down → zoom in_, so the owner's feel-call and Google's own platform agree. Recorded rather than quietly flipped, because the sheet-consistency argument is sound enough that someone will make it again, and because a call made from feel that later turns out to match the reference is worth being able to point at.
...
- **The device pass ran the same day and spent two of its three items** (session 194): `SPAN_SHARE` is gone as a concept (§4's amendment) and the direction was settled on the mockup. What it also produced is two defects the suite had agreed were fine — the sensitivity model and the double-tap ladder — which is the epic's fourth instance of a real device correcting something derived (after ADR-0129, session 139's re-fit guard, and ADR-0121's session-134 camera). **On this surface, "reasoned and unit-tested" has now been wrong four times in a way only a phone caught.**
- **What remains for a phone:** whether the recalibrated 120px reads right at both stops, `TAP_GAP_MS` now that it is 500,. **The anchoring question is spent** (§3's amendment): the double-tap anchors at the tapped point, the drag stays centred, and the split is by gesture for a stated reason.
- **Superseded, kept for the record — the original three items and their line** rather than opening a new one: `SPAN_SHARE` **on a real canvas** (§4 — settled once on the mockup, whose canvas is half the size, so the number is reported rather than calibrated), `TAP_GAP_MS`, and whether centre-anchoring reads correctly on a phone (§3 — the one that could reverse a decision rather than move a number). **The direction is NOT among them:** it was a device-pass item and the owner spent it on the mockup, which is the point of drawing one.
