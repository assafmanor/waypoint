...
> **Still true, and [ADR-0166](0166-place-enrichment-is-a-multi-source-pipe.md) §7 reaches for that template anyway — for a different subject, not against this paragraph.** What this warning protects is the **`Place` row**, and it holds: mutable domain data is not a cache tier. Enrichment **images** are the opposite case — third-party bytes under an immutable content key — so they take the avatar pipeline (`storage.ts` + `image-sniff.ts` + a `@Public` immutable route + `blob-cache.ts` read-through, ADR-0133 §12) as a second consumer. Read this paragraph as "not for rows", which is what it says.
...
- **What's cached on the row:** `googlePlaceId`, `name`, `address`, `lat`, `lng`, `timezone`. Later enrichment (hours, photos — vNext pipe, ADR-0106 OUT) lands as further nullable columns/source keys kept separable; not built here.
  - **Corrected 2026-08-04 by [ADR-0166](0166-place-enrichment-is-a-multi-source-pipe.md) on both halves of that prediction.** It does **not** land on this row: enrichment goes to a **global** `PlaceEnrichment` table with no `tripId`, because it is facts about the entity rather than this trip's view of it, and because **Google Places content may not be cached** — so the storable fields come from open-licensed sources (Wikidata/Wikipedia/Commons/OSM) and no Google-sourced value is written there at all. And it is **not further nullable columns**: every value carries its own `{source, license, attribution, fetchedAt, confidence}`, which as columns would be a migration per field _and_ per source, so it is one zod-validated JSON payload instead. What this bullet got right is the part that mattered — **source keys kept separable** is precisely what that design consumes. The fields listed above stay exactly here, unchanged.
...
> **Reconfirmed + corrected 2026-07-26 by [ADR-0121](0121-embedded-map-phase-6-design.md) §1** (the Phase-6 design session, which re-costed this against live SKUs). **Held:** Dynamic Maps ~$7/1,000 with 10,000/month free, the per-SKU free tiers, and the whole split-key model in Decision 1. **Corrected:** the Routes tiers are named **Essentials / Pro / Enterprise** (not Essentials/Basic → Advanced → Preferred), and **Compute Routes Essentials caps at 10 intermediate waypoints** — so the cheap tier does not cover a day of more than 12 stops. **Added:** the Dynamic Maps SKU bills **per map instantiation**, so ADR-0121 §3 forbids re-creating the map on a view toggle, a filter, or the Map tab's per-second clock tick (one instance per tab visit is accepted, on arithmetic recorded there). A `mapId` is now **mandatory** for advanced markers, which adds a human Phase-0 step (create a Map ID + cloud style) to the prerequisites checklist.
