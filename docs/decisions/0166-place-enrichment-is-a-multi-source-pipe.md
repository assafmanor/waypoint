...
Three field reports from the 2026-08-07 triage close here, and they are one feature: an airport should read `City · IATA` (`תל אביב · TLV`), the codes must be **real** rather than derived from a name, and a flight leg's place search should only offer airports. Research in [session 225](../planning/2026-08-08-session-225-flight-place-data-research.md); build and the decisions taken during it in [session 226](../planning/2026-08-08-session-226-workstream-e-airport-labels.md).
...
> **nickname** (if set) → **`City · IATA`** (if BOTH halves resolved) → **the existing name-stripping fallback**.

Both halves or neither: a lone code names nothing to someone who does not fly often, and a lone city is what the stripping already approximates. The UI is one field on the rename form (ADR-0148's card), offered only where the place already exists, whose hint states what the row will say if it is left empty.
