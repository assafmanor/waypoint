...
> **Amended 2026-08-04 by [0165](0165-a-place-says-what-it-is.md): the limitation above is gone, and the sentence after it was false.** A `Place` now has a `category` column, on this section's own argument for `icon` — a human's word about a place must survive deleting the idea it was written through — with the place's own category outranking the referencing entities' on every place-scoped surface. The ambiguity is answered by the place owning the field rather than by a rule for picking a referencing row.
>
> And "the icon is what persists" did not hold: a pill tap `redrive`s the glyph, which leaves `iconTouched` false, so on a place with no stored glyph — every place that arrived from a search or a drop without a pick — a rename whose only act was a category tap produced an **empty patch and no request**. The owner reported it as a no-op with no error, which is what a silent success looks like. **Only a PICK is stored stays true and is now load-bearing rather than lossy:** the category itself persists and the glyph derives from it.
