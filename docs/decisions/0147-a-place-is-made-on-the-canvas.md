...
>
> **One decision is already taken** (owner, 2026-07-30): the icon gets a real **`Place.icon`** column, accepting the migration, because the icon belongs to the _place_ and must survive deleting the idea it was written through. It sits at the **bottom** of the resolution chain, below the booking's type glyph. **And that has a consequence wider than this feature, recorded here so it is not rediscovered: it disqualifies a cross-trip "global place cache."** A user-chosen icon is trip-scoped data about a place, not a property of the entity Google describes — so `Place` stays a row _inside a trip_ (`@@unique([tripId, googlePlaceId])`) rather than a shared record a trip merely points at.
>
