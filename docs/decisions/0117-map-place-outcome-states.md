...
_Amended 2026-07-28 (session 146, report #21): **the row gained a second, wider quiet state — `behind` — and this one is not an outcome.** §4's treatment was the row's only fade, so the list faded on a **human's** `skipped` while the pin faded on the **clock** (ADR-0121 §6's `behind` tier), and the two halves of the split made different claims about the same place. The row now also carries `.place.behind` whenever `placeBlock` files it under `מה שמאחורינו` — the same `isDayUsagePast` the header and `מה נשאר` read (ADR-0124), so the fade, the header and the filter close a place at the same instant._

_Amended 2026-07-30 (session 185, [ADR-0137](0137-the-pin-says-what-happened.md)): **the canvas now draws the outcome too**, and it draws it as a MARK rather than as a second paint — so `.map-pin.skipped` stays the one behind-you treatment and a pin can say "behind you" and "we skipped it" at the same time. The row keeps both of its classes exactly as described below; what changes is that the pin is no longer silent about which of the three it is._

_The two are deliberately separate classes and not one. Reusing `.place.skipped` would re-apply the fade session 137 took **off** the ambient tier (ADR-0109's 2026-07-27 amendment — the hotel you are sleeping in tonight read as finished), and it would conflate "a human said this did not happen" with "the clock passed it": they coincide often and are different claims, which is exactly what §1's three states are for. So `behind` is lighter than `skipped`, a place carrying both takes the `skipped` treatment alone rather than a second fade, and the vocabulary now reads: **`behind` = where the place sits, `done`/`skipped` = what happened at it.** §1's tags are untouched — a passed-but-unsettled place still shows none, and its fade is the position speaking, not a tag._
...
- **Phase 6 inherits it**: the same outcome drives the rendered pin's treatment when the map lands, with no second derivation. _Amended 2026-07-30 (session 185, [ADR-0137](0137-the-pin-says-what-happened.md)): Phase 6 did **not** inherit it — it shipped one grey `behind` treatment for all three of §1's states, and `map-pane.css` recorded the gap rather than closing it ("every behind-you pin looks the same whatever closed it"). Now built, and the population that needed it most was the one this ADR never considered: the **`ghost`** tier, a place pencilled for another day, which is hollow and therefore has a free centre for the mark. A filled `behind` pin keeps its category glyph and takes the mark as a second badge beside its number. §1's `--ok`/`--miss` hues carry over to the canvas unchanged, which is what makes the two halves of the split finally agree in colour as well as in words._
