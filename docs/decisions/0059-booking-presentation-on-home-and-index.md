...
So the `BookingSheet` route row reshapes: the two endpoints move to a dedicated **route field** (two stacked `PlacePicker`s, origin → destination — phone-first, not a cramped inline pair), and the title row now shows a **read-only `RouteLabel` preview** derived from the picked endpoints (a ghost placeholder until both are empty). This reverses §3's "endpoints are the editable title, not a preview" specifically because the endpoints are no longer typed — you pick them. The stored title is still `origin → destination` (`routeTitle`), so the linked event's title and every route surface (Index row, board hero, detail heading via `eventRoute`/`RouteLabel`) are unchanged. `Booking.fromPlaceId`/`toPlaceId` (ADR-0048) are unchanged — they now point at enriched places instead of coordless lites.

**Seed note:** the PlanHome flight-checklist seed (a free-text destination) now prefills a leg only when a matching trip place already exists by name; otherwise the leg starts empty and the user picks (no place is created just by opening the form).
...
## Amendment (2026-07-24, session 94) — a long route falls back to a stacked origin/destination rail

> **Superseded by the session-95 amendment below.** The rail shipped and still failed: at ~150px of value column even two lines truncated the destination, and the dot/connector/arrowhead markers read as debris. The diagnosis below (both day surfaces rendered the stored title string, not the route) was right and still holds — the _layout_ answer was wrong. Shortening the names is what actually makes a route fit, so the rail was deleted.

§3 says a transport booking reads as its route (`origin → destination`), and the session-86 amendment made the endpoints picked places. Both assumed the route fits on one line. With real Google place names it often doesn't — `נמל התעופה בן גוריון ← נמל התעופה הבינלאומי קפלאוויק` is two full airport names — and each day surface failed differently: **Plan mode** wrapped the combined string into a large multi-line block, and **Trip mode** (after the session-93 title clamp) truncated it, which can hide the **destination** — the more important half, since it's where you're going.

The root cause was that both day surfaces rendered the **stored title string**, not the route. So:

- **Both day timelines now render the route through `EventTitle`/`RouteLabel`** — origin and destination as **separate values**, never one pre-joined string. (The board hero already did.) `titleText` keeps the plain stored title for menu headers and accessible names.
- **Inline while it fits; a stacked rail when it doesn't.** `RouteLabel` gains an opt-in `stack`: short routes render **exactly** as before (no surface that already fit changes), and only a route that would overflow switches to a compact vertical layout — an origin dot, a connector, a destination arrowhead, each endpoint on its own row.
- **Each endpoint clamps to two lines independently**, in its own grid row. So a long origin can never truncate or push out the destination — that's structural, not a tuned max-length. The whole-title clamp from session 93 stands down for a stacked rail (`:has(.route-stack)`), since it would otherwise re-introduce the exact bug.
- **The rail is SVG** (three new `Icon` members: `route-origin`, `route-line`, `route-dest`) — never a text glyph, emoji, or CSS-drawn triangle, for the same reason the rest of `Icon` exists (substituted fallback glyphs sit off-baseline and drift). Every name stays in a `<bdi>` inside the RTL rail, so Hebrew, Latin, and mixed names each keep their own direction. Nothing here is airport-specific — it's any two `Place` names.
- **One shared mechanism, both modes.** The switch is `lib/useStackOnOverflow.ts`: measure the inline row's natural (nowrap) width against the space it has, latch the width it wanted, and compare that against the container on later resizes so the switch is **bidirectional** (rotation, tablet, a card opening) without mounting a hidden duplicate of the text as a ruler. It is the sibling of `useShrinkToFit` (measure-and-observe to shrink a font, rather than to switch a layout); if a third such case appears, generalize the pair instead of adding a fourth.

## Amendment (2026-07-24, session 95) — shorten the names; a destination-primary fallback; the rail is deleted

The session-94 rail treated a too-long route as a layout problem. It isn't: two full official place names (`נמל התעופה בן גוריון`, `נמל התעופה הבינלאומי קפלאוויק`) cannot fit a ~150px value column in _any_ arrangement, so the rail truncated the destination anyway. The fix is to **shorten what we show**, which lets the route stay the inline identity §3 asks for.

- **Place names are shortened for display** (`lib/place-label.ts`'s `shortPlaceLabel`), so the row reads `בן גוריון ← קפלאוויק` — one line, no truncation, and §3's `origin → destination` grammar is preserved rather than replaced.
  - **Google has no short-name field for a POI.** `shortFormattedAddress` is an address, `addressComponents[].shortText` abbreviates address parts only, and there's no IATA. Reading the airport's `locality` doesn't work either — Ben Gurion's is "לוד". So the shortening is ours.
  - It strips **generic category phrasing, never place names** — Hebrew `נמל התעופה (הבינלאומי)`, `שדה התעופה`, `תחנת (ה)רכבת (המרכזית)`, `תחנת (ה)אוטובוסים (המרכזית)`; English trailing `International Airport`, `Airport`, `Railway/Train Station`, `Central Station`, `Station`. The list therefore grows with how many **kinds** of place exist, not how many places: nine patterns cover Keflavík, Narita, Haneda, Charles de Gaulle, Amsterdam Central and Savidor without knowing anything about them. This is explicitly **not** an abbreviation dictionary — `נתב״ג` needs one, which is what a user-set place nickname is for.
  - **It cannot corrupt a name.** Display-only (`Place.name` untouched, so the detail view and tooltips keep the full name, and no migration exists to undo); the strip boundary requires whitespace, so a name that _is_ the category phrase is kept whole; a remainder under two characters or one that is only a modifier (`המרכזית`, `International`) is rejected. The failure mode is "no change" — a script we have no patterns for (`東京駅`) simply displays in full, exactly as today.
- **Fallback, only when even the shortened route overflows:** the row goes **destination-primary** — the title is the destination alone and the meta becomes `מ־<origin>`. The destination keeps the title line because it's where you're going. Nothing is clamped or truncated at any point: an unshortenable name wraps.
- **One decision drives both slots, and it is deterministic.** `ui/route-display.tsx`'s `routeDisplay(route)` returns the title node _and_ the meta line together, so they can never disagree, and both day surfaces call it. The choice is a **pure function of the shortened names** (`ROUTE_INLINE_MAX_CHARS`), not a measurement of the row.
  - This was measured at first (session-94's `useOverflows`, kept briefly), and that was wrong for a **consistency** requirement: the Plan builder row carries a drag grip, ▲/▼ and the ⋯ button, so it has materially less title width than the Trip card and fell back to destination-primary where Trip stayed inline — the same flight reading two different ways in the two modes. Reuse wasn't the problem (both called the same code); the _input_ differed. A shared character threshold takes the same input on both surfaces, so divergence is impossible by construction.
  - It also removes real machinery: no `ResizeObserver`, no latched natural width, no bidirectional re-measure, and no width-stubbing to test. The threshold is tuned for the **narrower** of the two rows, so a route that stays inline fits in both.
- **The meta line stops repeating the origin.** It previously resolved through `bookingPlaceId` → `fromPlaceId`, so a transport row showed its origin twice. Inline it now carries the destination's **full** name (so shortening loses nothing); in the fallback it carries `מ־<origin>`.
...
- the **hard-conflict flag** (`EventCard`, `Board`, `EventForm` — the reported case: a soft event's flag read `חופף ל-נמל התעופה בן גוריון ← נמל התעופה הבינלאומי קפלאוויק` while the flight's own card one row down read `בן גוריון ← קפלאוויק`);
- the **hard-edit / hard-delete confirm gate**, the **⋯ manage-menu header** on both day modes, a **transition row**, and the **change feed**.

Two defects, one cause: the full official names came back, and the arrow was the **text** `←` — the glyph the Assistant body font lacks, so it renders low, exactly what `NavArrow` exists to prevent.

- **A stored title is parsed back into its endpoints and re-rendered as a route.** `lib/route-title.ts` now owns both directions — `routeTitle` (moved here from `booking-edit`, its `arrow` parameter gone) writes it, `parseRouteTitle` reads it, and `ROUTE_TITLE_ARROW` is the single separator both use. Parsing is safe because the separator is ours: it is padded with a space on both sides, and a title without exactly one of them is returned untouched, so a hand-typed title can never be mistaken for a route. Like `shortPlaceLabel`, it fails to "no change", never to a wrong name.
- **`ui/TitleLabel` is the display form**, and it is what a title-only surface renders: a route title becomes a shortened `RouteLabel` (SVG arrow), anything else passes through as-is. `EventTitle`/`BookingTitle`/the two day rows now fall through to it instead of emitting a raw string, so "the booking isn't in reach" degrades to a route rather than to debris. Where the entity IS in reach the id-resolved route still wins (it reflects a renamed place); this is the fallback, not a replacement.
- **`shortRoute`** (`place-label.ts`) replaces the four copies of "shorten both endpoints", so no surface can shorten one half and not the other.
- **Copy that wraps a title is split around it**, rather than interpolating it: `event.conflictWarn` became `{before, after(time)}` and the two `confirm.*Body` strings lost their `title` parameter (the node leads the sentence). A sentence with a node in it is the shape to reuse for the next one.
- **Where only a string can go, it is still shortened.** `shortTitleText` is the text form (short names, textual separator) — the change-feed narration and screen-reader/heading labels. The booking detail's accessible name keeps the **full** names: it is the record.
- **No arrow glyph in copy, enforced.** `i18n/he.ts` lost its `arrows.route` entry, and the lint guard now covers string/template literals in `frontend/src/i18n/**` (plus template literals inside JSX), not just JSX text. The app's one textual arrow is `ROUTE_TITLE_ARROW`, which is data, not copy.
- **Two unrelated raw glyphs went with it:** the TimePicker's "nearest round" hint was a CSS `content: '↩'` (now an `<Icon name="undo">` in the trailing slot the selected row's ✓ uses), and `ICONS.fxUp`/`fxDown` (`▲`/`▼`) were unused entries waiting to be rendered low.
- **`Modal`/`Sheet`/`RowManageSheet` take a `ReactNode` title.** A visible sheet heading that shows an event's title can therefore show its route with the SVG arrow, instead of the primitive forcing a string. `aria-labelledby` still names the dialog by the heading's rendered text.
