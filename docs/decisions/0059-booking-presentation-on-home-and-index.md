...
- **One decision drives both slots.** `ui/useRouteDisplay.tsx` returns the title node _and_ the meta line together, so they can never disagree, and both day surfaces call it. It measures via `lib/useOverflows.ts` (the session-94 hook, renamed for its new job): compare the inline row's natural nowrap width against the space it has, latch the width it wanted, then compare that against the container on later resizes — bidirectional, with no hidden duplicate of the text as a ruler.
