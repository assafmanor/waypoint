...
**Status:** Accepted 2026-09-05 (owner: _"figure everything out, then write up ADRs and a detailed phased plan"_). **Built 2026-09-05**, all five phases. Build plan: [`planning/2026-09-05-a-day-is-a-place-you-can-see-build-plan.md`](../planning/2026-09-05-a-day-is-a-place-you-can-see-build-plan.md). Build note: [`planning/2026-09-05-a-day-is-a-place-you-can-see-build.md`](../planning/2026-09-05-a-day-is-a-place-you-can-see-build.md). Mockup: [`mockups/a-day-is-a-place-you-can-see-v1.html`](../../mockups/a-day-is-a-place-you-can-see-v1.html). Brief: [`planning/2026-09-05-a-day-is-a-place-you-can-see.md`](../planning/2026-09-05-a-day-is-a-place-you-can-see.md).
...
**And the same badge belongs on every row that IS a place** (amended 2026-09-05, owner: _"the bookings screen should also render the same as the day (with place icons etc)"_). `PlaceBadge` has taken a `photoUrl` since ADR-0167 §1; `ListRow` — the app's generic managed-list row — simply never forwarded one, so the booking whose day row showed its photograph showed a ticket glyph in the Index. One booking drawn two ways is a difference about a **fact** between two surfaces, which is what ADR-0159 §1 forbids between the day pair and is no more defensible across this one. The row forwards `photoUrl` and the host resolves it through the same `rowPhoto`, so the picked-icon rule cannot drift between them.

`rowPhoto` now takes the **resolved booking** rather than the whole array, and its event is optional: the third host holds one booking and no array, and a booking with no linked row still has a place and still has a photograph — there is simply no glyph anybody could have chosen on it.
...
**`BookingDetail` is the second host, as this section said it would be** (amended 2026-09-05, owner: _"bookings with place enrichment should also have the place details in the booking"_). It resolves the place the same way its own location fact does — `bookingPlaceId`, so a transport booking reads its origin — and owns the `MediaViewer` the hero opens, exactly as `EventDetail` and `Map.tsx` own theirs. Same place, same enrichment, and the two read surfaces no longer disagree about what is known.
...
### Phase 5 (2026-09-05) — the read knows the place, and a passed row's photo greys

**Both halves are small and one of them corrected this ADR.** `.wp-event.passed .wp-placebadge-photo img { filter: grayscale(1) }` sits beside `.wp-event.passed`'s own 0.66, and the pair is the point: opacity alone leaves a full-contrast photograph inside a faded card, and grayscale alone leaves a sharp grey one. `done` keeps its colour, because a record is not a fade.

`PlaceKnowledge`'s base rules are `ui/domain/place-knowledge.css` now. **The class names stay `map-*`** — they are what the component renders and what the Map's own layout selects through, and renaming them would be a rewrite of every positioning rule for no behaviour. What stayed in `map.css` is `.map-placecard:has(.map-hero) > .place`, the canvas card's grid, which is a fact about a card pinned over a canvas. The second host needed exactly one thing the Map's card was providing for free: a flex row for the block's `flex-basis: 100%` items, which is `.wp-read-know`.

**The read resolves its place through `eventPlaceId`, not through `eventMapPlace`** — the same authority rule minus the coordinate gate. `eventMapPlace` answers `undefined` for a coordless Place-lite because there is nowhere to centre a map, and a place we know nothing about _where_ is still one we may know a great deal about _what_.
...
### Third follow-up round (2026-09-05) — the same badge, and the same read, on the surfaces that were not asked

Two reports, and both are the first round's finding arriving at a host nobody had walked to yet: the badge photograph on the bookings Index, and `PlaceKnowledge` on the booking's read. Neither needed a new rule — §1 and §6 already state both — so both are amendments to those sections rather than decisions of their own.

**What made them invisible is worth naming, because it is not the same thing as the earlier rounds.** Those were a rule asked in a weaker FORM one surface over. These are a rule never asked at all on a surface that has always existed: the Index's row and the booking's read were simply not in the phase plan, and a plan is a list of the surfaces somebody thought of. `ListRow` had carried the capability the whole time — `PlaceBadge` takes `photoUrl`, the row just did not pass it — which is the tell: when a primitive already supports something and one host does not use it, that is worth a look before it is called a decision.

**The count that mattered.** `rowPhoto`'s signature changed to take a resolved booking, and PlanDay was already resolving one on the line above (a `.find()` this deletes) while DayView had one four lines up under the same name. Three hosts, one lookup each, and the array was only ever there because the first two happened to hold one.
