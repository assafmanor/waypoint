...
2. **Tablet (secondary)** — supported and _nice_, especially **Plan mode** (itinerary building, booking entry, research benefit from width). Layouts should scale up gracefully (e.g. wider columns, side-by-side lists) rather than just stretching the phone column.
