...
- **And once it was drawn, it was 3px from the words** (owner, same day: _"no space between the tick and title"_ — measured in the running app, which is where the number came from). A 20px circle centred in `--sec-lead`'s 26px column leaves exactly that. The repair is **not** a wider column: the two leading elements want opposite placement inside the one cell, because a note's 2px rule is a **quote bar** and belongs against its text while a tick is a **control** and must not touch it. So the tick moves to the cell's start (`justify-content: flex-start` on `.tsk-row .note-item-lead`) and the gap is 6px. Note what that protects: §5's invariant is that both TEXTS start at the same x, and it is `--sec-lead` that guarantees it — growing the column would have honoured it too, and spent 6px of every note's width to fix a task's spacing.
