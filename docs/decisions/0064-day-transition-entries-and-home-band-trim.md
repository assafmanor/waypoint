...
- Touches: `lib/glance.ts` (extract the shared derivation), a small new `lib/day-entries.ts` (the `DayEntry` type + merge + tests), `screens/DayView.tsx` (render the merge + a `TransitionRow`, and the strictly-middle strip condition), `screens/Home.tsx` (drop the persistent band, gate the mid-stay treatment inside-span), `i18n/he.ts`, `screens.css`. Frontend + derivation only.
