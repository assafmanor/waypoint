...
**2. The picker is a scrollable 15-minute time list (the fast path) with an exact-time field at its head (the fallback).** Scroll-and-tap covers the ~90% round-time case in one gesture; the list auto-centres on the current value. The exact field handles the off-grid case — a flight at 09:07, a checkout time. List for speed, exact entry for precision, no separate mode to discover. (Rejected: an iOS-style two-column wheel — familiar to iOS users but not the native Android control, and finicky to build well with touch momentum + RTL.)

**2c. The exact field is the platform-native `<input type="time">`.** _(Supersedes the earlier custom free-text entry and its `maskTime`/`parseLoose` auto-colon layer — removed.)_ The native control is the well-known, intuitive mechanism the redesign was reaching for: the `:` separator is **always on screen** (it's chrome, not a character you type), invalid values (hour `76`) are **impossible**, and the numeric keypad drives it. Its `.value` is always canonical 24-hour `HH:MM` per the HTML spec regardless of how it's displayed, so parsing is trivial and no custom validation remains. **Known limitation:** the _display_ format (12h/24h) follows the browser's UI locale, not the page — there is no reliable HTML/CSS lever to force 24h. On the target users' Hebrew-locale phones it renders 24h (`18:57`, no AM/PM); an English-locale device would show `06:57 PM`. Accepted because the users are he-IL; if a guaranteed-24h display is ever needed on any locale, the fallback is a custom segmented mask (prototyped in `mockups/event-time-setter-v4.html`).

**2a. Reopening on an off-grid value _suggests_ the nearest round slot — it never rounds the value.** When the current time isn't on the 15-minute grid (e.g. an event created at 11:47), reopening the list centres on and highlights the nearest round slot (11:45) as a **suggestion** — a lighter treatment than a real selection, with no ✓ — so one tap lands you on a round time. The stored value is untouched until you tap: 11:47 stays 11:47. (This also fixes the reopen-lands-on-00:00 case, where an off-grid value matched no row and the list didn't scroll.) The same nearest-preset suggestion applies to the duration list. `nearestRoundSlot` is a pure, tested helper.

**3. End is entered as a duration off the start, but stored as an absolute end.** The duration menu (`30 דק׳ · שעה · 1:30 שע׳ · שעתיים …`) shows the resulting end time beside each option, and carries the same native "exact end" fallback (which back-computes the duration) so a hard event's committed end is still reachable. The component's public contract is `{ start, end }` as `HH:MM` strings — **duration is a UI affordance only**. `EventForm`'s save path (`zonedIso(date, time, tz)`) is unchanged, so nothing downstream (schema, sync, storage) knows the input model changed.
...
- **New primitive to reuse.** `TimePicker` (with pure, tested helpers `endToDuration` / `clampSameDay` / `nearestRoundSlot`, and a native `<input type="time">` for exact entry) is where any future time entry should go.
