...
**2b. The exact field auto-inserts the `:` — the user types digits only.** Typing `0907` shows `09:07`; `930` shows `9:30`. Digits map right-aligned (last two = minutes), matching `parseLoose`, so the numeric keypad is all you need — no reaching for the colon. `maskTime` is a pure, tested helper.
