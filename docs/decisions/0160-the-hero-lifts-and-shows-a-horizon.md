...
> **SUPERSEDED 2026-08-16 by [ADR-0193](0193-what-is-missing-counts-everything-open-and-the-plan-hero-lifts.md) §4 — and by this section's own last paragraph, not against it.** The "Revisit when" below fired: ADR-0193 §3 collapses Plan Home's far and undated tasks behind one row, at which point the hero summarises things the screen no longer shows inline, and the distance this section correctly found to be absent now exists. **The prep hero lifts, the tap opens the run-up banded against the departure, and the rebuff is retired from this surface** (a press that opens something needs no consolation). The violet-vs-`--on-dark` cost named in the third paragraph below was real and is measured in ADR-0193 §5: `--on-dark-dim` is **2.02:1** on light-mode `--plan-surface`. Its resolution is the owner's — deepen the ground rather than re-ink the type — which costs one surface token and leaves the shipped ramp untouched. Everything else in this section stands as the record of why the answer was no while it was no.
...
- **The arrival DAY, once §R made a red-eye reachable** (built 2026-08-05, and it is why this bullet exists rather than a §S). Owner, on being offered the choice: _"would we want to know that we land tomorrow, or we'd rather know how many hours/minutes to landing?"_ — **both, and they answer different questions.** The countdown is what you act on and it is already on this row; the day is a disambiguator for the one case where the time alone misleads, which is a landing at `06:00` reading as this morning. So the token `מחר` rides **with the arrival time**, never with the countdown (what is ambiguous is `06:00`, not `בעוד 13 שעות`), it appears **only when the landing is not today**, and it is the same token on both elevations. The comparison is well-defined for free: mid-journey the live zone **is** the destination's ([ADR-0107](0107-time-zones-in-a-multi-zone-trip.md) §4), so `today` and the landing's own day are read on one calendar — the same fact that made a "time there" chip redundant here. Measured before shipping, because §O's flex rule applies to this row too: `נחיתה 13:00 מחר · בעוד 13:24 שע׳ · #LH692` is every token that row can hold at once, and it holds one line at 360px and 390px (`e2e/hero-in-transit.spec.ts`).
...
Guarded in `frontend/e2e/hero-lift.spec.ts`, where Plan's rebuff test already lived — the Trip board's sits directly beside it, because one beat on two surfaces is proved by the pair rather than by either alone. Both read `element.getAnimations()` a frame after the click, which is what jsdom cannot do at all: the unit tests assert the class and stay green whether the keyframes exist, are misspelled, or are outranked by another rule. **A second spec file for this was written and deleted** — it duplicated the Plan half with a weaker technique, which is the parallel-copy shape root rule 8 exists to stop.

### R. A journey that crosses midnight is still happening
...
The cause is not in this ADR — it is [ADR-0054](0054-ambient-span-events-off-the-day-schedule.md)'s `ambientWhenMultiDay` being asked to answer two questions, and that ADR's session-215 amendment records the split. What belongs here is the consequence for the hero and the shape of the fix, because **`midSpan` (§Q) turned out to be the field that resolves it**: `isAmbient` says how a span renders across days, `midSpan.kind` says what its middle is, and only the second one decides whether the hero's NOW slot is occupied.
