...
**One thing the fix left standing on purpose.** §M noted that `נוחתים מחר` was deferred for want of a red-eye fixture to test against. There is one now (`hero-booking.test.ts`'s day-boundary block, plus the Home-level pair), so that line is buildable whenever it is wanted — the reason it stayed out was never the derivation.
