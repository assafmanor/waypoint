...
So the aim is watched instead (`lib/land-at-top.ts`, `LANDING_WATCH_MS`). Per frame, for a bounded window: no element yet → keep looking; the scroller is moving → leave it alone, that is our own eased scroll; it stopped and the geometry changed → aim again; it stopped and nothing changed → **ask once more**, because a clamped aim leaves no trace and asking is the only way to find out (and asking when the row is already in place costs nothing — the browser computes the same offset and does not scroll). Two rules are load-bearing: **a `pointerdown`, touch, wheel or key ends the watch** — past the first aim this is a correction, and a correction that overrules the person scrolling is worse than a landing 30px off — and the window is a **measurement**, not a feel call: 2.5s covers the 1.8s late notice plus the eased corrections themselves, and at 700ms the reveal case alone still fails.
