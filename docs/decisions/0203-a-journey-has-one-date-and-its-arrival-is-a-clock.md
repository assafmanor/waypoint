...
**Status:** Accepted (owner sign-off 2026-08-23). **Built 2026-08-23** — §1, §2, §3, §4, §5, §7 and §9 are in the form; §8's search-kind axis and suggestion machinery are in. **Amended 2026-08-23 (§10, and §3 twice) and 2026-08-24 (§6 built, §10's ground, §9's threshold)** after nine field reports on the shipped rail — see [§10](#10--a-clock-offers-forward-from-the-moment-before-it-amendment-2026-08-23) and the second build log; one of the six was a form that could not be completed at all. **§6 is BUILT (2026-08-24)** — see the third build log; the field asked for it with a dimension this ADR did not have (a different NUMBER of stops). **Fixed 2026-09-20** — §2's resolution runs over the whole journey where the legs are READ, not over the one moment being typed; the rail drew a day the leg did not store, and the fifth build log is what that cost. **§8's in-form place suggestion is still deferred** — the build log at the foot says what they turned out to touch, and why bundling them is how this lands half-wired. §3's branch is corrected there: it is `isSpan && titlesFromRoute(type)`, not `isSpan && isTransport`, because a hire is not a journey (ADR-0163).
**Date:** 2026-08-23
...
**Amends (2026-08-23)** [0192](0192-a-form-has-bands-and-its-content-sections-have-one-look.md) §2 — a step's children carry the rhythm the sheet around them has, and the step's identity is **pinned** rather than scrolling away with the content — and [0150](0150-a-form-refuses-at-the-field.md)'s mechanism: a refusal is delivered to the DOM the refusal itself produces, because marking a field can be what puts it on screen.
...
**The `—` in the 0–1 row is load-bearing, and the code missed it for two days** (corrected 2026-08-24). Those counts are inside the fold on both screens with nothing collapsed, so there was never anything for compaction to buy there. The threshold shipped as `nodes.length <= 2`, which is _zero_ stops — so a **one-stop** journey compacted to fix an overflow it did not have, and came back from the field as exactly that: _"the lines collapsing under your fingers could be a little confusing and so maybe do it only when the form is very long, and even then I'm not sure."_ It is `SUMMARISE_FROM_NODES = 3` now: nothing summarises at one stop or fewer, and the two cases this table was built for keep the behaviour.

Also confirmed on a device rather than only against this table, which matters because the rail has grown since it was measured (per-moment captions in §3, the 44px time rows in §10b): the owner's screenshot of a one-stop train journey shows the whole step — dots, type row, rail, zone note, commitment toggle, footer — with no scrolling.
...
## §10 — A clock offers **forward** from the moment before it (amendment, 2026-08-23)

Reported from the field, on the rail this ADR shipped:

> when choosing the time for the arrival for example, it always starts at 00:00 no matter the departure. This makes it inconvenient and it should always start ahead of the departure time, so that it's easier to scroll to the right time. If it crosses midnight maybe even make it circular, wdyt? This holds for other bookings and events as well, so maybe we should add this as a primitive and have it used in all relevant areas of the app.

**This is not a scroll position. It is §2's rule, applied to the offer.** `resolveJourneyDays` resolves every moment to the nearest **forward** instant after the one before it — and the time list opening at 00:00 was the only part of the rail that did not read forward. So the list is **rotated**: it begins one step after the previous moment's clock, runs through midnight, and returns to that clock last. Scrolling down means later, which is what the derivation already means.

**Rotated, never filtered — and `minTime` is why the distinction matters.** That prop already exists and it drops slots, which is right for a _bound_: an end before its own start on the same date is impossible. Here nothing is impossible. An arrival at 00:45 after a 20:30 departure is tomorrow, not an error, so filtering would remove the answer. All 96 slots stay and only the order changes. `minTime`'s own docblock is also the reason the report exists: it is passed only while the end sits on the same calendar day, and "a later day passes nothing" — so precisely the reported case fell back to 00:00.

**What rotation buys, measured rather than asserted.** The row a leg lands on today depends on where midnight happens to fall relative to the departure — a fact about nothing:

| leg length | row today (anchors ⁦20:30⁩ / ⁦06:00⁩ / ⁦23:30⁩) | row rotated             |
| ---------- | ----------------------------------------------- | ----------------------- |
| ⁦1:00⁩     | 2–86                                            | **3**, at every anchor  |
| ⁦4:15⁩     | 3–41                                            | **16**, at every anchor |
| ⁦12:00⁩    | 34–72                                           | **47**, at every anchor |

Rotated, the row is `leg / 15 − 1` and depends only on how long the leg is. Short legs — the majority — are always at the top. **The rotation is not always closer:** for a 20:30 departure a 4h15 leg arrives at 00:45, which the shipped order happens to put at row 3. What changes is that the distance becomes a property of the journey instead of a coincidence.

**The day turns inside the list, and it is NOT drawn at midnight.** A divider row states where the following slots land on a later day, so the relative day is visible _while choosing_ rather than stated afterwards. It cannot be computed locally: Tokyo 21:00 → Honolulu 09:00 is the **same** calendar day, because the flight also crossed nineteen hours westward — the exact case §2 exists for. A midnight divider would announce `למחרת` over a value the derivation reads as today. So `TimeField` takes a `dayOffsetOf(hhmm)` callback and the **host** answers it through the same `resolveJourneyDays` everything else reads. The primitive never computes a day.

**"All relevant areas of the app" has a countable answer, and it is one.** `transportProfile` sets `inMotion: true` for flight, train and transit and for nothing else, and all three also set `titleFrom: 'route'` — which is the other half of `isJourney`. So **every booking type whose span is spent in motion is a journey and renders the rail.** What is left in `WhenField` is a stay and a hire, whose two endpoints are independent times of day, and where anchoring the second on the first would be _wrong_ rather than merely unnecessary: a checkout at 11:00 does not follow from a check-in at 15:00, and anchoring would bury it. The prop lives on the primitive so a future in-motion non-journey span gets it free; the rail is its only caller today by construction, not by omission.

### §10b — a shipped 44px miss on the row this list is made of

The render measured `.tp-list button` at **36.1px** against ADR-0017's 44px floor — on the control a traveller taps most while setting a journey, and shipped long before this ADR. `padding-block: 14.5px` brings it to exactly 44px, and `.tp-list`'s cap grows from 210px to 226px so the same five rows stay visible: a floor paid for by showing less would only move the cost. (14px measured 43.1px — the same quarter-pixel miss `value-token.css` records against its own first number, and the reason every reach here is read off the DOM rather than computed from the padding.)

## §3 — two amendments the field found (2026-08-23)

**A layover's two clocks name themselves.** Reported: _"On layovers you are asked to pick the time of arrival and of departure to the next destination, but the form doesn't explain what each time is"_ — two identical `הוספת שעה` triggers, stacked, with nothing between them. Which is the misread this whole ADR is about, one level down. §3's original plan gave the job to `ConnectionBand` ("it needs no label of its own: the band above it says what the line below it is") and **that plan cannot work**: the band measures the _wait_, so it renders only once both clocks exist. It is absent for exactly as long as the pair is ambiguous. So each moment carries a caption in its own `.wf-line` — glyph included, because `🛬` against `🛫` is the fastest read of the two — and an interior node's heading stops naming one of them, since a heading that names one of two reads as a caption for both. An endpoint keeps its heading: it has one clock and the heading already is its caption.

**A node body is a column with a gap, and an empty segment row is not drawn.** Reported as one thing — _"the line spacing seems off: both for the layover and for the timezone"_ — and it was two. `.jf-body` was a plain block while `WhenField` stacks the same pieces in a flex column with a `gap`, which is where the `ZoneChip` gets its air; here it got **0px**, measured. And separately, the segment row between two nodes was drawn whenever a node had one above it but filled only when the leg could be _measured_, so a half-filled journey reserved an 8px band that reads as a missing line rather than as nothing to say. A segment row now exists only when there is a duration to state; the rail stays unbroken because every row paints its own full-height line, so dropping the one between two nodes just makes them adjacent.
...
## Build log — 2026-08-23 (second pass: six field reports on the shipped rail)

**The blocker, and it is the one worth reading.** _"I'm unable to continue to the next step and complete the form! Can't go past choosing the times!"_, then _"happened after I added a layover"_, then _"Perhaps I had missing fields in the form, but I received no nudge so idk"_. All three sentences are one defect.

A refusal on the rail can land on a node §9 has **summarised**, and a summarised node renders no `Field` at all. `useFormErrors` needs that box for all three of its jobs — the message renders in it, and `report` looks it up in the live DOM to nudge it and scroll to it. So the form refused to advance and said nothing: the exact failure that file's own docblock names ("a refusal the user cannot see is the bug this whole file exists for"). **Only reachable from three nodes up**, because `openIndex` is `null` for a two-node journey and nothing summarises — which is why adding a layover is what surfaced it, and why every spec written for §9 missed it.

Two fixes, because there were two bugs stacked:

1. **A refused node is never summarised.** Read off `mark.error`, not the mark object — `errors.field()` always returns one, carrying the `ref` whether or not anything is wrong.
2. **A refusal is delivered after the commit, not during the call.** Marking a field can be what _puts it on screen_, so `report` reading its node map synchronously read a DOM that did not yet contain the box the refusal had just created. It now stashes the list and an effect delivers it. This is a fix to ADR-0150's mechanism and applies to every form with conditional rendering, not just this rail — pinned by a spec that fails without it.

**The way home was offered the trip's first day.** `tripEdgeFor` was handed `routePoints` on both sides — the outbound's endpoints — so it saw a journey towards the destination twice and suggested `תחילת הטיול` for the return. **Five call sites already spelled out `side === 'out' ? routePoints : reversed` inline before this one read past it**, which is the copy-paste root rule 8 is about; the fix names it once (`pointsFor`) and replaces all six. The count is the finding, not the one-line diff.

**The step's identity is pinned, and it had to become one box.** Reported with two screenshots, before and after scrolling, in which both `מתי` and `טיסה` were gone. The obvious fix — a sticky rule on each — leaves the scroll container's own gap between them, and the content scrolls **through** that gap: **24px of it, measured**. So `FormStepPanel` grows a `header` slot, the type row moves into it, and one sticky box carries both. The pin is asserted by measurement rather than by eye: the identity sits **−150px** from the scroller's top after 150px of scroll today, and **0px** pinned.
...
## Build log — 2026-08-24 (fourth pass: §9's threshold was one stop too low)

> Though I wonder whether the collapsing a layover to a single line is better when there's only one layover and the form is not that long to begin with. I think that the lines collapsing under your fingers could be a little confusing and so maybe do it only when the form is very long, and even then I'm not sure.

**The report is right and this ADR already contained the proof.** §9's fold table marks 0–1 stops as inside the fold on both phones, with dashes for both the all-open and summarised columns — because compaction was never needed there. The code said `nodes.length <= 2`, which is zero stops, so one stop collapsed. A behaviour written to fix a measured overflow was being applied to a case that had none, and compaction is never free: it trades a control for the line it reads as.

**Kept above the threshold, and the numbers are why.** Two stops is 718.5px all-open against a 675px sheet — and against the 512px fold at 360, ADR-0017's _design_ width, it is ~206px over; collapsed it is 25px over. Three stops is 894px against 609px. Deleting compaction outright would restore the 783px overflow §7 measured and got wrong on its first pass.

**The number now has a name**, `SUMMARISE_FROM_NODES` in `constants.ts`, because it is the fold and not the node count — a reader who sees `<= 3` inline learns nothing about why three.

**On the second half of the report — "and even then I'm not sure".** Held rather than acted on. The collapse currently fires in the same frame the time panel closes, so a large layout change is already happening at that instant and the collapse rides along with it; whether that reads as helpful or jumpy at _two_ stops is a device question, and the cheap softening if it still grates is to collapse a node when another is opened rather than the moment it is finished — same end state, movement attached to a tap already being made. Recorded on the backlog rather than guessed at here.

**Two specs moved rather than being weakened.** The summarised-node refusal delivery and the walk-the-rail spec both staged themselves on one stop, which no longer summarises; both now use two, keeping the fact and changing only the depth that reaches it. A third pins the threshold with **both** arms in one spec — one stop quiet, two stops compacting — because a spec asserting only the quiet side would still pass with compaction deleted.

**One measured cost, and CI is what forced it to be measured properly.** With nothing collapsing at one stop more controls stay live between picks, and since §10 each picker open renders the whole rotated 96-slot list — so a spec driving several of them is expensive. Locally that pushed the suite's most expensive spec (six pickers through a real save) past the 5s default; it was given a budget.

**Then CI failed a _different_ spec in the same file, and the lesson is the one root `CLAUDE.md` already writes down.** The summarised-node refusal spec passed locally at ~2.4s and timed out at 5s on CI — hardware roughly twice as slow. Reading the per-spec numbers instead of fixing the reported one showed that **two more** would have failed the next run (2894ms and 2368ms locally ⇒ ~6.0s and ~4.9s there). Counting beat inferring, again.

Treated per spec rather than with a blanket raise: the refusal spec was **trimmed** (it filled two clocks past what staged its case), and the three whose picker opens _are_ the thing being proved got budgets. Four in that file now carry one, which is the honest ceiling — a fifth means making the panel cheaper (it renders 96 rows to show five) rather than raising another number. On the backlog as that, not as a test-tuning chore.

## Build log — 2026-09-20 (fifth pass: the rail resolved the journey and the legs resolved one moment)

> "There's a bug in the flight travel times" — a return flight departing Rome 22:10 and landing in Tel Aviv 02:30, drawn by the rail as `למחרת` with a ⁦3:20⁩ leg, and refused underneath it with `זמן הסיום צריך להיות אחרי ההתחלה`.

**Both halves of that screen were reading the same legs, and only one of them was resolving §2.** `JourneyField` gets `resolveJourneyDays` over **every** moment on every render, so the day it draws is always the forward one. `LegTimes` learned a day only for the moment being typed — `onTimeChange` resolved a probe for that one clock and `withMomentTime` wrote it. So any clock that something FOLLOWS could move past it and leave the moments after it on the day they were first resolved onto: displayed forwards, stored backwards. The reported case is the ordinary one — the arrival typed before the departure it follows (with no departure above it, an arrival resolves to offset 0), or a departure corrected to a later clock afterwards.

**Everything downstream reads the legs, which is the whole point of the adapter — so everything downstream was wrong, and the refusal was the only thing that noticed.** `endBeforeStart` is unreachable on a journey by construction (§2's forward resolution cannot produce a backwards leg), so seeing it fire IS the divergence; had it not been there, `buildSpanSeed` would have written an event whose `endsAt` precedes its `startsAt`.

**Fixed by resolving on READ, not by resolving harder on write** (`withResolvedDays` in `lib/journey-legs.ts`, applied where `resizeLegs` already normalises the leg COUNT against the route). The comment above that call was already the argument: _"normalising on read makes that unrepresentable instead of a bug that only appears when a stop is added mid-edit"_ — the days are the same class of fact as the count, and a write-side fix would only have moved the gap to the next thing that can change under a stored day. A zone can: re-picking an endpoint re-reads a typed clock in a new zone (§8), which flips a forward leg backwards with no clock touched at all.

**It removes a derivation rather than adding one.** `onTimeChange`'s probe is gone — it built the moment list, the zone per moment and the resolution a second time, to answer for one moment what the read now answers for all of them — and `withMomentTime` no longer takes a day: it writes a clock on the journey's date, and where that day lands is one function's business. The explicit-day write stays where it belongs, on `withMomentDayOffset`, which is the override token's own writer.

**What survives unchanged:** a day a human tapped up. Each moment's stored day still enters the resolution as its FLOOR, so `+2 ימים` is kept and only a day the clocks contradict is dropped — §2's rule for overrides, now applied to the stored journey and not just to the drawn one.

**And the trip's own end was the second half of the same report, found by the owner reading the screenshot rather than the code.** With the days corrected the flight home is refused again, now with `התאריך מחוץ לטווח הטיול`: the trip ran to the 25th and the arrival is the 26th. That bound is [ADR-0083](0083-whenfield-datetime-standard.md)'s, written when a span's end was a second **date field** — and §2 turned that field into a derived moment without the bound noticing, so the refusal landed on a day nobody typed and no control can move. It is **amended there**: `tripEnd` bounds an end a person PICKS. A journey's arrival is not bounded by it, because [ADR-0037](0037-overnight-events.md) §1 files a span under the day it STARTS — the booking sits on the 25th and carries the 26th as `endDate`, placing nothing outside the trip. What stays bounded is any moment that DATES a booking: a leg's departure, a layover's included, since a leg filed outside the range is one no day surface can show.

**`EventForm` had been reading it that way the whole time**, in a comment that states the rule — _"an event belongs to a day within [startDate, endDate] — an overnight event on the last day still files under that day (ADR-0037)"_ — and bounds the date alone. So a night out ending at ⁦02:00⁩ on the trip's last day saved, and a flight landing at ⁦02:30⁩ did not: two authoring forms disagreeing about one fact, which is the divergence root `CLAUDE.md` names and the reason the count is the deliverable. ADR-0037 §3 had even named transport as the looser case in advance — _"a flight can end whenever it lands"_.

**Pinned by seven specs**, at three levels: the unit ones (`journey-legs.test.ts`) that a moved departure carries its arrival and every later moment with it, and that a westward crossing still keeps its calendar day; and the form ones (`BookingSheet.test.tsx`) that the journey the rail draws is the journey it saves, through both orders a human can type the two clocks in; and the reported journey end to end — a return leaving ⁦22:10⁩ on the trip's last day and landing ⁦02:30⁩, asserted at the **save**, because what a reader needs is not the absence of a mark but that the journey on screen is the one written. The range pair rewrites the spec that asserted the opposite (`marks only the moment that falls outside the trip`) and keeps its other half: a layover departing outside the trip is still marked, on its own departure and on nothing else.
