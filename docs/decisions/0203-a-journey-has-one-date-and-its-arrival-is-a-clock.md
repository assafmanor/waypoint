...
**Pinned by four specs**, two at each level: the unit ones (`journey-legs.test.ts`) that a moved departure carries its arrival and every later moment with it, and that a westward crossing still keeps its calendar day; and the form ones (`BookingSheet.test.tsx`) that the journey the rail draws is the journey it saves, through both orders a human can type the two clocks in. The form specs fail on the old code at the save, having already asserted `למחרת` on screen — which is the divergence stated as a test.
