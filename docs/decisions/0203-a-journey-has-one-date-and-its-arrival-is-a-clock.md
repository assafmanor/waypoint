...
**One measured cost, stated rather than hidden.** With nothing collapsing at one stop, more controls stay live between picks; the suite's most expensive spec (six pickers through a real save, each panel rendering §10's full rotated 96-slot list) went from ~4.5s to over the 5s default. Given its own 15s budget with the reason written at the call site, rather than thinned — what makes it slow is what it exists to prove.
