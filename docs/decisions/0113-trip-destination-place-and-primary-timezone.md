...
**Status:** Accepted — **fully implemented** across three slices (sessions 82–84, 2026-07-24). **Slice 1:** backend + data model — `Trip` structured destination fields + migration, `@waypoint/shared` mirror, trip-agnostic `/destinations/search`+`/resolve` (geo-type autocomplete + geocode→`geo-tz` zone + multi-zone `candidateZones`), per-user throttled, no persistence. **Slice 2:** the shared `ui/primitives/ZonePicker` (searchable full-IANA sheet, suggested candidates first) replacing trip settings' 5-item `<select>`. **Slice 3:** the creation-flow `DestinationPicker` (+ lighter `useDestinationSearch` hook) — destination is now a Places pick that sets the derived-default primary timezone inline, editable via the `ZonePicker` (multi-zone note from `candidateZones`), with a "use as typed" fallback.
**Date:** 2026-07-23
