...
**Status:** Accepted (building in slices). **Slice 1 shipped (2026-07-24, session 82):** the backend + data model — `Trip` structured destination fields + migration, the `@waypoint/shared` mirror (`tripSchema`/`createTripSchema` + `resolveDestinationSchema`/`destinationResultSchema`), and the trip-agnostic `/destinations/search`+`/resolve` endpoints (geo-type-restricted autocomplete + geocode→`geo-tz` zone + multi-zone `candidateZones`), per-user throttled, no persistence. **Pending:** the shared `ZonePicker` primitive + settings swap (slice 2) and the creation-flow destination picker (slice 3).
**Date:** 2026-07-23
