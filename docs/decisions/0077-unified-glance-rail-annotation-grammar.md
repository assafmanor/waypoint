...
- **Post-build fix (2026-08-03): a skipped booking gets no anchor.** Reported from the phone — two skipped bus legs sat in the day's shelf and still drew amber pills on the glance, reading exactly like the two flights that were really happening. The grammar inherited `bookingTransitionsOnDate` whole and that derivation never read `status`, so a shelved leg was still a commitment. It is filtered in the **shared** derivation (ADR-0064's "the two never diverge" cuts both ways), so the day view's transition rows drop a skipped booking with it. What does **not** change: the skipped event keeps its struck block on the rail and still stretches the window (ADR-0045) — a struck block is the day's texture, an amber pill is a claim about time & commitment (ADR-0028), and only the second one a skip retracts.
