...
- ~~`BEAT.REBUFF` on `.prep` (ADR-0160 §H) is **retired** — the press now opens something, so the rebuff has no condition left on that surface.~~ **Corrected 2026-08-17 (owner, from the shipped screen): the rebuff comes back for the empty case.** The retirement rested on "there is no state now where the card is pressable and empty", and the state exists — a trip whose five checks are satisfied and whose task list is empty renders the hero as the `<div>` it was before, and a tap on it did nothing at all. So the surface is a `<button>` when there is a run-up and a rebuffing `<div>` when there is not, which is exactly the Trip board's shape (ADR-0160 §Q) rather than a second answer beside it. **The beat itself was never at risk**: the board is a claimant either way.
