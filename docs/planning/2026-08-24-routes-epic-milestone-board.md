...
| **M8b** | Mode per leg + trip default | impl   | ✅ **+ 3 field fixes**                          | M8a          | M10          | `routes/m8b-mode` · [#727](https://github.com/assafmanor/waypoint/pull/727) · `routes/m8b-fixes` · `claude/field-report-three-bugs-ytehge`                                                                                                                                                                                                                                                | 2026-08-27 |
...
| **M10** | Offline route pack          | impl   | ✅                                              | M4           | M5–M9        | `routes/m10-offline` · [#730](https://github.com/assafmanor/waypoint/pull/730)                                                                                                                                                                                                                                                                                                            | 2026-08-27 |
| **M11** | Day travel total            | impl   | ✅ **+ 1 owner call · LAST V1 — M13 unblocked** | M6a          | M8, M10      | `claude/routes-m11-total-joqlcq` · [#731](https://github.com/assafmanor/waypoint/pull/731)                                                                                                                                                                                                                                                                                                | 2026-08-27 |
...
**🔵 DRAWN (2026-08-27, inside M8a — [#726](https://github.com/assafmanor/waypoint/pull/726)).**
§AK3's four questions are answered in [ADR-0206 §AL4/§AL5](../decisions/0206-a-travel-time-belongs-between-two-points.md),
and two of §AK's own claims are corrected there: its precedent (the avatar badge) **does not exist in
the code** — the idiom is `PlaceBadge`'s corner mark — and its matrix count is seven rather than
eight, because transit × mark cannot be reached. The `icon={overrunning ? 'warn' : travelMode}`
reversal is **M8b's to code**, along with the rest of the set.
...
### M6a + M8b — the field-report round, and none of the three was where it looked (2026-08-27)

Three reports off one screen at one moment (Georgia, day 27, dark mode, phone), taken ahead of M13
because two of them make the app state a falsehood. **Reproduced against the dev seed on a pinned
clock before anything was changed** — a Tbilisi trip whose stops are all in Israel, `waypoint:dev-now`
at 19:30 — and the reproduction is what makes this round worth a card: **all three reproduced and
none of the three had the cause the report implied.** ADR-0206 §AQ ·
branch `claude/field-report-three-bugs-ytehge`.

- **§AQ1 · M6a — the day advised `יציאה 20:31` into an event that starts at 20:00, and the
  arithmetic was never wrong.** Two causes were proposed from the screenshot and both were
  falsified by the reproduction: `arriveByMs` is not read off `endsAt` (both surfaces read
  `startsAt`, and the seeded row printed a correct `19:31`), and §AJ's clamp cannot be it (a
  flexible arrival states no departure at all). **The owner found it: the clock was printed in the
  trip's zone.** UTC+4 against UTC+3 — one instant, `19:31` where the traveller is and `20:31` where
  the trip is filed. `JourneyRowProps.tz` was one prop serving two questions: its own docblock said
  "the DAY's own zone", both hosts handed it `trip.timezone`, and `JoinRow` was **correctly** using
  the same value for the gap slot's wall-clocks — so it could not be fixed without splitting it.
  Now `legDisplayZones`, the leg's own two zones, on both surfaces.
- **§AQ2 · M8b — the hero used a mode nobody chose.** `derivedTravelMode(bookings)`, the **trip's**
  default, on a surface §AM made per-leg — 53 minutes wrong about a departure. The board reads
  `useDayTravelReads` now and holds no derivation of its own. **`derivedTravelMode` had three
  production consumers and has two**, both of them the fallback role it is for.
- **§AQ3 · the `בדרך` report was a SYMPTOM of §AQ2** — the board was on the walk (`PASSED`, so the
  control) and the day on the drive (`AHEAD`, so none). Both surfaces already had mark _and_ unmark,
  which the running app settled in one pass. **Underneath it was a real gap**: `OVERRUNS` is checked
  before `onWay`, so a leg that does not fit could never offer the mark — on the leg where saying so
  matters most. One predicate, no new widget.
- **§AQ4 · and a fourth defect, left out on purpose.** The day's last bookend leg reads a stay's
  check-in instant as this hole's deadline, so it says nothing at all on a middle night. Nobody
  reported it; it is on the backlog with its reproduction rather than smuggled into this round.

**What the next session should carry.** The first two guesses in this round were both mine and both
specific enough to sound like diagnoses — one of them had the arithmetic land to the minute, on the
wrong cause. The card's own instruction ("the screenshot is evidence, not a diagnosis") is what saved
it, and the thing that actually resolved it was **standing the app up on a fixture shaped like the
reporter's trip**, which took about an hour and settled all three. **The guard for §AQ1 had to be at
the render level**: the invariant a reader would reach for — a departure is never after the arrival
it is counted back from — was true on `main` throughout, because the defect was in the formatting.
...
**✅ Each one met (2026-08-27) in
[`mockups/the-mode-set-and-transit-declared-v1.html`](../../mockups/the-mode-set-and-transit-declared-v1.html)**,
with [ADR-0206 §AL](../decisions/0206-a-travel-time-belongs-between-two-points.md) as the decision
record and the catalog entry in place. `mockups/` + `docs/` only — no app code, so M9/M10/M11 were
never blocked. **The drawing is with the owner; M8b may not start until it is signed off (§M).**

### What the next session needs to know

**Read [ADR-0206 §AL](../decisions/0206-a-travel-time-belongs-between-two-points.md) rather than this
card** — it is eight subsections and it is the build spec. The five things that change what M8b
expected to find:

1. **The three mode glyphs already ship** (`ui/Icon.tsx:238-245`), so M8b mints exactly **one** new
   asset: `transit`, a front-facing bus. §AL2.
2. **`ticket` is NOT the transit mark** — `constants.ts:1520` is `booking: 'ticket'` and four screens
   spend it there. `Icon.tsx:63`'s comment says otherwise and is now wrong; fix it in the same change.
3. **`warn` is the composited infeasibility mark and nothing else.** The declared read carries no
   glyph — the missing duration is the statement and `בלי הערכת זמן` labels it. §AL3.
4. **The mode control is four glyph-only `.wp-chip.touch` chips**, word in `aria-label`, in the
   `.day-trv-modes` slot `.day-trv-acts` was shaped for. §AA4's "four chips fit one row at 360" was
   measured in the wrong box and is corrected in §AL7's table — glyph+word does **not** fit.
5. **§AK2's stated precedent (the avatar badge) does not exist.** Start from
   `.wp-placebadge-mark`/ADR-0167 §1. §AL4 carries the measured geometry, including that
   `.day-trv`'s `overflow: hidden` does **not** clip the overhang (4.2px of slack).
...
### What the next session needs to know (M8b, 2026-08-27)

**Decisions are in ADR-0206 §AM (§AM1–§AM7)** — the key, what was rejected, and the three things the
build found. Read §AM6 before touching the day block: a declared leg has its **own arm**, and the
reason is that suppressing the estimate alone made the block vanish, taking the mode control with it.

- **The override key is `(tripId, fromPlaceId, toPlaceId)`, unordered** — canonicalised (sorted) in
  storage, in the outbox mirror and in the read, so one row serves both directions. The argument is
  §AM1's: תחב״צ is a fact about the journey between two places, so it survives reordering the day.
  §AM2 records what unordered costs and the trigger that would revisit it.
- **`TravelMode` did not widen and will not.** `LegTravelMode = TravelMode | 'transit'` is what a leg
  stores; `isRoutableMode` is the single narrowing at the provider boundary. §AM5 has the three
  `Record<TravelMode, …>` sites and which one gained a word.
- **The switch issues no request, and that is now a spec** (`lib/day-travel.mode.test.ts`) — it
  rerenders through driving → transit → cycling and asserts `fetchRoutes` was called **once**. If a
  later change makes a switch fetch, that spec fails rather than a reviewer noticing.
- **`useDayTravelReads.overrides` is required, deliberately** (§AM7). If you add a third day surface,
  the compiler will make you wire it.
...
**Kind:** implementation (small). **Branch:** `routes/m11-total` — ran as
`claude/routes-m11-total-joqlcq`, the branch the session was handed (M2b's card records the same
substitution). **Conflict surface:** the day header or Plan summary, `he.ts` — landed in the
`day-ambient` strip of **both** day surfaces, plus `lib/day-joins.ts` for the derivation and one new
`ui/domain/` component; see the note below on why the card's "or" became "and". ADR-0206 **§V1.9**,
amended in place — `3.2 ק״מ · ~48 דק׳`, from data M6a already fetched. **Exit criteria:** no new
request; hidden rather than zero when nothing is routable.

**What the next session needs to know:**

- **§V1.9's line was stale before it was built, and the owner corrected it** (2026-08-27): the mode
  name is dropped and the minutes are hedged. The row was written when a trip had one mode; M8b made
  the mode per-leg three days later, so `הליכה` names one leg of a mixed day and is false about the
  rest. The row above is amended in place and **[§AP](../decisions/0206-a-travel-time-belongs-between-two-points.md)**
  is the record.
- **The two halves do not cover the same legs, and that is the whole derivation** (§AP1). A declared
  תחב״צ leg keeps its distance and has no duration (§AA4 / §AM6), so the kilometres count every leg
  and the minutes count only the ones that could be timed. §D5's `~` is what carries "this counts
  what could be counted". A day of nothing but declared legs states its distance alone.
- **`dayTravelTotal` rolls up the JOURNEYS, not the day's legs** (§AP2) — `dayFeasibility`'s
  argument one read over. It takes already-derived values, so "no new request" is structural rather
  than inspected; asserted anyway on both surfaces (one `useDayTravel` stops fingerprint, the legs'
  own stops, no `fetch` during the render).
- **The card's "day header OR Plan summary" is an OR the repo does not allow here** (§AP3). A day's
  total distance is a FACT, and ADR-0159 §1 forbids the two surfaces differing about one — so it is
  one derivation and one component (`ui/domain/DayTravelTotal`) on both. The day header itself was
  measured and rejected: `.sec-title` is already heading + add-button at 360, and a third part is
  §M2's own break.
- **It is the one row in the strip that is not a card** (§AP4), because two amber cards in one strip
  on an overrunning Plan day read as two warnings. Amber survives on the glyph, and the glyph is
  `navigate` rather than a mode mark for the same reason the copy dropped `הליכה`.
- **Measured on the LIVE PAGE at 360×640** (§AP5): ⁦128.1px⁩ of a ⁦328.0px⁩ box, both surfaces, both
  themes, on the widest honest string (`12 ק״מ · ~2:23 שע׳`). `e2e/day-total.spec.ts` is the harness
  and §AN6's three traps are why it is an e2e rather than a mockup number; verified non-vacuous by
  widening the line until it clipped.
...
**That gate is now open, as of M11 (2026-08-27).** M11 was the last V1 ⬜, so **M13 — the leave-by
notification — is unblocked** under the rule above and may be opened by the next session. M12 is not
gated by this rule at all; it depends on `all` and closes the epic. Nothing else in the V2 sketch is
promoted by this: M13 was promoted out of it on its own merits (see its card), and each remaining
line still gets its own ADR when it is taken.
