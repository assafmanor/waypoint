...
4. _"We don't have to gate search with a `חיפוש בגוגל` button. We'll live with the expenses."_ — and then, on the redesign: _"can we work around 'every search costs' … only when there are not enough results, so the user can't see that Google results aren't there?"_
...
**§8b — the exposure comes back through the floor, not by withholding results.** The owner asked whether "every search costs" could be worked back by calling Google only when the trip has no matches (A) or not enough (B), so the absence is invisible. **Both rejected, and upstream of the mechanism: the count of trip matches is not evidence about relevance.** The near miss decides it — you search `בלו בוטל`, the trip has _Kiyosumi_ saved, you want Shibuya; under either gate the app stays quiet and you reasonably conclude there is nothing else. That is a wrong conclusion **caused by** the withholding. A is worse than B for the same reason sharpened: it withholds when the trip has one weak match.

So Google fetches on every pause past the floor, and **the saving is taken from the floor instead**: `PLACE_SEARCH_MIN_CHARS` **2 → 3** (owner's number). Comparable order of saving, **zero** UX cost, no hidden absence, no new mechanism, one constant. The saving was never in "did we ask when the trip already answered" — it is in how many requests a session makes, and the floor removes the ones that were structurally useless. Blast radius worth knowing: three hooks read it (`usePlaceSearch` for the Map tab and the in-form picker, `useDestinationSearch` for the trip destination), all the same relay, so no fork — and `usePlaceSearch.test.ts` fixtures three cases on `setQuery('sh')`, which activates at 2 and goes **inert** at 3.

The demand-gated deferral is **designed, drawn and verified but not shipped**, kept as the named lever with ADR-0108 §6's budget alert as its trigger. Building it earned two notes that only matter if it ships: the reveal **animates**, so "did the list fill" is unanswerable on the frame the query changed (an `IntersectionObserver` is async and re-fires as layout settles, so it is right by construction — a second argument for it over any hand-rolled snapshot); and the one scroll region is `.wp-snapsheet-body`, not the `.map-sheet-scroll` inside it, so a wrong root fails **silently**.

**§8a — the arm is withdrawn.** ADR-0115 §6's "Plan mode only" and §1's arm are both overruled. What remains is most of the machinery: the min-chars floor, the pause-gated debounce, one session token, dedup-before-spend, `PlacesThrottlerGuard`, and the daily quota cap. What goes is the **intent gate** — so past the floor, every search on this tab spends, in both modes. **The cap is the backstop, not the gate, and the ADR says so plainly so the decision is findable later** — with §8b's floor as the thing that takes most of the exposure back.
