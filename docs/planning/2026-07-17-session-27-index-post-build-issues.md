...
**Fix (see ADR-0053):** when the event being edited has a `bookingId`, route the edit to the merged `BookingSheet` (seeded from the booking + its event), not `EventForm`. `EventForm` stays for unlinked manual events. This finishes ADR-0047 §2 rather than inventing anything. The day-view event card otherwise stays itself: its "⋯" keeps Edit (now → merged sheet) / Delete (deletes the Event, booking survives unlinked — ADR-0047 §3) / Swap; it does **not** become the Index detail view. The two "⋯" menus differ only in what "delete" means, mirroring the entity you're looking at — spelled out in **ADR-0053 §4**.
