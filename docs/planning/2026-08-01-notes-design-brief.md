...
**B1 — ~~the create-time fork~~ DECIDED (2026-08-01, owner): the note is written _on the way_, in the host's own form.** No second form, ever. One `textarea` on the host form; a blank one writes nothing; the host's save commits both. Everything sparable is spared — category and glyph are inherited (resolved through `chosenIcon`'s chain, never copied), there is no title field on the host form, and there is no second commit.

**And the objection this brief raised against it was wrong, which is worth keeping rather than deleting.** It argued that one save writing two entities is, offline, two outbox ops with an id dependency. The code says otherwise: host ids are **client-generated** (`crypto.randomUUID()`, `frontend/src/state/trip-state.tsx:930`), so the id exists before the save; the outbox is **FIFO**, stated verbatim above `createPlace` for the Place→Booking case that already depends on it; and the shape is solved twice already, the second time with a reusable pattern ([ADR-0093](../decisions/0093-offline-booking-linked-event-coherence.md): the derived entity is one synthetic `Change` through the same appliers the WS echo uses). See ADR-0152 §6b.
