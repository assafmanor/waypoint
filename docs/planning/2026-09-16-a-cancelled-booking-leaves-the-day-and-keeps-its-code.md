...
Then, with Plan and Trip side by side: _"Plan day got it right, day view has it wrong… the right alignment is wrong."_ Plan's `button.bld-time` was rendered through the same harness and measured beside the Trip card: its box sits on the title's start edge (⁦0⁩), Trip's stuck out ⁦8px⁩ — §1's `- 8px` that kept the digits in place at the border's expense. The `- 8px` is gone; both read ⁦0⁩ now. The glyph-to-chip gap was already Plan's ⁦2px⁩ after the first fix, so it was left alone and said so.
