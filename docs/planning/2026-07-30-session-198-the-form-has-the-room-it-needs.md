...
## 13. Three more from the phone, and one thing they share

- **The icon picker opened off the bottom of the screen.** Its panel is anchored below its trigger, which is right in every other host it has and wrong in a card anchored to the canvas's bottom. The side is measured now and the panel is capped to the room it has there — in the panel, not as a Map-local override, so the four other hosts get it too. And **the first pass floored that cap at a usable 180px, which reproduced the bug**: on 360×640 with a keyboard neither side has 180, so the floor drew the panel taller than the space and cut its own title. There is no floor. Twelve device/keyboard states measured in a browser against the real stylesheets.
- **A long press panned and zoomed.** §3 sent every source through the camera's framing arrival, and a drop names a pixel you are looking at — the exact "inconvenient" ADR-0129 §1 took off a pin tap. The rule already existed; this was the case it had not reached. The intent rides in the value now (`MapArrival = { at, frame }`), and `framePlace` became `arrival` because "frame" means _with a zoom_ here.
