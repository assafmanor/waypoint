...
| The account sheet already reaches every shell       | `AccountSheet` (`App.tsx:524`) is mounted from the in-trip header, `ZeroStateWithAccount` and `AllTripsWithAccount` (`App.tsx:554`, `:567`). "From the trip and outside" is already solved for the **entry** |
...
| …and it hides more of the group than it shows       | `MEMBER_AVATAR_CAP = 2` (`constants.ts:128`). On the ~5-person trip the product is built for, the header renders **2 faces and an inert `+2`**                                                               |
