...
| **5 — the hero slot**       | `design/tasks-phase-5-hero-slot`     | **built** — brief §E designed first ([mockup](../../mockups/a-task-in-the-hero-horizon-v1.html)), then [ADR-0160 §U](../decisions/0160-the-hero-lifts-and-shows-a-horizon.md) · [note](2026-08-16-tasks-phase-5-built.md) |
...
**Built 2026-08-16** ([ADR-0160 §U](../decisions/0160-the-hero-lifts-and-shows-a-horizon.md) · [mockup](../../mockups/a-task-in-the-hero-horizon-v1.html) · [note](2026-08-16-tasks-phase-5-built.md)), in that order — §E was drawn and signed off before a line was written. **Two of this section's own premises were stale and the amendment says so:** §3 has admitted **four** things since §T added an attached document on 2026-08-08 without renumbering it, so a task is the _second_ fourth-thing; and §13's "no note on the **next** event" was **false in the shipped app** — `HeroLift.tsx` has rendered it since commit `9b284e57`, phase 1. §U0 replaces §3's list with a rule (**content is a block, a way OUT of the point is a chip**), which is also why a task is not §T's chip. Three forks went to the owner and all three came back as drawn: the **checkbox** as the mark (with the reads-as-pressable risk scheduled onto the device pass, fallback drawn), the assignee **face** (measured free), and **one** task plus `ועוד N`. Live: **87.1px** of card, **0px** of line-start delta against the note beside it, **0** interactive descendants. The one build call worth knowing is `tasksForContext` — a task about a flight is written on the **booking**, so the hero resolves tasks through the same host context its notes and documents already go through, and the running app is what proved it.
