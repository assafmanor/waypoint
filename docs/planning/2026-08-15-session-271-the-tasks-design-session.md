...
- **The two manage sheets rendered at zero padding, and the owner caught it — not the render.** Worth recording as a miss rather than a finding. §3's menus are `RowManageSheet` → `Sheet` → `Modal variant="sheet"`, which portals to `document.body`; a static page cannot, so they are drawn inline. The first draft drew a bare `.modal-card` with `.modal-head`/`.modal-body` children **that exist in no stylesheet**, and the sheet's `padding: var(--space-4)` is scoped to `.modal-overlay[data-variant='sheet'] .modal-card` — an ancestor that was never drawn. Right class names, wrong nesting, so it looked plausible and was measuring nothing. This is `references/pitfalls.md`'s "real CSS over an invented tree is not the app" and the same shape as `notes-screen-v1.html`'s own `.index` correction; the lesson that generalises is that **inlining the app's CSS proves nothing unless the ancestor chain the rules are scoped to is drawn too**. Fixed to the real tree with only the layer undone, and a probe row now reads the computed padding back (16px) so the next regression is visible rather than plausible.
