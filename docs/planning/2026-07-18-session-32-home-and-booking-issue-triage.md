...
- Nothing on the ADR front — **all of this session's ADRs are now Accepted** (Assaf sign-off 2026-07-18): 0059 (booking presentation; window durations answered = 2h check-in grace / 3h check-out lead / ~45 min arrival emphasis; gate/terminal not shown, not modeled), 0060 (idle-resume), 0061 (plan-home: one new documents/passports check, code-completeness dropped, readiness advisory, completed collapse, per-traveller; **+ refinement** — type-specific create-form CTAs and a round-trip-aware flights check), 0062 (zoom), 0063 (the profile foundation). The 0035/0052/0054 amendments stand. Only the earlier Index post-build batch (0052/0053/0054 core) remains Proposed, unchanged by this session.
