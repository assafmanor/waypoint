...
## A citation that needed correcting, and a motion rule

Writing the facet disclosure, I cited ADR-0100 §3's cover-the-row-in-place search as "the idiom the Index already runs". It does not: **ADR-0101 superseded it**, because "once the on-screen keyboard opens it covers most of the remaining screen, hiding almost every result." Checking the precedent turned the citation from an appeal to authority into the actual argument — a facet strip has no keyboard and no result list, its results being the pins and rows already on screen, so the thing that killed in-place for search is the thing that makes it right here. Worth recording, because "the Index already does this" is the sentence a build session would have trusted.

The owner also asked for the sort chip's appearance and disappearance to be smoother, which turned into one rule rather than one animation: **three objects come and go on this surface** (the chip, the place card, the pre-prompt), and they share a rise-and-fade at `--t-quick` so the canvas does not acquire three motion vocabularies. Two parts of that are decisions rather than polish: the chip is hidden with `visibility` and stays **mounted** (so both directions animate while it still leaves the tab order and the a11y tree — a control you cannot see must not be tappable), and **stop-driven hiding animates while capability-driven absence stays unmounted** (offline the chip is gone, not faded — ADR-0121 §11). Verified: mid-fade at 60ms, un-focusable when settled, and instant under `prefers-reduced-motion`, where the resting states carry the whole meaning.
