...
- **Document upload pick-control redesign** (Med, **Proposed** — ADR-0086, mockup `mockups/document-upload-v1.html`) — replace the raw native `<input type="file">` in `DocumentUploadSheet.tsx` (the LTR "Choose File" eyesore) with a designed drop-zone that states the accept+size contract up front, a first-class camera-capture path (`accept=image/* capture=environment`, phone-first), and a picked-file preview card (image objectURL / PDF tile + name + type·size + ✕). Frontend + `he.ts` copy only; no backend/shared/data-model change, and ADR-0052's spinner/progress/error/badges are reused unchanged. Follow-up (deferred): auto-crop/enhance a captured passport scan.
