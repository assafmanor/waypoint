...
The Map is the last unbuilt core tab. [ADR-0106](decisions/0106-maps-and-places-epic-scope-and-phasing.md) is the frame: one mode-re-emphasized tab, picker-first, list-of-pins before an embedded map, filters that are pure client-side derivation. Design / FE-arch / BE-arch sessions come before implementation. **BE-arch is decided** ([ADR-0108](decisions/0108-maps-and-places-backend-architecture-key-model-and-cost.md)): a two-key split (public browser key for the Maps JS API only; a server key for Places New + Routes behind a thin backend proxy on the existing `places` module), `geo-tz` server-side for `Place.timezone`, the `Place` row as the cache with `@@unique([tripId, googlePlaceId])`. FE-arch still open. Phase order below.

- **Google Cloud project setup** (human, Phase 0) — OAuth consent, Maps/Places + Routes API, billing, and **two restricted keys** (ADR-0108): a browser key (`VITE_GOOGLE_MAPS_BROWSER_KEY`, Maps JS API only, referrer-locked) and a server key (`GOOGLE_MAPS_SERVER_KEY`, Places New + Routes). Re-confirm current pricing when enabling billing. Gates the whole epic (and calendar sync). See also Integrations below.
- **Places picker** (Phase 1, keystone) — a Google Places Autocomplete (session tokens) **through the backend proxy** (ADR-0108: proxy routes on the existing `places` module, server key, dedup-before-spend on `(tripId, googlePlaceId)`) that creates/links + enriches a `Place` (`googlePlaceId` + coords + address, deduped by `googlePlaceId`), wired into every place field (event location, booking location, transport origin/destination, maybe-item). **Also resolves `Place.timezone` server-side via `geo-tz`** (ADR-0107/0108). Schema additions: `Place.timezone String?` + `@@unique([tripId, googlePlaceId])`, mirrored in `@waypoint/shared`. Re-adds EventForm place authoring (the free-text input ADR-0051 removed); name-only `Place` stays the offline fallback.
