...
Triaged from Assaf's on-the-ground review. Each item is an independently deployable task; the session note maps file ownership so several can run in parallel. All the ADRs below are **Accepted** (mockups signed off 2026-07-18) — build-ready.
...
- **Booking presentation across Home & Index (ADR-0059, Accepted; applies ADR-0063)** — mockup `mockups/booking-presentation-v1.html`. The board hero renders a **bracketed** event by its profile (`isBracketed`) at its **transition moments** (check-in with a 2h grace / check-out 3h lead; departure/arrival, arrival emphasized ~45 min) instead of across its whole span; a distinct **teal "inside a booking now"** treatment (slim dismissible strip for an ambient hotel mid-stay; the hero's NOW slot for a flight in transit); and a **shared appearance grammar** applied to the hero, the Index row (`BookingLi`), and the read-only detail view (`BookingDetail`), all reading the profile. Gate/terminal not shown (not modeled). Frontend + derivation only; subsumes the old "board hero booking presentation" item.
...
- **Plan-mode Home "what's missing" rework (ADR-0061, Accepted — build-ready)** — mockup `mockups/plan-home-readiness-v1.html`. Build: `readiness.ts` (keep the four checks; make **flights round-trip aware** — complete only with an outbound leg to the destination + a return leg from it, via the flight bookings' origin/destination Places, ADR-0048/0051; add one new per-traveller **documents/passports** check reading the snapshot docs list, ADR-0058; + tests), `PlanHome.tsx` (**each CTA opens the type-specific create form** — lodging→create-lodging, flights→create-flight seeded with the missing direction; empty-day→day builder on the first empty day; group→settings invite; collapse completed checks into a summary with a show-completed toggle; readiness stays advisory), `he.ts` copy. Code-completeness check dropped (too minor); Google/Gmail/WhatsApp stay out.
