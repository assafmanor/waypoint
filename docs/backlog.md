...
- **Loading states as one language (ADR-0105, mockup `mockups/loading-states-v1.html`)** — extends U-10's feedback family across the three loading moments. Replace the bare `shell.booting` word with the light **boot** "board power-on" (cool-paper `--screen` day ground, ink clock + a small amber halo, a warm-up glow ramp — not the reserved live pulse — and a sweeping departure-line); upgrade the **snapshot** `LoadingState` from an anonymous shimmer to a **content-shaped `Skeleton`** that pre-draws Home, per mode (Trip dark-board / Plan violet-prep + checklist). The **dark** boot is **designed-not-built** — it rides the `data-theme='dark'` toggle from U-08, inert until then. The pending **upload row** shows a determinate `NN%` + mini-bar while uploading (an upload has a known size), replacing the indeterminate `מעלה…` spinner and needing upload-progress wiring (rides ADR-0056's outbox upload); offline stays the static `ממתין להעלאה`. Amber stays a time accent throughout (a full amber ground was tried and rejected as off-budget, ADR-0028).
