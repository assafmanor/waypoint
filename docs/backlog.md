...
- **U-04 per-item save/sync state** (High) — _Landed_ (ADR-0080): the `SyncStatus` model (`synced/pending/failed(reason)`) derived frontend-only from the outbox (id-keyed) + the F-03 failed store, `useSyncStatus(entityId)` + a non-color-coded `SyncBadge`, and a persistent review/retry (dead-letter) sheet replacing the timed failed-badge; DocumentsSection is the reference wiring. _Remaining:_ migrate booking + event rows onto `<SyncBadge>` (Wave 3).
