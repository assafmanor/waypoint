...
- **U-04 per-item save/sync state** (High) — _Landed_ (ADR-0080): the `SyncStatus` model (`synced/pending/failed(reason)`) derived frontend-only from the outbox (id-keyed) + the F-03 failed store, `useSyncStatus(entityId)` + a non-color-coded `SyncBadge`, and a persistent review/retry (dead-letter) sheet replacing the timed failed-badge; DocumentsSection is the reference wiring. _Remaining:_ migrate event rows onto `<SyncBadge>` (Wave 3; booking rows landed via D-index).
...
- **U-03 domain components** (Med) — the `ui/domain/` layer **landed** (Wave 3 / D0): `ListRow`+`RowManageSheet`, `MaybeCard` (real `meta` prop, no `maybeMeta` fixture — U-07), `Board`, `EventCard`, `DayStrip`, `GlanceCard`, `StatTile` — all presentational (data via props, no `state/*`), new `wp-*` classes with co-located CSS ported faithfully from `screens.css`, one `*.test.tsx` each. _Remaining:_ migrate the screens to consume them and delete the inline copies + the superseded `screens.css` rules — DayView/PlanDay (`MaybeCard` ×2, `EventItem` → EventCard), Home (`.board`/`.glance-day` → Board/GlanceCard), App.tsx header (`.day-strip` → DayStrip), PlanHome (`.prep-stat` → StatTile). Index/DocumentsSection landed via D-index (`.li.bk`/`.li.doc` → ListRow, both manage menus → RowManageSheet, per-row `<SyncBadge>` on the booking row; the shared `.row-action`/`.kebab` rules stay — still used by DayView/PlanDay/TripSettings).
