...
- **Booking preview duration** (separate PR after the ADR-0083 date/time work merges) — the booking **detail/open preview** and the **Index list** rows show only the transition time(s), not how long the booking lasts. Add a derived, per-category duration read-out (reusing the ADR-0063 category time-behaviour profile): flight/train → hours (e.g. "5 שע׳ 45 דק׳"), hotel → nights ("3 לילות"), same-day activity → hours, multi-day activity → days. Detail preview shows start · end · duration; each Index row shows its time(s) + duration. Touches `BookingDetail`, the Index booking row (`lib/index-bookings.ts`), and a shared duration formatter in `lib/booking-timing.ts` (nights helper + reuse `formatCountdown`). Relates ADR-0059/0063.
