...
- **Plan-mode Home "what's missing" rework (ADR-0061, mockup built, awaiting sign-off)** — mockup `mockups/plan-home-readiness-v1.html` (session 32) proposes the reworked check set (four existing with real CTAs + 🛂 documents/passports and 🔑 code-completeness; completed checks collapsed) and answers the open questions. On sign-off: `readiness.ts` (new pure checks + tests) + `PlanHome.tsx` (row behavior/CTAs) + `he.ts`. Confirm the ADR's proposals with Assaf first.
