...
- **Per-day transition entries (ADR-0064 §B/§C)** — a multi-day bracketed booking (`isBracketed && isMultiDay`) contributes discrete day-timeline entries: a start-edge row (check-in / departure) on its start day, an end-edge row (check-out / arrival) on its end day; nothing on middle days. Same-day brackets stay a single span row. Lift the inline per-day start/end logic in `glance.ts` into one pure `bookingTransitionsOnDate(events, date)` (feeds both the Home markers and the day entries); a new `lib/day-entries.ts` with a typed `DayEntry` union (`event | transition`, **no synthetic `TripEvent`s**) + merge + tests; `DayView.tsx` renders a read-only `TransitionRow` (→ booking detail, ADR-0053; Navigate on the start edge only) merged into the timeline by instant. Backdrop strip → strictly-middle nights (`date < activeDate < endDate`), removing the wrong checkout-day strip. Derivation-only; no schema/backend change.
