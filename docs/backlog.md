...
- **U-08 tail: migrate raw px + ship dark mode** (Med, foundation) — the non-color tokens now exist in `tokens.css` (`--space-*`/`--text-*`/`--leading-*`/`--radius-*`/`--elevation-*`/`--status-*`/`--bp-*`/safe-area; ADR-0077). Remaining: migrate the ~6.5k lines of hard-coded px in `App.css`/`screens.css` onto them (opportunistically as screens are touched, then a final sweep), verify the dark remap, and add a CI lint budget discouraging raw px in `ui/` component CSS.
