...
The Map is the last unbuilt core tab. [ADR-0106](decisions/0106-maps-and-places-epic-scope-and-phasing.md) is the frame: one mode-re-emphasized tab, picker-first, list-of-pins before an embedded map, filters that are pure client-side derivation. Design / FE-arch / BE-arch sessions come before implementation. The **design session is done** — [ADR-0108](decisions/0108-map-tab-design.md) + `mockups/map-tab-v1.html` fix the Map surface's look/behaviour (list-first tab, Index-grammar filters, the Waypoint pin, geolocation UX, near-me, ADR-0107's zone chip) and resolve ADR-0106's open design questions; FE-arch + BE-arch are still open. Phase order below.
...
- **Map tab v1 — list + filters** (Phase 3, design in [ADR-0108](decisions/0108-map-tab-design.md)) — the `Place` registry as a list; a derived place-usage index driving day/type/maybes filters (reusing the Index chip grammar); Trip mode defaults to today's places, Plan to all; the Waypoint pin (teal body + category glyph + amber-core commitment).
- **Trip-mode live jobs** (Phase 4, design in [ADR-0108](decisions/0108-map-tab-design.md)) — device geolocation permission (just-in-time, degrades if denied) → "near me now" as a list re-sort + distance chips; **navigate-to-next** (deferred out of ADR-0045; deep-link to the transport origin `Place`) as the returning fourth Home tile + on the Map.
