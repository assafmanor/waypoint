...
- **Booking presentation across Home & Index (ADR-0059, design pass first)** — mockup `mockups/booking-presentation-v1.html`, then: the board hero surfaces a booking at its **transition moments** (hotel around check-in/out with a short post-check-in grace, transport at departure/arrival) instead of across its whole span; a distinct **"you're inside a booking now"** treatment (at the hotel / in transit) replacing the generic chip; and a **shared appearance grammar** applied to the hero, the Index row (`BookingLi`), and the read-only detail view (`BookingDetail`). Frontend + derivation only; subsumes the old "board hero booking presentation" item.
- **Glance check-in/out markers (ADR-0054 amendment)** — fold into the ADR-0054 build: `glance.ts` emits **uncounted point markers** for ambient check-in/check-out at their clock position; `remaining`/`buildTimeTree` untouched. (Same files as the ADR-0054 task — do them together.)
...
- **Plan-mode Home "what's missing" rework (ADR-0061, design pass first)** — re-verify the four existing checks' CTA behavior against the now-real Index/Day screens; reconsider the suggestion set (a **documents/passport** check is now buildable post-ADR-0058). Settle the final check set + copy in a mockup, then `readiness.ts` + `PlanHome.tsx`. Answer the ADR's open questions with Assaf first.
