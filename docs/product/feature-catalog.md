...
| Glance cards (weather / FX)             | Could    | v1.1  | **FX half built** ([ADR-0180](../decisions/0180-currency-is-derived-and-a-rate-is-a-glance-card.md)) — the `מבט מהיר` section is back, carrying `RateCard`; **weather is the remaining tenant**. Budget dropped (ADR-0045/0004) |
...
| Feature                        | Priority    | Phase | Notes                                                                                                                                                                                                                                                                                     |
| ------------------------------ | ----------- | ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Currency rate display          | Should      | v1    | **Built** 2026-08-09 ([ADR-0180](../decisions/0180-currency-is-derived-and-a-rate-is-a-glance-card.md)): the trip's currency derived from the destination country, a `User.preferredCurrency` for the home side, a `RateCard` in the restored `מבט מהיר` section, and the converter sheet |
| Weather                        | Should      | v1    | **Not built.** The `מבט מהיר` section's reserved second tenant (ADR-0045 §4); `FxService` is the shape to copy. Tracked in [backlog.md](../backlog.md)                                                                                                                                    |
| Emergency numbers by country   | Should      | v1    | **Not built, and nothing in the tree references it.** A static table on `destinationCountryCode`, like `COUNTRY_CURRENCY`. Tracked in [backlog.md](../backlog.md)                                                                                                                         |
