...
- **Quick-access grid** — 4 icon buttons. Per [ADR-0045](../decisions/0045-trip-home-real-data-only.md) these are **real shortcuts into data/surfaces we have**, not concierge fixtures: next confirmation code (→ index), WiFi copy, documents (→ index), and navigate-to-next (a Maps deep-link, shown only when the next/now event has a usable place). The original "nearby ATM" is gone (needs live location — ADR-0006).
- **Day-at-a-glance card** — the Trip-Home glance, **derived 100% from `events`** (ADR-0045): a segmented events bar (done · now · upcoming) plus done/remaining/hard-anchors-left and the free-until / end-of-day line. Offline-safe, no fixtures. It **replaces** the old weather / FX / today's-budget glance row: those were fixtures for unbuilt pipes (budget deferred entirely — ADR-0014 amendment). Weather/FX return as their own glance cards when the integration pipes land (ADR-0004).
