...
- **The lint guard covers control emoji in rendered JSX**, non-test source only. If you add one, note the trap ADR-0138 §9 records: these selector regexes have no `u` flag, so a character class of astral emoji is a class of surrogate halves and matches the whole plane. Use an alternation.
