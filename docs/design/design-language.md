...
A small **decorative palette** (avatar identity colors, **map pin categories**) exists alongside these — always pastel/muted, never amber or teal, so it reads as gentle variety rather than a second meaning system. See the Map pins entry below (the 5 category hues; ADR-0038/0108).
