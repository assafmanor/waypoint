...
A small **decorative palette** (avatar identity colors) exists alongside these — always pastel/muted, never amber or teal, so it reads as gentle variety rather than a second meaning system. (Map pins are **not** part of this palette: they spend teal + amber inside the budget, carrying category by glyph, not a decorative fill — see the Map pins entry below, ADR-0108.)
...
- **Map** (redesigned in ADR-0108, superseding the `trip-dashboard-v2.html` sketch) — a **quiet neutral base, loud semantic pins**: a desaturated cool-paper canvas (never a teal flood), with the **Waypoint marker** (ADR-0087) as the pin — **teal body = location, the glyph inside = category** (content, ADR-0038, _not_ a per-category fill), and an **amber core = commitment / time-anchor** (lit for a hard scheduled place, dim for soft, absent for a maybe-only idea or a mid-stay ambient base; colour-by-most-committed). Coordless "Place-lite" places list as a hollow ring (not navigable until the picker enriches them). Day scope uses the **existing shared header day strip** (`ui/domain/DayStrip`): the map is a day-scoped surface like the Day view, so tapping a day focuses it **in place** (the strip's existing update-in-place rule; "all days" is a map-local scope chip, since the app tracks one active date). The filter chips (**type · maybes**) **reuse the Index chip/search/mode-accent grammar** (ADR-0098/0100), not a second copy. "Near me now" (Phase 4) is a list re-sort + teal distance chips, not a spatial dot, until the embedded map (Phase 6) lands. The blue "me" dot stays an OS-map convention outside the amber/teal system. Offline state: desaturated backdrop + "last saved locations" banner + "distance unavailable" labels on the list.
