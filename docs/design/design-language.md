...
**True everywhere since [ADR-0137](../decisions/0137-the-row-menu-is-one-surface-and-icons-are-ui.md) (2026-07-29), not just for arrows.** For years this rule was enforced only for arrow/caret glyphs and aspirational for the rest, so ✏️ 🗑️ 📥 🔄 👑 🚪 ⬆️ 📷 🔗 all drew controls. Three things changed:
...
- **The lint guard covers control emoji in rendered JSX**, non-test source only. If you add one, note the trap ADR-0137 §9 records: these selector regexes have no `u` flag, so a character class of astral emoji is a class of surrogate halves and matches the whole plane. Use an alternation.
