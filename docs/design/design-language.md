...
- **Day-at-a-glance card** — the Trip-Home glance, **derived 100% from `events`** (ADR-0045): a segmented **block** bar + a lead **"נותרו"** count + the next hard anchor + a free-until / end-of-day line. Offline-safe, no fixtures. Two rules make it honest: counts are **phase-derived** (`eventPhase` — a passed-but-unmarked event drops out of "נותרו" automatically, never misfiled as remaining), and the bar/count are on **top-level containment-forest blocks, not raw events** (ADR-0041) — a nesting envelope or an overlap cluster is one block, so the day never looks busier than it is. **Empty day** = a calm teach state ("היום עוד פתוח"), never a hidden card or a 0/0 bar; no amber. It **replaces** the old weather / FX / today's-budget glance row (fixtures for unbuilt pipes; budget deferred entirely — ADR-0014 amendment). Weather/FX return as their own glance cards when the pipes land (ADR-0004).
