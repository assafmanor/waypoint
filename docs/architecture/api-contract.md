...
| PATCH  | `/trips/:tripId/notes/:noteId`                    | `updateNoteSchema` (`{ title?, body?, url?, category? }`) → `Note` — a **whole-content submit**, so an absent field clears. The host is deliberately **not** editable: attachment is established from the host's side and v1 has no host picker (ADR-0153 §5)                                                                                |
