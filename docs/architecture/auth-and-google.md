...
- **The callback can arrive twice for one code, and the second arrival is not an error** (owner report, 2026-08-06 — a 500 after logging in from Chrome with the PWA installed). An installed PWA **captures in-scope navigations** on Android, so Google's redirect to `/auth/google/callback` is handled by the browser _and_ handed to the app window. The WebAPK shares the browser's cookie jar, so both requests find the same `wp_oauth`, both pass the `state` check, and both redeem the code — which is single-use, so the loser gets Google's `invalid_grant`. That used to escape `AuthController.googleCallback` as an unhandled 500, and since the app window is what the person is looking at, **a login that had succeeded rendered as a crash**. The winner has already set the refresh cookie on that host in the shared jar, so the loser now redirects home like every other unusable callback and lands signed in. The Google reason is still logged in full: a code refused for any _other_ cause looks identical at this seam and must stay diagnosable. Note the diagnostic tell — `prompt=consent` in the failing callback's URL means the **first** exchange had already succeeded, which is what rules out a `redirect_uri`/credential misconfiguration.
