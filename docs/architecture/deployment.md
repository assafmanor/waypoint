...
**Live map range cache (ADR-0187).** `GET /map/planet-<build>.pmtiles` range-reads the configured planet archive through a bounded in-memory LRU (`MAP_PLANET_CACHE_MAX_BYTES`, default 128 MB) and an optional local-FS tier (`MAP_PLANET_CACHE_DIR`). The route is guarded, accepts closed byte ranges only, validates exact upstream partial responses, and rejects a build id that does not match `MAP_TILES_SOURCE_URL`. Both tiers are disposable caches; an empty ephemeral filesystem only causes the ranges to warm again.
