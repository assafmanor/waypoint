...
**Never set in production:** `DEV_AUTH` (auth bypass). `VITE_API_BASE_URL` stays unset — the client defaults to same-origin. Later additions when their features land: `REDIS_URL` (v1.1) and, with the Phase-6 embedded map, three **build-time** frontend args — `VITE_GOOGLE_MAPS_BROWSER_KEY` (the public browser key, restricted to the Maps JavaScript API and referrer-locked to this origin — ADR-0108 §1), `VITE_GOOGLE_MAPS_MAP_ID` (the cloud-styled Map ID; **mandatory**, since advanced markers do not load without one — ADR-0121 §1/R3), and `VITE_GOOGLE_MAPS_MAP_ID_DARK` (the night style, inert until dark mode ships — ADR-0121 §9). All three are build args, not runtime env: they are baked into the client bundle. Absent, the Map tab degrades to its list-only form rather than failing (ADR-0121 R3). _(This line previously named the key `VITE_GOOGLE_MAPS_API_KEY`, which contradicted ADR-0108 and `.env.example`; ADR-0121 R3 settled the single name used above.)_
