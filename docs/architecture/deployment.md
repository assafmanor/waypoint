...
**A kill switch is turned OFF by deleting the variable, never by setting it to `0` or `false`** — measured, not inferred. `PUSH_DISABLED`, `ENRICHMENT_DISABLED` and `DOC_CACHE_DISABLED` are read as a bare truthiness check on the string, and in JavaScript the strings `"0"` and `"false"` are **truthy** — so `PUSH_DISABLED=false` **stops every push**, which is the exact opposite of what it looks like it says. Only an absent or empty value re-enables. (`FX_DISABLED` is the odd one out: it tests `=== '1'`, so `FX_DISABLED=false` leaves FX running. Four switches, two behaviours — worth collapsing, and not yet done.)
