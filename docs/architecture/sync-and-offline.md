...
- **A rebuilt SW takes over immediately, and says so** (ADR-0181). `skipWaiting`/`clientsClaim` are deliberately on: without them a new SW waits until every tab of the old one closes, and an offline reload in between would still run stale JS. The cost lands on the tab that is already open — its running JS holds the **old** build's chunk hashes while the new SW has already replaced the precache, so a dynamic import of a not-yet-loaded route can miss. `lib/useAppUpdate.ts` is the compensating control: it registers through `virtual:pwa-register/react` and raises an app-wide `StatusBanner` offering a reload. Its `onNeedReload` callback is also what **suppresses the plugin's default, a silent `window.location.reload()` of the live tab** — see the ADR before changing either the callback or `registerType`, both of which behave differently from what the plugin's docs suggest. An open tab re-checks `sw.js` every `SW_UPDATE_CHECK_MS`, skipped while offline.
