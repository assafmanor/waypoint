...
**Status:** ACCEPTED (core choices ratified 2026-07-09). Optimized for: a self-owned traditional backend, **TypeScript end-to-end** (shared types), small groups per trip, real collaboration, offline reads; not yet scaled but Postgres-clean so it can grow to many trips and users later (ADR-0065).
...
| **Lint**             | **ESLint** (flat config, typescript-eslint) + Prettier                                                                                         | Root `eslint.config.mjs`, wired into each package's `lint` script (T-028)                                                                                                                                                              |
| **Auth**             | **Google OAuth** (Passport / Auth.js) → own JWT; **Google-only**                                                                               | Everyone has Google; unlocks Maps/Calendar/Gmail scopes (ADR-0013)                                                                                                                                                                     |
| **Realtime**         | **WebSockets** (NestJS gateway / `ws`) + in-process per-trip channels                                                                          | Simple fan-out per trip (~5 users); `LISTEN/NOTIFY` when we scale out                                                                                                                                                                  |
...
| **Hosting**          | **Railway**, one project: single Dockerfile service (PWA + API + WS, one origin) + Railway Postgres; GitHub auto-deploy from `main` (ADR-0031) | Cheap/simple for a private tool; single-origin per ADR-0020; runbook in [deployment.md](deployment.md)                                                                                                                                 |
...
- **zod 4**, **Vitest** for tests, **ESLint flat config** wired (T-028).
