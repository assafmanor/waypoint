// **How an enrichment value reads** — the display answer for what the pipe delivers (ADR-0167).
//
// One of them now: the summary, which needs a language chosen and marked (§5). Its old
// companion, `placeCredit`, moved to `@waypoint/shared` (ADR-0219 §6) — the reader composed a
// second one server-side and the two disagreed about which end the photographer went, so the
// line is composed once, where both sides can reach it.
//
