...
import { LOCAL_READ_PHASE, LOCAL_READ_TIMEOUT_MS } from '../constants';
...
/** Best-effort in time, not only in errors — the one thing every entry point here shares,
 *  and the reason this module has no bare `try`/`catch` left (field-report #20: `caches.open()`
 *  and `cache.match()` sit ahead of the network, so a jammed storage handle wedged the first
 *  open and the cached one alike, and the handle is per-page, so reopening the document
 *  re-entered the same jam). The mechanism is `lib/deadline.ts`'s, shared with the Dexie
 *  snapshot read; this binds it to this store's phase and bound. */
function bestEffortCache<T>(work: () => Promise<T>, fallback: T): Promise<T> {
  return bestEffort(LOCAL_READ_PHASE.DOC_BLOB, LOCAL_READ_TIMEOUT_MS.HANDLE, work, fallback);
}
