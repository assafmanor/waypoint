...
//
// There are two such paths now: an uploaded avatar's bytes (ADR-0133 §12) and an enrichment
// image's (ADR-0167 §1). Both are root-relative and assembled server-side precisely so no
// client knows the route shape, which is also why the origin cannot be baked into them.
