...
- `mockups/booking-presentation-v1.html` — historical design record for **booking presentation across the Trip-mode Home & Index** (ADR-0059 applying ADR-0063's per-category time-behaviour profile — **Accepted, shipped** session 33): the board hero's transition-moment surfacing (hotel check-in/out, transport departure/arrival), the "inside a booking" ambient/teal treatment, the shared row/detail/hero grammar, and the glance's uncounted check-in/out markers (ADR-0054 rebased). Kept for the design rationale, not a build spec.
