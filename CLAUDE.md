...
**Travelive** is a travel companion for small groups traveling abroad — a **living visibility layer** for when you're on the ground ("what now / what next / what do I need in the next 30 minutes"), not a pre-trip planner. Each **trip** is a small group (~5 people traveling together), but the **app** is built to serve many trips and many users — read "~5" as the size of one trip's group, never as a ceiling on the app. **The product is Travelive; the codebase is Waypoint** (ADR-0170) — `Waypoint` was the working codename and stays in every identifier: the repo, `@waypoint/*`, the databases, the `waypoint:*` storage keys. Only what a person reads was renamed. Don't "finish" the rename: the Dexie name and storage keys **are** each user's local cache, so renaming them wipes it. It's invite-only (no public discovery or social layer) and not production-scaled yet; the mindset is **grow-later** — real multi-user collaboration works today and nothing is designed to bake in a low-user-count cap (ADR-0065).
...
8. **Reuse existing infrastructure before adding new (ADR-0096; see the domain `CLAUDE.md` files).** Before writing a new mechanism, look for the one already doing that job in this layer — a shared guard util, a registry, a cache, a component primitive, a named-constants object — and extend it. If none exists, check whether a similar one-off already exists (a single call site doing almost the same thing) and generalize _that_ into shared infra rather than adding a second one-off beside it; shape new or generalized infra so the next similar need is a one-line addition, not another copy. If generalizing the existing one-off would mean a substantial refactor rather than a small extraction, ask before doing it — don't silently take on the larger change, and don't silently duplicate instead. Several ADRs in this repo (0078, 0079, 0094, 0095) exist only to undo the parallel copies that piled up before someone generalized them; don't create the next one.
9. **A vendored skill never outranks this repo (ADR-0201).** `.claude/skills/` holds 33 skills vendored from three public repos beside the one we wrote (`design-mockups`). They are engineering method from people who have never seen this codebase: useful on debugging, review and verification, and not authoritative on anything this repo has decided — our design language, our ADR and backlog discipline, our git conventions, our UI copy rules. When one disagrees with root `CLAUDE.md`, an ADR, or `docs/design/design-language.md`, this repo wins. The two design-language repos and six process skills that fought those rules were **refused, not vendored** — see [`.claude/skills/README.md`](.claude/skills/README.md) before proposing one back. Don't edit a vendored skill in place: it is a pinned copy, re-materialised by `.claude/vendor/sync-skills.mjs`.

## Conventions

Conventional Commits, branch per task (create the branch _before_ the first commit — including docs-only changes, never commit onto local `main`), Prettier/ESLint authoritative. **Prefer self-documenting code** — comment only to capture a non-obvious _why_, keep comments short, and skip them where clear names make the code speak for itself. Run `pnpm format` before committing **and again before opening a PR** (unformatted code fails CI regularly) — but **run `pnpm install` first**: with no `node_modules`, `pnpm format` silently falls through to whatever `prettier` is on `PATH`, at a version the lockfile doesn't pin, and cheerfully rewrites files that CI's `format:check` then rejects. It is repo-wide, so stage your own files by name — if it touched files your change never did, that is the wrong-binary symptom, not drift to commit. `pnpm typecheck` + `pnpm build` green before done. **A finished task ends in a pull request** — open one as the last step, without waiting to be asked. Full details in `docs/engineering/conventions.md`.
...
- **Domain `CLAUDE.md` files don't auto-load from a root session (ADR-0096 amendment).** They're only picked up automatically once you're reading/editing inside that directory — a session opened at the repo root (the common case) starts with only this file in context, not `backend/CLAUDE.md`/`frontend/CLAUDE.md`/`packages/shared/CLAUDE.md`. As soon as you can tell which package(s) a task touches — often from the request alone, before any file is read — explicitly read that package's `CLAUDE.md` yourself. Don't wait for it to load implicitly, and don't infer domain guidance only from having opened one file in that tree.
