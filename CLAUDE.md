...
9. **A vendored skill never outranks this repo (ADR-0200).** `.claude/skills/` holds 47 skills vendored from five public repos beside the one we wrote (`design-mockups`). They are advice from people who have never seen this codebase: useful for method, wrong by default on our palette, our git and docs conventions, and our UI copy rules. When one disagrees with root `CLAUDE.md`, an ADR, or `docs/design/design-language.md`, this repo wins — see [`.claude/skills/README.md`](.claude/skills/README.md) for the conflicts that are already known, and don't edit a vendored skill in place: it is a pinned copy, re-materialised by `.claude/vendor/sync-skills.mjs`.
