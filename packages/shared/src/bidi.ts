...
/**
 * **A one-letter Hebrew prefix, bound the way Hebrew binds it.**
 *
 * `ב` attaches directly to a Hebrew word — `באיסלנד` — and takes a MAQAF before a foreign
 * word or an acronym, where `בIceland` would be unreadable: `ב-Iceland`. Writing the hyphen
 * unconditionally gives `ב-איסלנד`, which is simply wrong (owner, 2026-08-31).
 *
 * Here rather than in either i18n file because BOTH renderers need it and the words are not
 * its business: the prefix is an argument. It asks `baseDirection`, looking past any bidi
 * controls the caller already wrapped the value in.
 */
export function bindPrefix(prefix: string, value: string): string {
  return `${prefix}${baseDirection(withoutBidiControls(value)) === 'rtl' ? '' : '-'}${value}`;
}
